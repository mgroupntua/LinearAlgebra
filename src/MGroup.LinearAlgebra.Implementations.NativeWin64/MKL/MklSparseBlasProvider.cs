...
namespace MGroup.LinearAlgebra.Implementations.NativeWin64.MKL
{
	...
	public class MklSparseBlasProvider : ISparseBlasProvider
	{
		...
		{
			var trans = "N"; //TODO: not sure about this
			var matdscrA = "SUNF"; // symmetric, upper, non-unit, fortran indexing
			var alpha = 1.0;
			var beta = 0.0;
		...
		{
			var ldB = transposeA ? numRowsA : numColsA;
			var ldC = transposeA ? numColsA : numRowsA;
		...
		{
			var count = zeroBasedIndices.Length;
			var oneBasesIndices = new int[count];
			for (var i = 0; i < count; ++i) oneBasesIndices[i] = zeroBasedIndices[i] + 1;
