...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGELS
    {
                     ...
                     , DORMQR dormqr, DTRTRS dtrtrs, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _ilaenv = ilaenv; _dlabad = dlabad; _dlamch = dlamch; _dlange = dlange; 
            _dgelqf = dgelqf;_dgeqrf = dgeqrf; _dlascl = dlascl; _dlaset = dlaset; _dormlq = dormlq; 
            _dormqr = dormqr;_dtrtrs = dtrtrs; _xerbla = xerbla; 
        ...
        public DGELS()
        {
            ...
            var dtrtrs = new DTRTRS(lsame, dtrsm, xerbla);
                         ...
                         , ref double[] B, int offset_b, int LDB, ref double[] WORK, int offset_work, int LWORK, ref int INFO)
        {
            ...
            var LQUERY = false; var TPSD = false; var BROW = 0; var I = 0; var IASCL = 0; var IBSCL = 0; var J = 0; var MN = 0; 
            var NB = 0;var SCLLEN = 0; var WSIZE = 0; double ANRM = 0; double BIGNUM = 0; double BNRM = 0; double SMLNUM = 0; 
            var offset_rwork = 0;
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_b = -1 - LDB + offset_b;  var o_work = -1 + offset_work; 
            ...
            MN = Math.Min(M, N);
            LQUERY = LWORK ==  - 1;
            if (!(_lsame.Run(TRANS, "N") || _lsame.Run(TRANS, "T")))
            {
                INFO =  - 1;
            }
            else
            ...
            if (INFO == 0 || INFO ==  - 10)
            {
                // *
                TPSD = true;
                if (_lsame.Run(TRANS, "N")) TPSD = false;
                // *
                if (M >= N)
                {
                    NB = _ilaenv.Run(1, "DGEQRF", " ", M, N,  - 1,  - 1);
                    if (TPSD)
                    {
                        NB = Math.Max(NB, _ilaenv.Run(1, "DORMQR", "LN", M, NRHS, N,  - 1));
                    }
                    else
                    {
                        NB = Math.Max(NB, _ilaenv.Run(1, "DORMQR", "LT", M, NRHS, N,  - 1));
                    }
                }
                else
                {
                    NB = _ilaenv.Run(1, "DGELQF", " ", M, N,  - 1,  - 1);
                    if (TPSD)
                    {
                        NB = Math.Max(NB, _ilaenv.Run(1, "DORMLQ", "LT", N, NRHS, M,  - 1));
                    }
                    else
                    {
                        NB = Math.Max(NB, _ilaenv.Run(1, "DORMLQ", "LN", N, NRHS, M,  - 1));
                    }
                }
                // *
            ...
            {
                _dlaset.Run("Full", Math.Max(M, N), NRHS, ZERO, ZERO, ref B, offset_b
                                 , LDB);
                return;
            }
            // *
            ...
            // *
            ANRM = _dlange.Run("M", M, N, A, offset_a, LDA, ref RWORK, offset_rwork);
            IASCL = 0;
            ...
            BROW = M;
            if (TPSD) BROW = N;
            BNRM = _dlange.Run("M", BROW, NRHS, B, offset_b, LDB, ref RWORK, offset_rwork);
            IBSCL = 0;
            if (BNRM > ZERO && BNRM < SMLNUM)
            {
                ...
                // *        Scale matrix norm up to SMLNUM
                // *
                _dlascl.Run("G", 0, 0, BNRM, SMLNUM, BROW
            ...
            else
            {
                if (BNRM > BIGNUM)
                {
                    ...
                    // *        Scale matrix norm down to BIGNUM
                    // *
                    _dlascl.Run("G", 0, 0, BNRM, BIGNUM, BROW
                                     , NRHS, ref B, offset_b, LDB, ref INFO);
                    IBSCL = 2;
                }
            }
            ...
            if (M >= N)
            {
                ...
                // *
                _dgeqrf.Run(M, N, ref A, offset_a, LDA, ref WORK, 1 + o_work, ref WORK, MN + 1 + o_work
                                 , LWORK - MN, ref INFO);
                // *
                ...
                if (!TPSD)
                {
                    ...
                    // *
                    _dormqr.Run("Left", "Transpose", M, NRHS, N, ref A, offset_a
                    ...
                    // *
                    _dtrtrs.Run("Upper", "No transpose", "Non-unit", N, NRHS, A, offset_a
                ...
                else
                {
                    ...
                    // *           B(1:N,1:NRHS) := inv(R') * B(1:N,1:NRHS)
                    // *
                    _dtrtrs.Run("Upper", "Transpose", "Non-unit", N, NRHS, A, offset_a
                                     , LDA, ref B, offset_b, LDB, ref INFO);
                    // *
                    if (INFO > 0)
                    {
                    ...
                    // *
                    _dormqr.Run("Left", "No transpose", M, NRHS, N, ref A, offset_a
                                     , LDA, WORK, 1 + o_work, ref B, offset_b, LDB, ref WORK, MN + 1 + o_work, LWORK - MN
                                     , ref INFO);
                    // *
                    // *           workspace at least NRHS, optimally NRHS*NB
            ...
            else
            {
                ...
                // *
                _dgelqf.Run(M, N, ref A, offset_a, LDA, ref WORK, 1 + o_work, ref WORK, MN + 1 + o_work
                                 , LWORK - MN, ref INFO);
                // *
                ...
                if (!TPSD)
                {
                    ...
                    // *
                    _dtrtrs.Run("Lower", "No transpose", "Non-unit", M, NRHS, A, offset_a
                                     , LDA, ref B, offset_b, LDB, ref INFO);
                    // *
                    if (INFO > 0)
                    {
                    ...
                    // *
                    _dormlq.Run("Left", "Transpose", N, NRHS, M, ref A, offset_a
                                     , LDA, WORK, 1 + o_work, ref B, offset_b, LDB, ref WORK, MN + 1 + o_work, LWORK - MN
                                     , ref INFO);
                    // *
                    // *           workspace at least NRHS, optimally NRHS*NB
                ...
                else
                {
                    ...
                    // *
                    _dormlq.Run("Left", "No transpose", N, NRHS, M, ref A, offset_a
                                     , LDA, WORK, 1 + o_work, ref B, offset_b, LDB, ref WORK, MN + 1 + o_work, LWORK - MN
                                     , ref INFO);
                    // *
                    // *           workspace at least NRHS, optimally NRHS*NB
                    ...
                    // *           B(1:M,1:NRHS) := inv(L') * B(1:M,1:NRHS)
                    // *
                    _dtrtrs.Run("Lower", "Transpose", "Non-unit", M, NRHS, A, offset_a
                                     , LDA, ref B, offset_b, LDB, ref INFO);
                    // *
                    if (INFO > 0)
                    {
            ...
            if (IASCL == 1)
            {
                _dlascl.Run("G", 0, 0, ANRM, SMLNUM, SCLLEN
                                 , NRHS, ref B, offset_b, LDB, ref INFO);
            }
            else
            {
                if (IASCL == 2)
                {
                    _dlascl.Run("G", 0, 0, ANRM, BIGNUM, SCLLEN
                                     , NRHS, ref B, offset_b, LDB, ref INFO);
                }
            }
            if (IBSCL == 1)
            {
                _dlascl.Run("G", 0, 0, SMLNUM, BNRM, SCLLEN
                                 , NRHS, ref B, offset_b, LDB, ref INFO);
            }
            else
            {
                if (IBSCL == 2)
                {
                    _dlascl.Run("G", 0, 0, BIGNUM, BNRM, SCLLEN
                                     , NRHS, ref B, offset_b, LDB, ref INFO);
                }
            }
            // *
