...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DTGSJA
    {
        ...
        public DTGSJA(LSAME lsame, DCOPY dcopy, DLAGS2 dlags2, DLAPLL dlapll, DLARTG dlartg, DLASET dlaset, DROT drot, DSCAL dscal, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _dcopy = dcopy; _dlags2 = dlags2; _dlapll = dlapll; _dlartg = dlartg; 
            _dlaset = dlaset;_drot = drot; _dscal = dscal; _xerbla = xerbla; 
        ...
        public DTGSJA()
        {
            ...
            var dlags2 = new DLAGS2(dlartg, dlasv2);
            var dlarfg = new DLARFG(dlamch, dlapy2, dnrm2, dscal);
            var dlapll = new DLAPLL(ddot, daxpy, dlarfg, dlas2);
                         ...
                         , ref int INFO)
        {
            ...
            var INITQ = false; var INITU = false; var INITV = false; var UPPER = false; var WANTQ = false; 
            var WANTU = false;var WANTV = false; var I = 0; var J = 0; var KCYCLE = 0; double A1 = 0; double A2 = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_b = -1 - LDB + offset_b;  var o_alpha = -1 + offset_alpha; 
             var o_beta = -1 + offset_beta; var o_u = -1 - LDU + offset_u;  var o_v = -1 - LDV + offset_v; 
             var o_q = -1 - LDQ + offset_q; var o_work = -1 + offset_work; 
            ...
            INITU = _lsame.Run(JOBU, "I");
            WANTU = INITU || _lsame.Run(JOBU, "U");
            // *
            INITV = _lsame.Run(JOBV, "I");
            WANTV = INITV || _lsame.Run(JOBV, "V");
            // *
            INITQ = _lsame.Run(JOBQ, "I");
            WANTQ = INITQ || _lsame.Run(JOBQ, "Q");
            // *
            INFO = 0;
            if (!(INITU || WANTU || _lsame.Run(JOBU, "N")))
            {
                INFO =  - 1;
            }
            else
            {
                if (!(INITV || WANTV || _lsame.Run(JOBV, "N")))
                {
                    INFO =  - 2;
                }
                else
                {
                    if (!(INITQ || WANTQ || _lsame.Run(JOBQ, "N")))
                    {
                        INFO =  - 3;
                    }
                    else
            ...
            if (INITU)
            {
                _dlaset.Run("Full", M, M, ZERO, ONE, ref U, offset_u
                                 , LDU);
            }
            if (INITV)
            {
                _dlaset.Run("Full", P, P, ZERO, ONE, ref V, offset_v
                                 , LDV);
            }
            if (INITQ)
            {
            ...
            for (KCYCLE = 1; KCYCLE <= MAXIT; KCYCLE++)
            {
                ...
                for (I = 1; I <= L - 1; I++)
                {
                    for (J = I + 1; J <= L; J++)
                    {
                        ...
                        // *
                        _dlags2.Run(UPPER, A1, A2, A3, B1, B2
                        ...
                        if (K + J <= M)
                        {
                            _drot.Run(L, ref A, K + J+(N - L + 1) * LDA + o_a, LDA, ref A, K + I+(N - L + 1) * LDA + o_a, LDA, CSU
                                           , SNU);
                        }
                        // *
                        // *              Update I-th and J-th rows of matrix B: V'*B
                        // *
                        _drot.Run(L, ref B, J+(N - L + 1) * LDB + o_b, LDB, ref B, I+(N - L + 1) * LDB + o_b, LDB, CSV
                                       , SNV);
                        // *
                        // *              Update (N-L+I)-th and (N-L+J)-th columns of matrices
                        // *              A and B: A*Q and B*Q
                        // *
                        _drot.Run(Math.Min(K + L, M), ref A, 1+(N - L + J) * LDA + o_a, 1, ref A, 1+(N - L + I) * LDA + o_a, 1, CSQ
                                       , SNQ);
                        // *
                        _drot.Run(L, ref B, 1+(N - L + J) * LDB + o_b, 1, ref B, 1+(N - L + I) * LDB + o_b, 1, CSQ
                                       , SNQ);
                        // *
                        if (UPPER)
                        {
                        ...
                        if (WANTU && K + J <= M)
                        {
                            _drot.Run(M, ref U, 1+(K + J) * LDU + o_u, 1, ref U, 1+(K + I) * LDU + o_u, 1, CSU
                                           , SNU);
                        }
                        // *
                        if (WANTV)
                        {
                            _drot.Run(P, ref V, 1+J * LDV + o_v, 1, ref V, 1+I * LDV + o_v, 1, CSV
                                           , SNV);
                        }
                        // *
                        if (WANTQ)
                        {
                            _drot.Run(N, ref Q, 1+(N - L + J) * LDQ + o_q, 1, ref Q, 1+(N - L + I) * LDQ + o_q, 1, CSQ
                                           , SNQ);
                        }
                        // *
                    }
                ...
                if (!UPPER)
                {
                    ...
                    {
                        _dcopy.Run(L - I + 1, A, K + I+(N - L + I) * LDA + o_a, LDA, ref WORK, offset_work, 1);
                        _dcopy.Run(L - I + 1, B, I+(N - L + I) * LDB + o_b, LDB, ref WORK, L + 1 + o_work, 1);
                        _dlapll.Run(L - I + 1, ref WORK, offset_work, 1, ref WORK, L + 1 + o_work, 1, ref SSMIN);
        ...
        LABEL50:;
            ...
            for (I = 1; I <= Math.Min(L, M - K); I++)
            {
                ...
                if (A1 != ZERO)
                {
                    ...
                    if (GAMMA < ZERO)
                    {
                        _dscal.Run(L - I + 1,  - ONE, ref B, I+(N - L + I) * LDB + o_b, LDB);
                        if (WANTV) _dscal.Run(P,  - ONE, ref V, 1+I * LDV + o_v, 1);
                    }
                    // *
                    _dlartg.Run(Math.Abs(GAMMA), ONE, ref BETA[K + I + o_beta], ref ALPHA[K + I + o_alpha], ref RWK);
                    // *
                    if (ALPHA[K + I + o_alpha] >= BETA[K + I + o_beta])
                    {
                        _dscal.Run(L - I + 1, ONE / ALPHA[K + I + o_alpha], ref A, K + I+(N - L + I) * LDA + o_a, LDA);
                    }
                    else
                    {
                        _dscal.Run(L - I + 1, ONE / BETA[K + I + o_beta], ref B, I+(N - L + I) * LDB + o_b, LDB);
                        _dcopy.Run(L - I + 1, B, I+(N - L + I) * LDB + o_b, LDB, ref A, K + I+(N - L + I) * LDA + o_a, LDA);
                    }
                    // *
                }
                else
