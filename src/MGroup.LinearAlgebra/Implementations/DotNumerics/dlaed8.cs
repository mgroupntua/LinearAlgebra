...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAED8
    {
                         ...
                         , ref double[] GIVNUM, int offset_givnum, ref int[] INDXP, int offset_indxp, ref int[] INDX, int offset_indx, ref int INFO)
        {
            ...
            var I = 0; var IMAX = 0; var J = 0; var JLAM = 0; var JMAX = 0; var JP = 0; var K2 = 0; var N1 = 0; var N1P1 = 0; 
            var N2 = 0;double C = 0; double EPS = 0; double S = 0; double T = 0; double TAU = 0; double TOL = 0; 
            ...
            #region Array Index Correction
             ...
             var o_dlamda = -1 + offset_dlamda; var o_q2 = -1 - LDQ2 + offset_q2;  var o_w = -1 + offset_w; 
             var o_perm = -1 + offset_perm; var o_givcol = -3 + offset_givcol;  var o_givnum = -3 + offset_givnum; 
             var o_indxp = -1 + offset_indxp; var o_indx = -1 + offset_indx; 
            ...
            I = 1;
            J = CUTPNT + 1;
            _dlamrg.Run(N1, N2, DLAMDA, offset_dlamda, 1, 1, ref INDX, offset_indx);
            for (I = 1; I <= N; I++)
            {
            ...
            if (RHO * Math.Abs(Z[IMAX + o_z]) <= TOL)
            {
                ...
                else
                {
                    ...
                    {
                        PERM[J + o_perm] = INDXQ[INDX[J + o_indx] + o_indxq];
                        _dcopy.Run(QSIZ, Q, 1+PERM[J + o_perm] * LDQ + o_q, 1, ref Q2, 1+J * LDQ2 + o_q2, 1);
                    }
                    _dlacpy.Run("A", QSIZ, N, Q2, 1+1 * LDQ2 + o_q2, LDQ2, ref Q, 1+1 * LDQ + o_q
                                     , LDQ);
                }
                return;
            }
        ...
        LABEL80:;
            ...
            else
            {
                ...
                if (Math.Abs(T * C * S) <= TOL)
                {
                    ...
                    if (ICOMPQ == 1)
                    {
                        _drot.Run(QSIZ, ref Q, 1+INDXQ[INDX[JLAM + o_indx] + o_indxq] * LDQ + o_q, 1, ref Q, 1+INDXQ[INDX[J + o_indx] + o_indxq] * LDQ + o_q, 1, C
                                       , S);
                    }
        ...
        LABEL110:;
            ...
            if (K < N)
            {
                if (ICOMPQ == 0)
                {
                    _dcopy.Run(N - K, DLAMDA, K + 1 + o_dlamda, 1, ref D, K + 1 + o_d, 1);
                }
                else
                {
                    _dcopy.Run(N - K, DLAMDA, K + 1 + o_dlamda, 1, ref D, K + 1 + o_d, 1);
                    _dlacpy.Run("A", QSIZ, N - K, Q2, 1+(K + 1) * LDQ2 + o_q2, LDQ2, ref Q, 1+(K + 1) * LDQ + o_q
                                     , LDQ);
                }
            }
            // *
