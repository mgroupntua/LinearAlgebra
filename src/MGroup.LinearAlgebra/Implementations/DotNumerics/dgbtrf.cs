...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGBTRF
    {
                      ...
                      , XERBLA xerbla)
        {
            ...
            _idamax = idamax; _ilaenv = ilaenv; _dcopy = dcopy; _dgbtf2 = dgbtf2; _dgemm = dgemm; 
            _dger = dger;_dlaswp = dlaswp; _dscal = dscal; _dswap = dswap; _dtrsm = dtrsm; 
            _xerbla = xerbla;
        ...
        public DGBTRF()
        {
            ...
            var dswap = new DSWAP();
            var lsame = new LSAME();
            var dlaswp = new DLASWP();
            var ilaenv = new ILAENV(ieeeck, iparmq);
            var dger = new DGER(xerbla);
            var dgbtf2 = new DGBTF2(idamax, dger, dscal, dswap, xerbla);
                         ...
                         , ref int[] IPIV, int offset_ipiv, ref int INFO)
        {
            ...
            var I = 0; var I2 = 0; var I3 = 0; var II = 0; var IP = 0; var J = 0; var J2 = 0; var J3 = 0; var JB = 0; var JJ = 0; 
            var JM = 0;var JP = 0; var JU = 0; var K2 = 0; var KM = 0; var KV = 0; var NB = 0; var NW = 0; double TEMP = 0; 
            var offset_work13 = 0; var o_work13 = -1 - LDWORK;var offset_work31 = 0; var o_work31 = -1 - LDWORK; 
            ...
            var WORK13_J = 0; var WORK31_J = 0; var AB_J = 0; var AB_0 = 0; var WORK13_JJ = 0; var AB_1 = 0; var AB_2 = 0; 
            ...
            #region Array Index Correction
             ...
             var o_ab = -1 - LDAB + offset_ab;  var o_ipiv = -1 + offset_ipiv; 
            ...
            // *
            NB = _ilaenv.Run(1, "DGBTRF", " ", M, N, KL, KU);
            // *
            ...
            if (NB <= 1 || NB > KL)
            {
                ...
                // *        Use unblocked code
                // *
                _dgbtf2.Run(M, N, KL, KU, ref AB, offset_ab, LDAB
            ...
            else
            {
                ...
                for (J = 1; NB >= 0 ? J <= Math.Min(M, N) : J >= Math.Min(M, N); J += NB)
                {
                    ...
                    for (JJ = J; JJ <= J + JB - 1; JJ++)
                    {
                        ...
                        // *
                        KM = Math.Min(KL, M - JJ);
                        JP = _idamax.Run(KM + 1, AB, KV + 1+JJ * LDAB + o_ab, 1);
                        ...
                        if (AB[KV + JP+JJ * LDAB + o_ab] != ZERO)
                        {
                            ...
                            if (JP != 1)
                            {
                                ...
                                {
                                    // *
                                    _dswap.Run(JB, ref AB, KV + 1 + JJ - J+J * LDAB + o_ab, LDAB - 1, ref AB, KV + JP + JJ - J+J * LDAB + o_ab, LDAB - 1);
                                }
                                else
                                {
                                    // *
                                    ...
                                    // *
                                    _dswap.Run(JJ - J, ref AB, KV + 1 + JJ - J+J * LDAB + o_ab, LDAB - 1, ref WORK31, JP + JJ - J - KL+1 * LDWORK + o_work31, LDWORK);
                                    _dswap.Run(J + JB - JJ, ref AB, KV + 1+JJ * LDAB + o_ab, LDAB - 1, ref AB, KV + JP+JJ * LDAB + o_ab, LDAB - 1);
                                }
                            }
                            // *
                            // *                 Compute multipliers
                            // *
                            _dscal.Run(KM, ONE / AB[KV + 1+JJ * LDAB + o_ab], ref AB, KV + 2+JJ * LDAB + o_ab, 1);
                            // *
                            ...
                            if (JM > JJ)
                            {
                                _dger.Run(KM, JM - JJ,  - ONE, AB, KV + 2+JJ * LDAB + o_ab, 1, AB, KV+(JJ + 1) * LDAB + o_ab
                        ...
                        if (NW > 0) _dcopy.Run(NW, AB, KV + KL + 1 - JJ + J+JJ * LDAB + o_ab, 1, ref WORK31, 1+(JJ - J + 1) * LDWORK + o_work31, 1);
                    }
                    if (J + JB <= N)
                    {
                        // *
                        ...
                        // *              A32.
                        // *
                        _dlaswp.Run(J2, ref AB, KV + 1 - JB+(J + JB) * LDAB + o_ab, LDAB - 1, 1, JB, IPIV, J + o_ipiv
                                         , 1);
                        // *
                        ...
                        if (J2 > 0)
                        {
                            ...
                            // *                 Update A12
                            // *
                            _dtrsm.Run("Left", "Lower", "No transpose", "Unit", JB, J2
                            ...
                            if (I2 > 0)
                            {
                                ...
                                // *                    Update A22
                                // *
                                _dgemm.Run("No transpose", "No transpose", I2, J2, JB,  - ONE
                            ...
                            if (I3 > 0)
                            {
                                ...
                                // *                    Update A32
                                // *
                                _dgemm.Run("No transpose", "No transpose", I3, J2, JB,  - ONE
                        ...
                        if (J3 > 0)
                        {
                            ...
                            // *
                            _dtrsm.Run("Left", "Lower", "No transpose", "Unit", JB, J3
                            ...
                            if (I2 > 0)
                            {
                                ...
                                // *                    Update A23
                                // *
                                _dgemm.Run("No transpose", "No transpose", I2, J3, JB,  - ONE
                            ...
                            if (I3 > 0)
                            {
                                ...
                                // *                    Update A33
                                // *
                                _dgemm.Run("No transpose", "No transpose", I3, J3, JB,  - ONE
                    ...
                    for (JJ = J + JB - 1; JJ >= J; JJ +=  - 1)
                    {
                        ...
                        if (JP != 1)
                        {
                            ...
                            if (JP + JJ - 1 < J + KL)
                            {
                                ...
                                // *
                                _dswap.Run(JJ - J, ref AB, KV + 1 + JJ - J+J * LDAB + o_ab, LDAB - 1, ref AB, KV + JP + JJ - J+J * LDAB + o_ab, LDAB - 1);
                            }
                            else
                            {
                                // *
                        ...
                        if (NW > 0) _dcopy.Run(NW, WORK31, 1+(JJ - J + 1) * LDWORK + o_work31, 1, ref AB, KV + KL + 1 - JJ + J+JJ * LDAB + o_ab, 1);
                    }
                }
            }
            // *
