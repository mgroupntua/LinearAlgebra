...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGGLSE
    {
        ...
        public DGGLSE(DAXPY daxpy, DCOPY dcopy, DGEMV dgemv, DGGRQF dggrqf, DORMQR dormqr, DORMRQ dormrq, DTRMV dtrmv, DTRTRS dtrtrs, XERBLA xerbla, ILAENV ilaenv)
        {
            ...
            _daxpy = daxpy; _dcopy = dcopy; _dgemv = dgemv; _dggrqf = dggrqf; _dormqr = dormqr; 
            _dormrq = dormrq;_dtrmv = dtrmv; _dtrtrs = dtrtrs; _xerbla = xerbla; _ilaenv = ilaenv; 
        ...
        public DGGLSE()
        {
            ...
            var dgerqf = new DGERQF(dgerq2, dlarfb, dlarft, xerbla, ilaenv);
            var dormr2 = new DORMR2(lsame, dlarf, xerbla);
            var dormrq = new DORMRQ(lsame, ilaenv, dlarfb, dlarft, dormr2, xerbla);
            var dggrqf = new DGGRQF(dgeqrf, dgerqf, dormrq, xerbla, ilaenv);
                         ...
                         , ref int INFO)
        {
            ...
            var LQUERY = false; var LOPT = 0; var LWKMIN = 0; var LWKOPT = 0; var MN = 0; var NB = 0; var NB1 = 0; var NB2 = 0; 
            var NB3 = 0;var NB4 = 0; var NR = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_b = -1 - LDB + offset_b;  var o_c = -1 + offset_c;  var o_d = -1 + offset_d; 
             var o_x = -1 + offset_x; var o_work = -1 + offset_work; 
            ...
            if (INFO == 0)
            {
                ...
                else
                {
                    ...
                    NB3 = _ilaenv.Run(1, "DORMQR", " ", M, N, P,  - 1);
                    NB4 = _ilaenv.Run(1, "DORMRQ", " ", M, N, P,  - 1);
            ...
            // *     orthogonal.
            // *
            _dggrqf.Run(P, M, N, ref B, offset_b, LDB, ref WORK, offset_work
            ...
            // *                       ( c2 ) M+P-N
            // *
            _dormqr.Run("Left", "Transpose", M, 1, MN, ref A, offset_a
            ...
            if (P > 0)
            {
                _dtrtrs.Run("Upper", "No transpose", "Non-unit", P, 1, B, 1+(N - P + 1) * LDB + o_b
                ...
                // *
                _dcopy.Run(P, D, offset_d, 1, ref X, N - P + 1 + o_x, 1);
                // *
                // *        Update c1
                // *
                _dgemv.Run("No transpose", N - P, P,  - ONE, A, 1+(N - P + 1) * LDA + o_a, LDA
            ...
            if (N > P)
            {
                _dtrtrs.Run("Upper", "No transpose", "Non-unit", N - P, 1, A, offset_a
                ...
                // *
                _dcopy.Run(N - P, C, offset_c, 1, ref X, offset_x, 1);
            }
            // *
            ...
            if (M < N)
            {
                ...
                if (NR > 0)
                {
                    _dgemv.Run("No transpose", NR, N - M,  - ONE, A, N - P + 1+(M + 1) * LDA + o_a, LDA
            ...
            if (NR > 0)
            {
                _dtrmv.Run("Upper", "No transpose", "Non unit", NR, A, N - P + 1+(N - P + 1) * LDA + o_a, LDA
                                , ref D, offset_d, 1);
                _daxpy.Run(NR,  - ONE, D, offset_d, 1, ref C, N - P + 1 + o_c, 1);
            }
            // *
            // *     Backward transformation x = Q'*x
            // *
            _dormrq.Run("Left", "Transpose", N, 1, P, ref B, offset_b
