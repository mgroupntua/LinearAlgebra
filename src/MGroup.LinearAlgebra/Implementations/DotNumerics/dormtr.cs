...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DORMTR
    {
        ...
        public DORMTR(LSAME lsame, ILAENV ilaenv, DORMQL dormql, DORMQR dormqr, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _ilaenv = ilaenv; _dormql = dormql; _dormqr = dormqr; _xerbla = xerbla; 
                         ...
                         , ref int INFO)
        {
            ...
            var LEFT = false; var LQUERY = false; var UPPER = false; var I1 = 0; var I2 = 0; var IINFO = 0; var LWKOPT = 0; 
            var MI = 0;var NB = 0; var NI = 0; var NQ = 0; var NW = 0; 
            ...
            if (INFO == 0)
            {
                if (UPPER)
                {
                    if (LEFT)
                    {
                        NB = _ilaenv.Run(1, "DORMQL", SIDE + TRANS, M - 1, N, M - 1,  - 1);
                    }
                    else
                    {
                        NB = _ilaenv.Run(1, "DORMQL", SIDE + TRANS, M, N - 1, N - 1,  - 1);
                    }
                }
                else
                {
                    if (LEFT)
                    {
                        NB = _ilaenv.Run(1, "DORMQR", SIDE + TRANS, M - 1, N, M - 1,  - 1);
                    }
                    else
                    {
                        NB = _ilaenv.Run(1, "DORMQR", SIDE + TRANS, M, N - 1, N - 1,  - 1);
                    }
                }
            ...
            if (UPPER)
            {
                ...
                // *
                _dormql.Run(SIDE, TRANS, MI, NI, NQ - 1, ref A, 1+2 * LDA + o_a
            ...
            else
            {
                ...
                _dormqr.Run(SIDE, TRANS, MI, NI, NQ - 1, ref A, 2+1 * LDA + o_a
