...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DTREVC
    {
                      ...
                      , DLABAD dlabad)
        {
            ...
            _lsame = lsame; _idamax = idamax; _ddot = ddot; _dlamch = dlamch; _daxpy = daxpy; 
            _dcopy = dcopy;_dgemv = dgemv; _dlaln2 = dlaln2; _dscal = dscal; _xerbla = xerbla; 
            _dlabad = dlabad;
        ...
        public DTREVC()
        {
            ...
            var dlaln2 = new DLALN2(dlamch, dladiv);
                         ...
                         , ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var ALLV = false; var BOTHV = false; var LEFTV = false; var OVER = false; var PAIR = false; var RIGHTV = false; 
            var SOMEV = false;var I = 0; var IERR = 0; var II = 0; var IP = 0; var IS = 0; var J = 0; var J1 = 0; var J2 = 0; 
            var JNXT = 0;var K = 0; var KI = 0; var N2 = 0; double BETA = 0; double BIGNUM = 0; double EMAX = 0; double OVFL = 0; 
            ...
            var VR_IS = 0; var VR_0 = 0; var VL_IS = 0; var VL_0 = 0; 
            ...
            #region Array Index Correction
             ...
             var o_select = -1 + offset_select;  var o_t = -1 - LDT + offset_t;  var o_vl = -1 - LDVL + offset_vl; 
             var o_vr = -1 - LDVR + offset_vr; var o_work = -1 + offset_work; 
            ...
            BOTHV = _lsame.Run(SIDE, "B");
            RIGHTV = _lsame.Run(SIDE, "R") || BOTHV;
            LEFTV = _lsame.Run(SIDE, "L") || BOTHV;
            // *
            ALLV = _lsame.Run(HOWMNY, "A");
            OVER = _lsame.Run(HOWMNY, "B");
            SOMEV = _lsame.Run(HOWMNY, "S");
            // *
            INFO = 0;
            if (!RIGHTV && !LEFTV)
            {
            ...
            // *
            UNFL = _dlamch.Run("Safe minimum");
            OVFL = ONE / UNFL;
            _dlabad.Run(ref UNFL, ref OVFL);
                    ...
                    if (IP == 0)
                    {
                        ...
                        for (J = KI - 1; J >= 1; J +=  - 1)
                        {
                            ...
                            if (J1 == J2)
                            {
                                ...
                                // *
                                _dlaln2.Run(false, 1, 1, SMIN, ONE, T, J+J * LDT + o_t
                                ...
                                // *                    Scale if necessary
                                // *
                                if (SCALE != ONE) _dscal.Run(KI, SCALE, ref WORK, 1 + N + o_work, 1);
                                WORK[J + N + o_work] = X[1+1 * 2 + o_x];
                                // *
                                // *                    Update right-hand side
                                // *
                                _daxpy.Run(J - 1,  - X[1+1 * 2 + o_x], T, 1+J * LDT + o_t, 1, ref WORK, 1 + N + o_work, 1);
                                // *
                            }
                            else
                            {
                                ...
                                // *
                                _dlaln2.Run(false, 2, 1, SMIN, ONE, T, J - 1+(J - 1) * LDT + o_t
                                ...
                                // *                    Update right-hand side
                                // *
                                _daxpy.Run(J - 2,  - X[1+1 * 2 + o_x], T, 1+(J - 1) * LDT + o_t, 1, ref WORK, 1 + N + o_work, 1);
                                _daxpy.Run(J - 2,  - X[2+1 * 2 + o_x], T, 1+J * LDT + o_t, 1, ref WORK, 1 + N + o_work, 1);
                            }
                        LABEL60:;
                        }
                        // *
                        ...
                        if (!OVER)
                        {
                            _dcopy.Run(KI, WORK, 1 + N + o_work, 1, ref VR, 1+IS * LDVR + o_vr, 1);
                            // *
                            II = _idamax.Run(KI, VR, 1+IS * LDVR + o_vr, 1);
                            REMAX = ONE / Math.Abs(VR[II+IS * LDVR + o_vr]);
                            _dscal.Run(KI, REMAX, ref VR, 1+IS * LDVR + o_vr, 1);
                            // *
                            VR_IS = IS * LDVR + o_vr;
                            for (K = KI + 1; K <= N; K++)
                            {
                        ...
                        else
                        {
                            if (KI > 1)
                            {
                                _dgemv.Run("N", N, KI - 1, ONE, VR, offset_vr, LDVR
                                                , WORK, 1 + N + o_work, 1, WORK[KI + N + o_work], ref VR, 1+KI * LDVR + o_vr, 1);
                            }
                            // *
                            II = _idamax.Run(N, VR, 1+KI * LDVR + o_vr, 1);
                            REMAX = ONE / Math.Abs(VR[II+KI * LDVR + o_vr]);
                            _dscal.Run(N, REMAX, ref VR, 1+KI * LDVR + o_vr, 1);
                        }
                        // *
                    }
                    else
                    {
                        ...
                        for (J = KI - 2; J >= 1; J +=  - 1)
                        {
                            ...
                            if (J1 == J2)
                            {
                                ...
                                // *                    1-by-1 diagonal block
                                // *
                                _dlaln2.Run(false, 1, 2, SMIN, ONE, T, J+J * LDT + o_t
                                                 , LDT, ONE, ONE, WORK, J + N + o_work, N, WR
                                ...
                                if (SCALE != ONE)
                                {
                                    _dscal.Run(KI, SCALE, ref WORK, 1 + N + o_work, 1);
                                    _dscal.Run(KI, SCALE, ref WORK, 1 + N2 + o_work, 1);
                                }
                                WORK[J + N + o_work] = X[1+1 * 2 + o_x];
                                ...
                                // *
                                _daxpy.Run(J - 1,  - X[1+1 * 2 + o_x], T, 1+J * LDT + o_t, 1, ref WORK, 1 + N + o_work, 1);
                                _daxpy.Run(J - 1,  - X[1+2 * 2 + o_x], T, 1+J * LDT + o_t, 1, ref WORK, 1 + N2 + o_work, 1);
                                // *
                            }
                            else
                            {
                                ...
                                // *                    2-by-2 diagonal block
                                // *
                                _dlaln2.Run(false, 2, 2, SMIN, ONE, T, J - 1+(J - 1) * LDT + o_t
                                                 , LDT, ONE, ONE, WORK, J - 1 + N + o_work, N, WR
                                                 , WI, ref X, offset_x, 2, ref SCALE, ref XNORM, ref IERR);
                                // *
                                ...
                                _daxpy.Run(J - 2,  - X[1+1 * 2 + o_x], T, 1+(J - 1) * LDT + o_t, 1, ref WORK, 1 + N + o_work, 1);
                                _daxpy.Run(J - 2,  - X[2+1 * 2 + o_x], T, 1+J * LDT + o_t, 1, ref WORK, 1 + N + o_work, 1);
                                _daxpy.Run(J - 2,  - X[1+2 * 2 + o_x], T, 1+(J - 1) * LDT + o_t, 1, ref WORK, 1 + N2 + o_work, 1);
                                _daxpy.Run(J - 2,  - X[2+2 * 2 + o_x], T, 1+J * LDT + o_t, 1, ref WORK, 1 + N2 + o_work, 1);
                            }
                        LABEL90:;
                        }
                        // *
                        ...
                        if (!OVER)
                        {
                            _dcopy.Run(KI, WORK, 1 + N + o_work, 1, ref VR, 1+(IS - 1) * LDVR + o_vr, 1);
                            _dcopy.Run(KI, WORK, 1 + N2 + o_work, 1, ref VR, 1+IS * LDVR + o_vr, 1);
                            // *
                            EMAX = ZERO;
                            for (K = 1; K <= KI; K++)
                            {
                            ...
                            // *
                            REMAX = ONE / EMAX;
                            _dscal.Run(KI, REMAX, ref VR, 1+(IS - 1) * LDVR + o_vr, 1);
                            _dscal.Run(KI, REMAX, ref VR, 1+IS * LDVR + o_vr, 1);
                            // *
                            VR_0 = (IS - 1) * LDVR + o_vr;
                            VR_IS = IS * LDVR + o_vr;
                        ...
                        else
                        {
                            ...
                            if (KI > 2)
                            {
                                _dgemv.Run("N", N, KI - 2, ONE, VR, offset_vr, LDVR
                            ...
                            {
                                _dscal.Run(N, WORK[KI - 1 + N + o_work], ref VR, 1+(KI - 1) * LDVR + o_vr, 1);
                                _dscal.Run(N, WORK[KI + N2 + o_work], ref VR, 1+KI * LDVR + o_vr, 1);
                            }
                            // *
                            EMAX = ZERO;
                            for (K = 1; K <= N; K++)
                            ...
                            REMAX = ONE / EMAX;
                            _dscal.Run(N, REMAX, ref VR, 1+(KI - 1) * LDVR + o_vr, 1);
                            _dscal.Run(N, REMAX, ref VR, 1+KI * LDVR + o_vr, 1);
                        }
                    }
                    // *
            ...
            if (LEFTV)
            {
                ...
                LABEL150:;
                    ...
                    if (IP == 0)
                    {
                        ...
                        for (J = KI + 1; J <= N; J++)
                        {
                            ...
                            if (J1 == J2)
                            {
                                ...
                                {
                                    REC = ONE / VMAX;
                                    _dscal.Run(N - KI + 1, REC, ref WORK, KI + N + o_work, 1);
                                    VMAX = ONE;
                                    VCRIT = BIGNUM;
                                }
                                // *
                                WORK[J + N + o_work] -= _ddot.Run(J - KI - 1, T, KI + 1+J * LDT + o_t, 1, WORK, KI + 1 + N + o_work, 1);
                                // *
                                ...
                                // *                    Scale if necessary
                                // *
                                if (SCALE != ONE) _dscal.Run(N - KI + 1, SCALE, ref WORK, KI + N + o_work, 1);
                                WORK[J + N + o_work] = X[1+1 * 2 + o_x];
                            ...
                            else
                            {
                                ...
                                WORK[J + N + o_work] -= _ddot.Run(J - KI - 1, T, KI + 1+J * LDT + o_t, 1, WORK, KI + 1 + N + o_work, 1);
                                // *
                                WORK[J + 1 + N + o_work] -= _ddot.Run(J - KI - 1, T, KI + 1+(J + 1) * LDT + o_t, 1, WORK, KI + 1 + N + o_work, 1);
                                // *
                                // *                    Solve
                                // *                      [T(J,J)-WR   T(J,J+1)     ]'* X = SCALE*( WORK1 )
                                // *                      [T(J+1,J)    T(J+1,J+1)-WR]             ( WORK2 )
                                // *
                                _dlaln2.Run(true, 2, 1, SMIN, ONE, T, J+J * LDT + o_t
                                                 , LDT, ONE, ONE, WORK, J + N + o_work, N, WR
                                                 , ZERO, ref X, offset_x, 2, ref SCALE, ref XNORM, ref IERR);
                                // *
                                // *                    Scale if necessary
                        ...
                        if (!OVER)
                        {
                            _dcopy.Run(N - KI + 1, WORK, KI + N + o_work, 1, ref VL, KI+IS * LDVL + o_vl, 1);
                            // *
                            II = _idamax.Run(N - KI + 1, VL, KI+IS * LDVL + o_vl, 1) + KI - 1;
                            REMAX = ONE / Math.Abs(VL[II+IS * LDVL + o_vl]);
                            _dscal.Run(N - KI + 1, REMAX, ref VL, KI+IS * LDVL + o_vl, 1);
                            // *
                            VL_IS = IS * LDVL + o_vl;
                            for (K = 1; K <= KI - 1; K++)
                            {
                        ...
                        else
                        {
                            ...
                            if (KI < N)
                            {
                                _dgemv.Run("N", N, N - KI, ONE, VL, 1+(KI + 1) * LDVL + o_vl, LDVL
                                                , WORK, KI + 1 + N + o_work, 1, WORK[KI + N + o_work], ref VL, 1+KI * LDVL + o_vl, 1);
                            }
                            // *
                            II = _idamax.Run(N, VL, 1+KI * LDVL + o_vl, 1);
                            REMAX = ONE / Math.Abs(VL[II+KI * LDVL + o_vl]);
                            _dscal.Run(N, REMAX, ref VL, 1+KI * LDVL + o_vl, 1);
                            // *
                        }
                        // *
                    }
                    else
                    {
                        ...
                        for (J = KI + 2; J <= N; J++)
                        {
                            ...
                            if (J1 == J2)
                            {
                                ...
                                if (WORK[J + o_work] > VCRIT)
                                {
                                    REC = ONE / VMAX;
                                    _dscal.Run(N - KI + 1, REC, ref WORK, KI + N + o_work, 1);
                                    _dscal.Run(N - KI + 1, REC, ref WORK, KI + N2 + o_work, 1);
                                    VMAX = ONE;
                                    VCRIT = BIGNUM;
                                }
                                // *
                                WORK[J + N + o_work] -= _ddot.Run(J - KI - 2, T, KI + 2+J * LDT + o_t, 1, WORK, KI + 2 + N + o_work, 1);
                                WORK[J + N2 + o_work] -= _ddot.Run(J - KI - 2, T, KI + 2+J * LDT + o_t, 1, WORK, KI + 2 + N2 + o_work, 1);
                                // *
                                ...
                                if (SCALE != ONE)
                                {
                                    _dscal.Run(N - KI + 1, SCALE, ref WORK, KI + N + o_work, 1);
                                    _dscal.Run(N - KI + 1, SCALE, ref WORK, KI + N2 + o_work, 1);
                                }
                                WORK[J + N + o_work] = X[1+1 * 2 + o_x];
                                WORK[J + N2 + o_work] = X[1+2 * 2 + o_x];
                            ...
                            else
                            {
                                ...
                                WORK[J + N2 + o_work] -= _ddot.Run(J - KI - 2, T, KI + 2+J * LDT + o_t, 1, WORK, KI + 2 + N2 + o_work, 1);
                                // *
                                WORK[J + 1 + N + o_work] -= _ddot.Run(J - KI - 2, T, KI + 2+(J + 1) * LDT + o_t, 1, WORK, KI + 2 + N + o_work, 1);
                                // *
                                WORK[J + 1 + N2 + o_work] -= _ddot.Run(J - KI - 2, T, KI + 2+(J + 1) * LDT + o_t, 1, WORK, KI + 2 + N2 + o_work, 1);
                                // *
                                // *                    Solve 2-by-2 complex linear equation
                                // *                      ([T(j,j)   T(j,j+1)  ]'-(wr-i*wi)*I)*X = SCALE*B
                                // *                      ([T(j+1,j) T(j+1,j+1)]             )
                                // *
                                _dlaln2.Run(true, 2, 2, SMIN, ONE, T, J+J * LDT + o_t
                                                 , LDT, ONE, ONE, WORK, J + N + o_work, N, WR
                                                 ,  - WI, ref X, offset_x, 2, ref SCALE, ref XNORM, ref IERR);
                                // *
                                // *                    Scale if necessary
                        ...
                        {
                            _dcopy.Run(N - KI + 1, WORK, KI + N + o_work, 1, ref VL, KI+IS * LDVL + o_vl, 1);
                            _dcopy.Run(N - KI + 1, WORK, KI + N2 + o_work, 1, ref VL, KI+(IS + 1) * LDVL + o_vl, 1);
                            // *
                            EMAX = ZERO;
                            for (K = KI; K <= N; K++)
                            {
                            ...
                            REMAX = ONE / EMAX;
                            _dscal.Run(N - KI + 1, REMAX, ref VL, KI+IS * LDVL + o_vl, 1);
                            _dscal.Run(N - KI + 1, REMAX, ref VL, KI+(IS + 1) * LDVL + o_vl, 1);
                            // *
                            VL_IS = IS * LDVL + o_vl;
                            VL_0 = (IS + 1) * LDVL + o_vl;
                            for (K = 1; K <= KI - 1; K++)
                        ...
                        else
                        {
                            if (KI < N - 1)
                            {
                                _dgemv.Run("N", N, N - KI - 1, ONE, VL, 1+(KI + 2) * LDVL + o_vl, LDVL
                            ...
                            {
                                _dscal.Run(N, WORK[KI + N + o_work], ref VL, 1+KI * LDVL + o_vl, 1);
                                _dscal.Run(N, WORK[KI + 1 + N2 + o_work], ref VL, 1+(KI + 1) * LDVL + o_vl, 1);
                            }
                            // *
                            EMAX = ZERO;
                            for (K = 1; K <= N; K++)
                            ...
                            REMAX = ONE / EMAX;
                            _dscal.Run(N, REMAX, ref VL, 1+KI * LDVL + o_vl, 1);
                            _dscal.Run(N, REMAX, ref VL, 1+(KI + 1) * LDVL + o_vl, 1);
                            // *
                        }
                        // *
                    }
