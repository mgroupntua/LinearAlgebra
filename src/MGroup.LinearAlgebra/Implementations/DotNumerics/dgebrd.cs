...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGEBRD
    {
        ...
        public DGEBRD(DGEBD2 dgebd2, DGEMM dgemm, DLABRD dlabrd, XERBLA xerbla, ILAENV ilaenv)
        {
            ...
            _dgebd2 = dgebd2; _dgemm = dgemm; _dlabrd = dlabrd; _xerbla = xerbla; _ilaenv = ilaenv; 
                         ...
                         , ref double[] TAUQ, int offset_tauq, ref double[] TAUP, int offset_taup, ref double[] WORK, int offset_work, int LWORK, ref int INFO)
        {
            ...
            var LQUERY = false; var I = 0; var IINFO = 0; var J = 0; var LDWRKX = 0; var LDWRKY = 0; var LWKOPT = 0; 
            var MINMN = 0;var NB = 0; var NBMIN = 0; var NX = 0; double WS = 0; 
            ...
            INFO = 0;
            NB = Math.Max(1, _ilaenv.Run(1, "DGEBRD", " ", M, N,  - 1,  - 1));
            LWKOPT = (M + N) * NB;
            ...
            if (NB > 1 && NB < MINMN)
            {
                ...
                // *
                NX = Math.Max(NB, _ilaenv.Run(3, "DGEBRD", " ", M, N,  - 1,  - 1));
                // *
                ...
                if (NX < MINMN)
                {
                    ...
                    if (LWORK < WS)
                    {
                        ...
                        // *
                        NBMIN = _ilaenv.Run(2, "DGEBRD", " ", M, N,  - 1,  - 1);
                        if (LWORK >= (M + N) * NBMIN)
                        {
                            NB = LWORK / (M + N);
                        }
            ...
            for (I = 1; NB >= 0 ? I <= MINMN - NX : I >= MINMN - NX; I += NB)
            {
                ...
                // *        part of the matrix
                // *
                _dlabrd.Run(M - I + 1, N - I + 1, NB, ref A, I+I * LDA + o_a, LDA, ref D, I + o_d
                ...
                // *
                _dgemm.Run("No transpose", "Transpose", M - I - NB + 1, N - I - NB + 1, NB,  - ONE
                                , A, I + NB+I * LDA + o_a, LDA, WORK, LDWRKX * NB + NB + 1 + o_work, LDWRKY, ONE, ref A, I + NB+(I + NB) * LDA + o_a
                                , LDA);
                _dgemm.Run("No transpose", "No transpose", M - I - NB + 1, N - I - NB + 1, NB,  - ONE
            ...
            // *
            _dgebd2.Run(M - I + 1, N - I + 1, ref A, I+I * LDA + o_a, LDA, ref D, I + o_d, ref E, I + o_e
