...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAEXC
    {
        ...
        public DLAEXC(DLAMCH dlamch, DLANGE dlange, DLACPY dlacpy, DLANV2 dlanv2, DLARFG dlarfg, DLARFX dlarfx, DLARTG dlartg, DLASY2 dlasy2, DROT drot)
        {
            ...
            _dlamch = dlamch; _dlange = dlange; _dlacpy = dlacpy; _dlanv2 = dlanv2; _dlarfg = dlarfg; 
            _dlarfx = dlarfx;_dlartg = dlartg; _dlasy2 = dlasy2; _drot = drot; 
                         ...
                         , int J1, int N1, int N2, ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var IERR = 0; var J2 = 0; var J3 = 0; var J4 = 0; var K = 0; var ND = 0; double CS = 0; double DNORM = 0; 
            ...
            double WR1 = 0;double WR2 = 0; double XNORM = 0; var offset_d = 0; var o_d = -1 - LDD; var offset_u = 0; var o_u = -1; 
            var offset_u1 = 0; var o_u1 = -1;var offset_u2 = 0; var o_u2 = -1; var offset_x = 0; var o_x = -1 - LDX; 
            ...
            #region Array Index Correction
             ...
             var o_t = -1 - LDT + offset_t;  var o_q = -1 - LDQ + offset_q;  var o_work = -1 + offset_work; 
            ...
            if (N1 == 1 && N2 == 1)
            {
                ...
                // *
                _dlartg.Run(T[J1+J2 * LDT + o_t], T22 - T11, ref CS, ref SN, ref TEMP);
                // *
                // *        Apply transformation to the matrix T.
                // *
                if (J3 <= N)
                {
                    _drot.Run(N - J1 - 1, ref T, J1+J3 * LDT + o_t, LDT, ref T, J2+J3 * LDT + o_t, LDT, CS
                                   , SN);
                }
                _drot.Run(J1 - 1, ref T, 1+J1 * LDT + o_t, 1, ref T, 1+J2 * LDT + o_t, 1, CS
                               , SN);
                // *
                T[J1+J1 * LDT + o_t] = T22;
                T[J2+J2 * LDT + o_t] = T11;
                ...
                if (WANTQ)
                {
                    ...
                    // *
                    _drot.Run(N, ref Q, 1+J1 * LDQ + o_q, 1, ref Q, 1+J2 * LDQ + o_q, 1, CS
                                   , SN);
                }
                // *
            }
            else
            {
                ...
                // *
                ND = N1 + N2;
                _dlacpy.Run("Full", ND, ND, T, J1+J1 * LDT + o_t, LDT, ref D, offset_d
                                 , LDD);
                DNORM = _dlange.Run("Max", ND, ND, D, offset_d, LDD, ref WORK, offset_work);
                // *
                // *        Compute machine-dependent threshold for test for accepting
                // *        swap.
                // *
                EPS = _dlamch.Run("P");
                SMLNUM = _dlamch.Run("S") / EPS;
                THRESH = Math.Max(TEN * EPS * DNORM, SMLNUM);
                // *
                // *        Solve T11*X - X*T22 = scale*T12 for X.
                // *
                _dlasy2.Run(false, false,  - 1, N1, N2, D, offset_d
            ...
            LABEL10:;
                ...
                _dlarfg.Run(3, ref U[3 + o_u], ref U, offset_u, 1, ref TAU);
                U[3 + o_u] = ONE;
                T11 = T[J1+J1 * LDT + o_t];
                // *
                // *        Perform swap provisionally on diagonal block in D.
                // *
                _dlarfx.Run("L", 3, 3, U, offset_u, TAU, ref D, offset_d
                                 , LDD, ref WORK, offset_work);
                _dlarfx.Run("R", 3, 3, U, offset_u, TAU, ref D, offset_d
                                 , LDD, ref WORK, offset_work);
                // *
                ...
                // *
                _dlarfx.Run("L", 3, N - J1 + 1, U, offset_u, TAU, ref T, J1+J1 * LDT + o_t
                                 , LDT, ref WORK, offset_work);
                _dlarfx.Run("R", J2, 3, U, offset_u, TAU, ref T, 1+J1 * LDT + o_t
                                 , LDT, ref WORK, offset_work);
                // *
                ...
                if (WANTQ)
                {
                    ...
                    // *           Accumulate transformation in the matrix Q.
                    // *
                    _dlarfx.Run("R", N, 3, U, offset_u, TAU, ref Q, 1+J1 * LDQ + o_q
            ...
            LABEL20:;
                ...
                U[3 + o_u] = SCALE;
                _dlarfg.Run(3, ref U[1 + o_u], ref U, 2 + o_u, 1, ref TAU);
                U[1 + o_u] = ONE;
                T33 = T[J3+J3 * LDT + o_t];
                // *
                // *        Perform swap provisionally on diagonal block in D.
                ...
                // *        Accept swap: apply transformation to the entire matrix T.
                // *
                _dlarfx.Run("R", J3, 3, U, offset_u, TAU, ref T, 1+J1 * LDT + o_t
                                 , LDT, ref WORK, offset_work);
                _dlarfx.Run("L", 3, N - J1, U, offset_u, TAU, ref T, J1+J2 * LDT + o_t
                                 , LDT, ref WORK, offset_work);
                // *
                T[J1+J1 * LDT + o_t] = T33;
            ...
            LABEL30:;
                ...
                U1[3 + o_u1] = SCALE;
                _dlarfg.Run(3, ref U1[1 + o_u1], ref U1, 2 + o_u1, 1, ref TAU1);
                U1[1 + o_u1] = ONE;
                // *
                ...
                U2[3 + o_u2] = SCALE;
                _dlarfg.Run(3, ref U2[1 + o_u2], ref U2, 2 + o_u2, 1, ref TAU2);
                U2[1 + o_u2] = ONE;
                // *
                // *        Perform swap provisionally on diagonal block in D.
                // *
                _dlarfx.Run("L", 3, 4, U1, offset_u1, TAU1, ref D, offset_d
                                 , LDD, ref WORK, offset_work);
                _dlarfx.Run("R", 4, 3, U1, offset_u1, TAU1, ref D, offset_d
                                 , LDD, ref WORK, offset_work);
                _dlarfx.Run("L", 3, 4, U2, offset_u2, TAU2, ref D, 2+1 * LDD + o_d
                                 , LDD, ref WORK, offset_work);
                _dlarfx.Run("R", 4, 3, U2, offset_u2, TAU2, ref D, 1+2 * LDD + o_d
                                 , LDD, ref WORK, offset_work);
                // *
                // *        Test whether to reject swap.
                // *
                ...
                // *        Accept swap: apply transformation to the entire matrix T.
                // *
                _dlarfx.Run("L", 3, N - J1 + 1, U1, offset_u1, TAU1, ref T, J1+J1 * LDT + o_t
                                 , LDT, ref WORK, offset_work);
                _dlarfx.Run("R", J4, 3, U1, offset_u1, TAU1, ref T, 1+J1 * LDT + o_t
                                 , LDT, ref WORK, offset_work);
                _dlarfx.Run("L", 3, N - J1 + 1, U2, offset_u2, TAU2, ref T, J2+J1 * LDT + o_t
                                 , LDT, ref WORK, offset_work);
                _dlarfx.Run("R", J4, 3, U2, offset_u2, TAU2, ref T, 1+J2 * LDT + o_t
                                 , LDT, ref WORK, offset_work);
                // *
                T[J3+J1 * LDT + o_t] = ZERO;
                T[J3+J2 * LDT + o_t] = ZERO;
                ...
                if (WANTQ)
                {
                    ...
                    // *           Accumulate transformation in the matrix Q.
                    // *
                    _dlarfx.Run("R", N, 3, U1, offset_u1, TAU1, ref Q, 1+J1 * LDQ + o_q
                                     , LDQ, ref WORK, offset_work);
                    _dlarfx.Run("R", N, 3, U2, offset_u2, TAU2, ref Q, 1+J2 * LDQ + o_q
                                     , LDQ, ref WORK, offset_work);
                }
                // *
            LABEL40:;
                ...
                if (N2 == 2)
                {
                    ...
                    // *
                    _dlanv2.Run(ref T[J1+J1 * LDT + o_t], ref T[J1+J2 * LDT + o_t], ref T[J2+J1 * LDT + o_t], ref T[J2+J2 * LDT + o_t], ref WR1, ref WI1
                                     , ref WR2, ref WI2, ref CS, ref SN);
                    _drot.Run(N - J1 - 1, ref T, J1+(J1 + 2) * LDT + o_t, LDT, ref T, J2+(J1 + 2) * LDT + o_t, LDT, CS
                                   , SN);
                    _drot.Run(J1 - 1, ref T, 1+J1 * LDT + o_t, 1, ref T, 1+J2 * LDT + o_t, 1, CS
                                   , SN);
                    if (WANTQ)
                ...
                if (N1 == 2)
                {
                    ...
                    J3 = J1 + N2;
                    J4 = J3 + 1;
                    _dlanv2.Run(ref T[J3+J3 * LDT + o_t], ref T[J3+J4 * LDT + o_t], ref T[J4+J3 * LDT + o_t], ref T[J4+J4 * LDT + o_t], ref WR1, ref WI1
                                     , ref WR2, ref WI2, ref CS, ref SN);
                    if (J3 + 2 <= N)
                    {
                        _drot.Run(N - J3 - 1, ref T, J3+(J3 + 2) * LDT + o_t, LDT, ref T, J4+(J3 + 2) * LDT + o_t, LDT, CS
                                       , SN);
                    }
                    _drot.Run(J3 - 1, ref T, 1+J3 * LDT + o_t, 1, ref T, 1+J4 * LDT + o_t, 1, CS
                                   , SN);
                    if (WANTQ)
                    {
                        _drot.Run(N, ref Q, 1+J3 * LDQ + o_q, 1, ref Q, 1+J4 * LDQ + o_q, 1, CS
                                       , SN);
                    }
                }
                // *
