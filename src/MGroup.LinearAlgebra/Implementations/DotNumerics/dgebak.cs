...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGEBAK
    {
        ...
        public DGEBAK(LSAME lsame, DSCAL dscal, DSWAP dswap, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _dscal = dscal; _dswap = dswap; _xerbla = xerbla; 
                         ...
                         , int M, ref double[] V, int offset_v, int LDV, ref int INFO)
        {
            ...
            var LEFTV = false; var RIGHTV = false; var I = 0; var II = 0; var K = 0; double S = 0; 
            ...
            #region Array Index Correction
             ...
             var o_scale = -1 + offset_scale;  var o_v = -1 - LDV + offset_v; 
            ...
            RIGHTV = _lsame.Run(SIDE, "R");
            LEFTV = _lsame.Run(SIDE, "L");
            // *
            INFO = 0;
            ...
            if (N == 0) return;
            if (M == 0) return;
            if (_lsame.Run(JOB, "N")) return;
            // *
            if (ILO == IHI) goto LABEL30;
            // *
            // *     Backward balance
            // *
            if (_lsame.Run(JOB, "S") || _lsame.Run(JOB, "B"))
            {
                // *
                if (RIGHTV)
                {
                    ...
                    {
                        S = SCALE[I + o_scale];
                        _dscal.Run(M, S, ref V, I+1 * LDV + o_v, LDV);
                    }
                }
                // *
                if (LEFTV)
            ...
            // *
        LABEL30:;
            if (_lsame.Run(JOB, "P") || _lsame.Run(JOB, "B"))
            {
                if (RIGHTV)
                {
                    for (II = 1; II <= N; II++)
                    {
                        ...
                        if (K == I) goto LABEL40;
                        _dswap.Run(M, ref V, I+1 * LDV + o_v, LDV, ref V, K+1 * LDV + o_v, LDV);
                    LABEL40:;
                    }
                }
                // *
