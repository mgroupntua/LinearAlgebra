...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLASD3
    {
        ...
        public DLASD3(DLAMC3 dlamc3, DNRM2 dnrm2, DCOPY dcopy, DGEMM dgemm, DLACPY dlacpy, DLASCL dlascl, DLASD4 dlasd4, XERBLA xerbla)
        {
			...
			_dlamc3 = dlamc3; _dnrm2 = dnrm2; _dcopy = dcopy; _dgemm = dgemm; _dlacpy = dlacpy;
			_dlascl = dlascl; _dlasd4 = dlasd4; _xerbla = xerbla; 
                         ...
                         , ref double[] Z, int offset_z, ref int INFO)
        {
            ...
            var CTEMP = 0; var I = 0; var J = 0; var JC = 0; var KTEMP = 0; var M = 0; var N = 0; var NLP1 = 0; var NLP2 = 0; 
            var NRP1 = 0;double RHO = 0; double TEMP = 0; 
            ...
            var U_1 = 0; var U2_1 = 0; var U_K = 0; var VT_K = 0; var VT_I = 0; var U_I = 0; var Q_I = 0; var Q_1 = 0; 
            var Q_KTEMP = 0;
            ...
            #region Array Index Correction
             ...
             var o_d = -1 + offset_d;  var o_q = -1 - LDQ + offset_q;  var o_dsigma = -1 + offset_dsigma; 
             var o_u = -1 - LDU + offset_u; var o_u2 = -1 - LDU2 + offset_u2;  var o_vt = -1 - LDVT + offset_vt; 
             var o_vt2 = -1 - LDVT2 + offset_vt2; var o_idxc = -1 + offset_idxc;  var o_ctot = -1 + offset_ctot; 
             var o_z = -1 + offset_z;
            ...
            if (K == 1)
            {
				...
				_dcopy.Run(M, VT2, 1+1 * LDVT2 + o_vt2, LDVT2, ref VT, 1+1 * LDVT + o_vt, LDVT);
                if (Z[1 + o_z] > ZERO)
                {
					_dcopy.Run(N, U2, 1+1 * LDU2 + o_u2, 1, ref U, 1+1 * LDU + o_u, 1);
                }
                else
                {
                    U_1 = 1 * LDU + o_u;
            ...
            for (I = 1; I <= K; I++)
            {
                DSIGMA[I + o_dsigma] = _dlamc3.Run(DSIGMA[I + o_dsigma], DSIGMA[I + o_dsigma]) - DSIGMA[I + o_dsigma];
            }
			// *
			// *     Keep a copy of Z.
			// *
			_dcopy.Run(K, Z, offset_z, 1, ref Q, offset_q, 1);
            // *
            // *     Normalize Z.
            // *
            RHO = _dnrm2.Run(K, Z, offset_z, 1);
			_dlascl.Run("G", 0, 0, RHO, ONE, K
            ...
            for (J = 1; J <= K; J++)
            {
				_dlasd4.Run(K, J, DSIGMA, offset_dsigma, Z, offset_z, ref U, 1+J * LDU + o_u, RHO
            ...
            for (I = 1; I <= K; I++)
            {
                ...
                TEMP = _dnrm2.Run(K, U, 1+I * LDU + o_u, 1);
            ...
            else
            {
                ...
                {
					_dlacpy.Run("F", NL, K, U2, offset_u2, LDU2, ref U, offset_u
                                     , LDU);
                }
            }
			_dcopy.Run(K, Q, 1+1 * LDQ + o_q, LDQ, ref U, NLP1+1 * LDU + o_u, LDU);
            KTEMP = 2 + CTOT[1 + o_ctot];
            CTEMP = CTOT[2 + o_ctot] + CTOT[3 + o_ctot];
			_dgemm.Run("N", "N", NR, K, CTEMP, ONE
        ...
        LABEL100:;
            ...
            for (I = 1; I <= K; I++)
            {
                TEMP = _dnrm2.Run(K, VT, 1+I * LDVT + o_vt, 1);
			...
			_dgemm.Run("N", "N", K, NRP1, CTEMP, ONE
