...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGBTRS
    {
        ...
        public DGBTRS(LSAME lsame, DGEMV dgemv, DGER dger, DSWAP dswap, DTBSV dtbsv, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _dgemv = dgemv; _dger = dger; _dswap = dswap; _dtbsv = dtbsv; 
            _xerbla = xerbla;
        ...
        public DGBTRS()
        {
            ...
            var dtbsv = new DTBSV(lsame, xerbla);
                         ...
                         , int LDAB, int[] IPIV, int offset_ipiv, ref double[] B, int offset_b, int LDB, ref int INFO)
        {
            ...
            var LNOTI = false; var NOTRAN = false; var I = 0; var J = 0; var KD = 0; var L = 0; var LM = 0; 
            ...
            #region Array Index Correction
             ...
             var o_ab = -1 - LDAB + offset_ab;  var o_ipiv = -1 + offset_ipiv;  var o_b = -1 - LDB + offset_b; 
            ...
            if (NOTRAN)
            {
                ...
                if (LNOTI)
                {
                    for (J = 1; J <= N - 1; J++)
                    {
                        LM = Math.Min(KL, N - J);
                        L = IPIV[J + o_ipiv];
                        if (L != J) _dswap.Run(NRHS, ref B, L+1 * LDB + o_b, LDB, ref B, J+1 * LDB + o_b, LDB);
                        _dger.Run(LM, NRHS,  - ONE, AB, KD + 1+J * LDAB + o_ab, 1, B, J+1 * LDB + o_b
                ...
                for (I = 1; I <= NRHS; I++)
                {
                    ...
                    // *
                    _dtbsv.Run("Upper", "No transpose", "Non-unit", N, KL + KU, AB, offset_ab
            ...
            else
            {
                ...
                for (I = 1; I <= NRHS; I++)
                {
                    ...
                    // *           Solve U'*X = B, overwriting B with X.
                    // *
                    _dtbsv.Run("Upper", "Transpose", "Non-unit", N, KL + KU, AB, offset_ab
                                    , LDAB, ref B, 1+I * LDB + o_b, 1);
                }
                // *
                // *        Solve L'*X = B, overwriting B with X.
                ...
                if (LNOTI)
                {
                    ...
                    {
                        LM = Math.Min(KL, N - J);
                        _dgemv.Run("Transpose", LM, NRHS,  - ONE, B, J + 1+1 * LDB + o_b, LDB
