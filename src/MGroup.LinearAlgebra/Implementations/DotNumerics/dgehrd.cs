...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGEHRD
    {
        ...
        public DGEHRD(DAXPY daxpy, DGEHD2 dgehd2, DGEMM dgemm, DLAHR2 dlahr2, DLARFB dlarfb, DTRMM dtrmm, XERBLA xerbla, ILAENV ilaenv)
        {
            ...
            _daxpy = daxpy; _dgehd2 = dgehd2; _dgemm = dgemm; _dlahr2 = dlahr2; _dlarfb = dlarfb; 
            _dtrmm = dtrmm;_xerbla = xerbla; _ilaenv = ilaenv; 
                         ...
                         , ref double[] WORK, int offset_work, int LWORK, ref int INFO)
        {
            ...
            var LQUERY = false; var I = 0; var IB = 0; var IINFO = 0; var IWS = 0; var J = 0; var LDWORK = 0; var LWKOPT = 0; 
            var NB = 0;var NBMIN = 0; var NH = 0; var NX = 0; double EI = 0; var offset_t = 0;
            ...
            INFO = 0;
            NB = Math.Min(NBMAX, _ilaenv.Run(1, "DGEHRD", " ", N, ILO, IHI,  - 1));
            LWKOPT = N * NB;
            ...
            if (NB > 1 && NB < NH)
            {
                ...
                // *
                NX = Math.Max(NB, _ilaenv.Run(3, "DGEHRD", " ", N, ILO, IHI,  - 1));
                if (NX < NH)
                {
                    // *
                    ...
                    if (LWORK < IWS)
                    {
                        ...
                        // *              unblocked code
                        // *
                        NBMIN = Math.Max(2, _ilaenv.Run(2, "DGEHRD", " ", N, ILO, IHI,  - 1));
                        if (LWORK >= N * NBMIN)
                        {
                            NB = LWORK / N;
                        }
            ...
            else
            {
                ...
                for (I = ILO; NB >= 0 ? I <= IHI - 1 - NX : I >= IHI - 1 - NX; I += NB)
                {
                    ...
                    // *
                    _dlahr2.Run(IHI, I, IB, ref A, 1+I * LDA + o_a, LDA, ref TAU, I + o_tau
                    ...
                    _dgemm.Run("No transpose", "Transpose", IHI, IHI - I - IB + 1, IB,  - ONE
                    ...
                    // *           right
                    // *
                    _dtrmm.Run("Right", "Lower", "Transpose", "Unit", I, IB - 1
                                    , ONE, A, I + 1+I * LDA + o_a, LDA, ref WORK, offset_work, LDWORK);
                    for (J = 0; J <= IB - 2; J++)
                    {
                        _daxpy.Run(I,  - ONE, WORK, LDWORK * J + 1 + o_work, 1, ref A, 1+(I + J + 1) * LDA + o_a, 1);
                    }
                    // *
                    // *           Apply the block reflector H to A(i+1:ihi,i+ib:n) from the
                    // *           left
                    // *
                    _dlarfb.Run("Left", "Transpose", "Forward", "Columnwise", IHI - I, N - I - IB + 1
            ...
            // *
            _dgehd2.Run(N, I, IHI, ref A, offset_a, LDA, ref TAU, offset_tau
