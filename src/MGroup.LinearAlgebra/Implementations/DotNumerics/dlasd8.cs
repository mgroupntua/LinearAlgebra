...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLASD8
    {
        ...
        public DLASD8(DCOPY dcopy, DLASCL dlascl, DLASD4 dlasd4, DLASET dlaset, XERBLA xerbla, DDOT ddot, DLAMC3 dlamc3, DNRM2 dnrm2)
        {
			...
			_dcopy = dcopy; _dlascl = dlascl; _dlasd4 = dlasd4; _dlaset = dlaset; _xerbla = xerbla;
			_ddot = ddot; _dlamc3 = dlamc3; _dnrm2 = dnrm2; 
                         ...
                         , ref double[] DIFL, int offset_difl, ref double[] DIFR, int offset_difr, int LDDIFR, ref double[] DSIGMA, int offset_dsigma, ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var I = 0; var IWK1 = 0; var IWK2 = 0; var IWK2I = 0; var IWK3 = 0; var IWK3I = 0; var J = 0; double DIFLJ = 0; 
            ...
            #region Array Index Correction
             ...
             var o_d = -1 + offset_d;  var o_z = -1 + offset_z;  var o_vf = -1 + offset_vf;  var o_vl = -1 + offset_vl; 
             var o_difl = -1 + offset_difl; var o_difr = -1 - LDDIFR + offset_difr;  var o_dsigma = -1 + offset_dsigma; 
			...
			// *
			_dlaset.Run("A", K, 1, ONE, ONE, ref WORK, IWK3 + o_work
                             , K);
            // *
            ...
            for (J = 1; J <= K; J++)
            {
				_dlasd4.Run(K, J, DSIGMA, offset_dsigma, Z, offset_z, ref WORK, IWK1 + o_work, RHO
            ...
            for (J = 1; J <= K; J++)
            {
                ...
                for (I = 1; I <= J - 1; I++)
                {
                    WORK[I + o_work] = Z[I + o_z] / (_dlamc3.Run(DSIGMA[I + o_dsigma], DSIGJ) - DIFLJ) / (DSIGMA[I + o_dsigma] + DJ);
                }
                for (I = J + 1; I <= K; I++)
                {
                    WORK[I + o_work] = Z[I + o_z] / (_dlamc3.Run(DSIGMA[I + o_dsigma], DSIGJP) + DIFRJ) / (DSIGMA[I + o_dsigma] + DJ);
                }
                TEMP = _dnrm2.Run(K, WORK, offset_work, 1);
                WORK[IWK2I + J + o_work] = _ddot.Run(K, WORK, offset_work, 1, VF, offset_vf, 1) / TEMP;
                WORK[IWK3I + J + o_work] = _ddot.Run(K, WORK, offset_work, 1, VL, offset_vl, 1) / TEMP;
                if (ICOMPQ == 1)
                {
			...
			// *
			_dcopy.Run(K, WORK, IWK2 + o_work, 1, ref VF, offset_vf, 1);
			_dcopy.Run(K, WORK, IWK3 + o_work, 1, ref VL, offset_vl, 1);
            // *
            return;
            // *
            // *     End of DLASD8
