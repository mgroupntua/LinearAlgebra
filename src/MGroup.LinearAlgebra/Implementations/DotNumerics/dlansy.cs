...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLANSY
    {
        ...
        double dlansy = 0;
            ...
            var I = 0; var J = 0; double ABSA = 0; double SCALE = 0; double SUM = 0; double VALUE = 0; 
                    ...
                    else
                    {
                        if (_lsame.Run(NORM, "F") || _lsame.Run(NORM, "E"))
                        {
                            ...
                            if (_lsame.Run(UPLO, "U"))
                            {
                                for (J = 2; J <= N; J++)
                                {
                                    _dlassq.Run(J - 1, A, 1+J * LDA + o_a, 1, ref SCALE, ref SUM);
                                }
                            }
                            else
                            {
                                for (J = 1; J <= N - 1; J++)
                                {
                                    _dlassq.Run(N - J, A, J + 1+J * LDA + o_a, 1, ref SCALE, ref SUM);
                                }
                            }
                            SUM *= 2;
                            _dlassq.Run(N, A, offset_a, LDA + 1, ref SCALE, ref SUM);
