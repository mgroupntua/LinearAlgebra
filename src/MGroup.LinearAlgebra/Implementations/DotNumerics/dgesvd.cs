...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGESVD
    {
                      ...
                      , DORGQR dorgqr, DORMBR dormbr, XERBLA xerbla, LSAME lsame, ILAENV ilaenv, DLAMCH dlamch, DLANGE dlange)
        {
            ...
            _dbdsqr = dbdsqr; _dgebrd = dgebrd; _dgelqf = dgelqf; _dgemm = dgemm; _dgeqrf = dgeqrf; 
            _dlacpy = dlacpy;_dlascl = dlascl; _dlaset = dlaset; _dorgbr = dorgbr; _dorglq = dorglq; 
            _dorgqr = dorgqr;_dormbr = dormbr; _xerbla = xerbla; _lsame = lsame; _ilaenv = ilaenv; 
            _dlamch = dlamch;_dlange = dlange; 
                         ...
                         , int LWORK, ref int INFO)
        {
            ...
            var LQUERY = false; var WNTUA = false; var WNTUAS = false; var WNTUN = false; var WNTUO = false; 
            var WNTUS = false;var WNTVA = false; var WNTVAS = false; var WNTVN = false; var WNTVO = false; 
            var WNTVS = false;var BDSPAC = 0; var BLK = 0; var CHUNK = 0; var I = 0; var IE = 0; var IERR = 0; var IR = 0; 
            var ISCL = 0;var ITAU = 0; var ITAUP = 0; var ITAUQ = 0; var IU = 0; var IWORK = 0; var LDWRKR = 0; var LDWRKU = 0; 
            var MAXWRK = 0;var MINMN = 0; var MINWRK = 0; var MNTHR = 0; var NCU = 0; var NCVT = 0; var NRU = 0; var NRVT = 0; 
            var WRKBL = 0;double ANRM = 0; double BIGNUM = 0; double EPS = 0; double SMLNUM = 0; 
            var offset_dum = 0;
            ...
            #region Array Index Correction
             ...
             var o_vt = -1 - LDVT + offset_vt; var o_work = -1 + offset_work; 
            ...
            INFO = 0;
            MINMN = Math.Min(M, N);
            WNTUA = _lsame.Run(JOBU, "A");
            WNTUS = _lsame.Run(JOBU, "S");
            WNTUAS = WNTUA || WNTUS;
            WNTUO = _lsame.Run(JOBU, "O");
            WNTUN = _lsame.Run(JOBU, "N");
            WNTVA = _lsame.Run(JOBVT, "A");
            WNTVS = _lsame.Run(JOBVT, "S");
            WNTVAS = WNTVA || WNTVS;
            WNTVO = _lsame.Run(JOBVT, "O");
            WNTVN = _lsame.Run(JOBVT, "N");
            LQUERY = LWORK ==  - 1;
            // *
            ...
            if (INFO == 0)
            {
                ...
                if (M >= N && MINMN > 0)
                {
                    ...
                    // *
                    MNTHR = _ilaenv.Run(6, "DGESVD", JOBU + JOBVT, M, N, 0, 0);
                    BDSPAC = 5 * N;
                    if (M >= MNTHR)
                    {
                        if (WNTUN)
                        {
                            ...
                            // *
                            MAXWRK = N + N * _ilaenv.Run(1, "DGEQRF", " ", M, N,  - 1,  - 1);
                            MAXWRK = Math.Max(MAXWRK, 3 * N + 2 * N * _ilaenv.Run(1, "DGEBRD", " ", N, N,  - 1,  - 1));
                            if (WNTVO || WNTVAS) MAXWRK = Math.Max(MAXWRK, 3 * N + (N - 1) * _ilaenv.Run(1, "DORGBR", "P", N, N, N,  - 1));
                        ...
                        else
                        {
                            if (WNTUO && WNTVN)
                            {
                                ...
                                WRKBL = Math.Max(WRKBL, 3 * N + N * _ilaenv.Run(1, "DORGBR", "Q", N, N, N,  - 1));
                            ...
                            else
                            {
                                if (WNTUO && WNTVAS)
                                {
                                    ...
                                    WRKBL = Math.Max(WRKBL, 3 * N + 2 * N * _ilaenv.Run(1, "DGEBRD", " ", N, N,  - 1,  - 1));
                                    WRKBL = Math.Max(WRKBL, 3 * N + N * _ilaenv.Run(1, "DORGBR", "Q", N, N, N,  - 1));
                                    WRKBL = Math.Max(WRKBL, 3 * N + (N - 1) * _ilaenv.Run(1, "DORGBR", "P", N, N, N,  - 1));
                                    WRKBL = Math.Max(WRKBL, BDSPAC);
                                    MAXWRK = Math.Max(N * N + WRKBL, N * N + M * N + N);
                                    MINWRK = Math.Max(3 * N + M, BDSPAC);
                                }
                    ...
                    else
                    {
                        ...
                        // *
                        MAXWRK = 3 * N + (M + N) * _ilaenv.Run(1, "DGEBRD", " ", M, N,  - 1,  - 1);
                        if (WNTUS || WNTUO) MAXWRK = Math.Max(MAXWRK, 3 * N + N * _ilaenv.Run(1, "DORGBR", "Q", M, N, N,  - 1));
                        if (WNTUA) MAXWRK = Math.Max(MAXWRK, 3 * N + M * _ilaenv.Run(1, "DORGBR", "Q", M, M, N,  - 1));
                        if (!WNTVN) MAXWRK = Math.Max(MAXWRK, 3 * N + (N - 1) * _ilaenv.Run(1, "DORGBR", "P", N, N, N,  - 1));
                        MAXWRK = Math.Max(MAXWRK, BDSPAC);
                        MINWRK = Math.Max(3 * N + M, BDSPAC);
                    }
                }
                    ...
                    if (MINMN > 0)
                    {
                        ...
                        if (N >= MNTHR)
                        {
                            if (WNTVN)
                            {
                                ...
                                MAXWRK = Math.Max(MAXWRK, 3 * M + 2 * M * _ilaenv.Run(1, "DGEBRD", " ", M, M,  - 1,  - 1));
                                if (WNTUO || WNTUAS) MAXWRK = Math.Max(MAXWRK, 3 * M + M * _ilaenv.Run(1, "DORGBR", "Q", M, M, M,  - 1));
                                MAXWRK = Math.Max(MAXWRK, BDSPAC);
                            ...
                            else
                            {
                                if (WNTVO && WNTUN)
                                {
                                    ...
                                    WRKBL = Math.Max(WRKBL, 3 * M + (M - 1) * _ilaenv.Run(1, "DORGBR", "P", M, M, M,  - 1));
                                    WRKBL = Math.Max(WRKBL, BDSPAC);
                                ...
                                else
                                {
                                    if (WNTVO && WNTUAS)
                                    {
                                        ...
                                        WRKBL = Math.Max(WRKBL, 3 * M + 2 * M * _ilaenv.Run(1, "DGEBRD", " ", M, M,  - 1,  - 1));
                                        WRKBL = Math.Max(WRKBL, 3 * M + (M - 1) * _ilaenv.Run(1, "DORGBR", "P", M, M, M,  - 1));
                                        WRKBL = Math.Max(WRKBL, 3 * M + M * _ilaenv.Run(1, "DORGBR", "Q", M, M, M,  - 1));
                                        WRKBL = Math.Max(WRKBL, BDSPAC);
                                        MAXWRK = Math.Max(M * M + WRKBL, M * M + M * N + M);
                                        MINWRK = Math.Max(3 * M + N, BDSPAC);
                                    }
                        ...
                        else
                        {
                            ...
                            // *
                            MAXWRK = 3 * M + (M + N) * _ilaenv.Run(1, "DGEBRD", " ", M, N,  - 1,  - 1);
                            if (WNTVS || WNTVO) MAXWRK = Math.Max(MAXWRK, 3 * M + M * _ilaenv.Run(1, "DORGBR", "P", M, N, M,  - 1));
                            if (WNTVA) MAXWRK = Math.Max(MAXWRK, 3 * M + N * _ilaenv.Run(1, "DORGBR", "P", N, N, M,  - 1));
                            if (!WNTUN) MAXWRK = Math.Max(MAXWRK, 3 * M + (M - 1) * _ilaenv.Run(1, "DORGBR", "Q", M, M, M,  - 1));
                            MAXWRK = Math.Max(MAXWRK, BDSPAC);
                            MINWRK = Math.Max(3 * M + N, BDSPAC);
                        }
                    }
            ...
            if (M >= N)
            {
                ...
                if (M >= MNTHR)
                {
                    ...
                    if (WNTUN)
                    {
                        ...
                        // *
                        _dgeqrf.Run(M, N, ref A, offset_a, LDA, ref WORK, ITAU + o_work, ref WORK, IWORK + o_work
                        ...
                        if (WNTVO || WNTVAS)
                        {
                            ...
                            // *
                            _dorgbr.Run("P", N, N, N, ref A, offset_a, LDA
                        ...
                        // *
                        _dbdsqr.Run("U", N, NCVT, 0, 0, ref S, offset_s
                        ...
                        if (WNTVAS)
                        {
                            _dlacpy.Run("F", N, N, A, offset_a, LDA, ref VT, offset_vt
                                             , LDVT);
                        }
                        // *
                    }
                    else
                    {
                        if (WNTUO && WNTVN)
                        {
                            ...
                            if (LWORK >= N * N + Math.Max(4 * N, BDSPAC))
                            {
                                ...
                                // *
                                _dorgbr.Run("Q", N, N, N, ref WORK, IR + o_work, LDWRKR
                                ...
                                // *
                                _dbdsqr.Run("U", N, 0, N, 0, ref S, offset_s
                                ...
                                for (I = 1; LDWRKU >= 0 ? I <= M : I >= M; I += LDWRKU)
                                {
                                    ...
                                    _dgemm.Run("N", "N", CHUNK, N, N, ONE
                                                    ...
                                                    , LDWRKU);
                                    _dlacpy.Run("F", CHUNK, N, WORK, IU + o_work, LDWRKU, ref A, I+1 * LDA + o_a
                                                     , LDA);
                                }
                                // *
                            }
                            else
                            {
                                ...
                                // *                 (Workspace: need BDSPAC)
                                // *
                                _dbdsqr.Run("U", N, 0, M, 0, ref S, offset_s
                        ...
                        else
                        {
                            if (WNTUO && WNTVAS)
                            {
                                ...
                                if (LWORK >= N * N + Math.Max(4 * N, BDSPAC))
                                {
                                    ...
                                    // *
                                    _dlacpy.Run("U", N, N, A, offset_a, LDA, ref VT, offset_vt
                                                     , LDVT);
                                    if (N > 1)
                                    {
                                        _dlaset.Run("L", N - 1, N - 1, ZERO, ZERO, ref VT, 2+1 * LDVT + o_vt
                                                         , LDVT);
                                    }
                                    // *
                                    // *                 Generate Q in A
                                    ...
                                    // *                 (Workspace: need N*N+4*N, prefer N*N+3*N+2*N*NB)
                                    // *
                                    _dgebrd.Run(N, N, ref VT, offset_vt, LDVT, ref S, offset_s, ref WORK, IE + o_work
                                                     , ref WORK, ITAUQ + o_work, ref WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                    _dlacpy.Run("L", N, N, VT, offset_vt, LDVT, ref WORK, IR + o_work
                                                     , LDWRKR);
                                    // *
                                    ...
                                    // *
                                    _dorgbr.Run("P", N, N, N, ref VT, offset_vt, LDVT
                                                     , WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                    IWORK = IE + N;
                                    // *
                                    // *                 Perform bidiagonal QR iteration, computing left
                                    ...
                                    // *                 (Workspace: need N*N+BDSPAC)
                                    // *
                                    _dbdsqr.Run("U", N, N, N, 0, ref S, offset_s
                                ...
                                else
                                {
                                    ...
                                    // *                 (Workspace: need BDSPAC)
                                    // *
                                    _dbdsqr.Run("U", N, N, M, 0, ref S, offset_s
                                ...
                                if (WNTUS)
                                {
                                    ...
                                    if (WNTVN)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            // *                    (Workspace: need 2*N, prefer N+N*NB)
                                            // *
                                            _dorgqr.Run(M, N, N, ref U, offset_u, LDU, WORK, ITAU + o_work
                                                             , ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                            IE = ITAU;
                                            ITAUQ = IE + N;
                                            ITAUP = ITAUQ + N;
                                    ...
                                    else
                                    {
                                        if (WNTVO)
                                        {
                                            ...
                                            if (LWORK >= 2 * N * N + Math.Max(4 * N, BDSPAC))
                                            {
                                                ...
                                                // *
                                                _dlacpy.Run("U", N, N, A, offset_a, LDA, ref WORK, IU + o_work
                                                                 , LDWRKU);
                                                _dlaset.Run("L", N - 1, N - 1, ZERO, ZERO, ref WORK, IU + 1 + o_work
                                                                 , LDWRKU);
                                                // *
                                                // *                    Generate Q in A
                                                // *                    (Workspace: need 2*N*N+2*N, prefer 2*N*N+N+N*NB)
                                                ...
                                                // *
                                                _dgebrd.Run(N, N, ref WORK, IU + o_work, LDWRKU, ref S, offset_s, ref WORK, IE + o_work
                                                                 , ref WORK, ITAUQ + o_work, ref WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                                _dlacpy.Run("U", N, N, WORK, IU + o_work, LDWRKU, ref WORK, IR + o_work
                                                                 , LDWRKR);
                                                // *
                                                // *                    Generate left bidiagonalizing vectors in WORK(IU)
                                                // *                    (Workspace: need 2*N*N+4*N, prefer 2*N*N+3*N+N*NB)
                                                // *
                                                _dorgbr.Run("Q", N, N, N, ref WORK, IU + o_work, LDWRKU
                                                                 , WORK, ITAUQ + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                                // *
                                                ...
                                                // *
                                                _dorgbr.Run("P", N, N, N, ref WORK, IR + o_work, LDWRKR
                                                                 , WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                                IWORK = IE + N;
                                                // *
                                                // *                    Perform bidiagonal QR iteration, computing left
                                                ...
                                                // *                    (Workspace: need N*N)
                                                // *
                                                _dlacpy.Run("F", N, N, WORK, IR + o_work, LDWRKR, ref A, offset_a
                                                                 , LDA);
                                                // *
                                            }
                                            else
                                        ...
                                        else
                                        {
                                            if (WNTVAS)
                                            {
                                                ...
                                                if (LWORK >= N * N + Math.Max(4 * N, BDSPAC))
                                                {
                                                    ...
                                                    _dgebrd.Run(N, N, ref WORK, IU + o_work, LDWRKU, ref S, offset_s, ref WORK, IE + o_work
                                                                     , ref WORK, ITAUQ + o_work, ref WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                                    _dlacpy.Run("U", N, N, WORK, IU + o_work, LDWRKU, ref VT, offset_vt
                                                                     , LDVT);
                                                    // *
                                                    // *                    Generate left bidiagonalizing vectors in WORK(IU)
                                                    // *                    (Workspace: need N*N+4*N, prefer N*N+3*N+N*NB)
                ...
                else
                {
                    ...
                    if (WNTUAS)
                    {
                        ...
                        if (WNTUS) NCU = N;
                        if (WNTUA) NCU = M;
                        _dorgbr.Run("Q", M, NCU, N, ref U, offset_u, LDU
                                         , WORK, ITAUQ + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                    }
                    if (WNTVAS)
                    {
                    ...
                    if (!WNTUO && !WNTVO)
                    {
                        ...
                        // *              (Workspace: need BDSPAC)
                        // *
                        _dbdsqr.Run("U", N, NCVT, NRU, 0, ref S, offset_s
                                         , ref WORK, IE + o_work, ref VT, offset_vt, LDVT, ref U, offset_u, LDU, ref DUM, offset_dum
                                         , 1, ref WORK, IWORK + o_work, ref INFO);
                    }
                    else
            ...
            else
            {
                ...
                if (N >= MNTHR)
                {
                    ...
                    if (WNTVN)
                    {
                        ...
                        // *
                        _dgelqf.Run(M, N, ref A, offset_a, LDA, ref WORK, ITAU + o_work, ref WORK, IWORK + o_work
                                         , LWORK - IWORK + 1, ref IERR);
                        // *
                        // *              Zero out above L
                        // *
                        ...
                        if (WNTUO || WNTUAS)
                        {
                            ...
                            // *
                            _dorgbr.Run("Q", M, M, M, ref A, offset_a, LDA
                                             , WORK, ITAUQ + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                        }
                        IWORK = IE + M;
                        NRU = 0;
                        ...
                        // *              (Workspace: need BDSPAC)
                        // *
                        _dbdsqr.Run("U", M, 0, NRU, 0, ref S, offset_s
                                         , ref WORK, IE + o_work, ref DUM, offset_dum, 1, ref A, offset_a, LDA, ref DUM, offset_dum
                                         , 1, ref WORK, IWORK + o_work, ref INFO);
                        // *
                        ...
                        if (WNTUAS)
                        {
                            _dlacpy.Run("F", M, M, A, offset_a, LDA, ref U, offset_u
                                             , LDU);
                        }
                        // *
                    }
                    else
                    {
                        if (WNTVO && WNTUN)
                        {
                            ...
                            if (LWORK >= M * M + Math.Max(4 * M, BDSPAC))
                            {
                                ...
                                // *
                                _dlacpy.Run("L", M, M, A, offset_a, LDA, ref WORK, IR + o_work
                                                 , LDWRKR);
                                _dlaset.Run("U", M - 1, M - 1, ZERO, ZERO, ref WORK, IR + LDWRKR + o_work
                                                 , LDWRKR);
                                // *
                                // *                 Generate Q in A
                                // *                 (Workspace: need M*M+2*M, prefer M*M+M+M*NB)
                                ...
                                // *
                                _dgebrd.Run(M, M, ref WORK, IR + o_work, LDWRKR, ref S, offset_s, ref WORK, IE + o_work
                                                 , ref WORK, ITAUQ + o_work, ref WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                // *
                                // *                 Generate right vectors bidiagonalizing L
                                // *                 (Workspace: need M*M+4*M-1, prefer M*M+3*M+(M-1)*NB)
                                // *
                                _dorgbr.Run("P", M, M, M, ref WORK, IR + o_work, LDWRKR
                                                 , WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                IWORK = IE + M;
                                // *
                                // *                 Perform bidiagonal QR iteration, computing right
                                ...
                                // *
                                _dbdsqr.Run("U", M, M, 0, 0, ref S, offset_s
                                ...
                                for (I = 1; CHUNK >= 0 ? I <= N : I >= N; I += CHUNK)
                                {
                                    ...
                                    _dgemm.Run("N", "N", M, BLK, M, ONE
                                                    ...
                                                    , LDWRKU);
                                    _dlacpy.Run("F", M, BLK, WORK, IU + o_work, LDWRKU, ref A, 1+I * LDA + o_a
                                                     , LDA);
                                }
                                // *
                            }
                            else
                            {
                                ...
                                // *                 (Workspace: need BDSPAC)
                                // *
                                _dbdsqr.Run("L", M, N, 0, 0, ref S, offset_s
                                                 , ref WORK, IE + o_work, ref A, offset_a, LDA, ref DUM, offset_dum, 1, ref DUM, offset_dum
                                                 , 1, ref WORK, IWORK + o_work, ref INFO);
                                // *
                            }
                        ...
                        else
                        {
                            if (WNTVO && WNTUAS)
                            {
                                ...
                                if (LWORK >= M * M + Math.Max(4 * M, BDSPAC))
                                {
                                    ...
                                    // *
                                    _dlacpy.Run("L", M, M, A, offset_a, LDA, ref U, offset_u
                                                     , LDU);
                                    _dlaset.Run("U", M - 1, M - 1, ZERO, ZERO, ref U, 1+2 * LDU + o_u
                                                     , LDU);
                                    // *
                                    // *                 Generate Q in A
                                    // *                 (Workspace: need M*M+2*M, prefer M*M+M+M*NB)
                                    ...
                                    // *                 (Workspace: need M*M+4*M, prefer M*M+3*M+2*M*NB)
                                    // *
                                    _dgebrd.Run(M, M, ref U, offset_u, LDU, ref S, offset_s, ref WORK, IE + o_work
                                                     , ref WORK, ITAUQ + o_work, ref WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                    _dlacpy.Run("U", M, M, U, offset_u, LDU, ref WORK, IR + o_work
                                                     , LDWRKR);
                                    // *
                                    ...
                                    // *
                                    _dorgbr.Run("Q", M, M, M, ref U, offset_u, LDU
                                                     , WORK, ITAUQ + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                    IWORK = IE + M;
                                    // *
                                    // *                 Perform bidiagonal QR iteration, computing left
                                    ...
                                    // *                 (Workspace: need M*M+BDSPAC)
                                    // *
                                    _dbdsqr.Run("U", M, M, M, 0, ref S, offset_s
                                ...
                                else
                                {
                                    ...
                                    // *                 (Workspace: need BDSPAC)
                                    // *
                                    _dbdsqr.Run("U", M, N, M, 0, ref S, offset_s
                                                     , ref WORK, IE + o_work, ref A, offset_a, LDA, ref U, offset_u, LDU, ref DUM, offset_dum
                                                     , 1, ref WORK, IWORK + o_work, ref INFO);
                                    // *
                                }
                                ...
                                if (WNTVS)
                                {
                                    ...
                                    if (WNTUN)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            // *                    (Workspace: need 2*M, prefer M+M*NB)
                                            // *
                                            _dorglq.Run(M, N, M, ref VT, offset_vt, LDVT, WORK, ITAU + o_work
                                                             , ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                            IE = ITAU;
                                            ITAUQ = IE + M;
                                            ITAUP = ITAUQ + M;
                                            ...
                                            // *                    (Workspace: need BDSPAC)
                                            // *
                                            _dbdsqr.Run("U", M, N, 0, 0, ref S, offset_s
                                    ...
                                    else
                                    {
                                        if (WNTUO)
                                        {
                                            ...
                                            if (LWORK >= 2 * M * M + Math.Max(4 * M, BDSPAC))
                                            {
                                                ...
                                                // *
                                                _dlacpy.Run("L", M, M, A, offset_a, LDA, ref WORK, IU + o_work
                                                                 , LDWRKU);
                                                _dlaset.Run("U", M - 1, M - 1, ZERO, ZERO, ref WORK, IU + LDWRKU + o_work
                                                                 , LDWRKU);
                                                // *
                                                // *                    Generate Q in A
                                                // *                    (Workspace: need 2*M*M+2*M, prefer 2*M*M+M+M*NB)
                                                ...
                                                // *
                                                _dgebrd.Run(M, M, ref WORK, IU + o_work, LDWRKU, ref S, offset_s, ref WORK, IE + o_work
                                                                 , ref WORK, ITAUQ + o_work, ref WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                                _dlacpy.Run("L", M, M, WORK, IU + o_work, LDWRKU, ref WORK, IR + o_work
                                                                 , LDWRKR);
                                                // *
                                                ...
                                                // *
                                                _dorgbr.Run("P", M, M, M, ref WORK, IU + o_work, LDWRKU
                                                                 , WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                                // *
                                                // *                    Generate left bidiagonalizing vectors in WORK(IR)
                                                // *                    (Workspace: need 2*M*M+4*M, prefer 2*M*M+3*M+M*NB)
                                                // *
                                                _dorgbr.Run("Q", M, M, M, ref WORK, IR + o_work, LDWRKR
                                                                 , WORK, ITAUQ + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                                IWORK = IE + M;
                                                // *
                                                // *                    Perform bidiagonal QR iteration, computing left
                                                ...
                                                // *                    (Workspace: need M*M)
                                                // *
                                                _dlacpy.Run("F", M, M, WORK, IR + o_work, LDWRKR, ref A, offset_a
                                                                 , LDA);
                                                // *
                                            }
                                            else
                                        ...
                                        else
                                        {
                                            if (WNTUAS)
                                            {
                                                ...
                                                if (LWORK >= M * M + Math.Max(4 * M, BDSPAC))
                                                {
                                                    ...
                                                    _dgebrd.Run(M, M, ref WORK, IU + o_work, LDWRKU, ref S, offset_s, ref WORK, IE + o_work
                                                                     , ref WORK, ITAUQ + o_work, ref WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                                                    _dlacpy.Run("L", M, M, WORK, IU + o_work, LDWRKU, ref U, offset_u
                                                                     , LDU);
                                                    // *
                                                    // *                    Generate right bidiagonalizing vectors in WORK(IU)
                ...
                else
                {
                    ...
                    if (WNTUAS)
                    {
                        ...
                        _dlacpy.Run("L", M, M, A, offset_a, LDA, ref U, offset_u
                                         , LDU);
                        _dorgbr.Run("Q", M, M, N, ref U, offset_u, LDU
                                         , WORK, ITAUQ + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                    }
                    if (WNTVAS)
                    {
                        ...
                        if (WNTVA) NRVT = N;
                        if (WNTVS) NRVT = M;
                        _dorgbr.Run("P", NRVT, N, M, ref VT, offset_vt, LDVT
                                         , WORK, ITAUP + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                    }
                    if (WNTUO)
                    {
                        ...
                        // *              (Workspace: need 4*M-1, prefer 3*M+(M-1)*NB)
                        // *
                        _dorgbr.Run("Q", M, M, N, ref A, offset_a, LDA
                                         , WORK, ITAUQ + o_work, ref WORK, IWORK + o_work, LWORK - IWORK + 1, ref IERR);
                    }
                    if (WNTVO)
                    {
                    ...
                    if (!WNTUO && !WNTVO)
                    {
                        ...
                        // *              (Workspace: need BDSPAC)
                        // *
                        _dbdsqr.Run("L", M, NCVT, NRU, 0, ref S, offset_s
                                         , ref WORK, IE + o_work, ref VT, offset_vt, LDVT, ref U, offset_u, LDU, ref DUM, offset_dum
                                         , 1, ref WORK, IWORK + o_work, ref INFO);
                    }
                    else
            ...
            if (ISCL == 1)
            {
                ...
                {
                    _dlascl.Run("G", 0, 0, BIGNUM, ANRM, MINMN - 1
                ...
                {
                    _dlascl.Run("G", 0, 0, SMLNUM, ANRM, MINMN - 1
                                     , 1, ref WORK, 2 + o_work, MINMN, ref IERR);
                }
            }
            // *
