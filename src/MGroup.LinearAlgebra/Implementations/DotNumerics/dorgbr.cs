...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DORGBR
    {
        ...
        public DORGBR(LSAME lsame, ILAENV ilaenv, DORGLQ dorglq, DORGQR dorgqr, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _ilaenv = ilaenv; _dorglq = dorglq; _dorgqr = dorgqr; _xerbla = xerbla; 
                         ...
                         , double[] TAU, int offset_tau, ref double[] WORK, int offset_work, int LWORK, ref int INFO)
        {
            ...
            var LQUERY = false; var WANTQ = false; var I = 0; var IINFO = 0; var J = 0; var LWKOPT = 0; var MN = 0; var NB = 0; 
            ...
            INFO = 0;
            WANTQ = _lsame.Run(VECT, "Q");
            MN = Math.Min(M, N);
            LQUERY = LWORK ==  - 1;
            if (!WANTQ && !_lsame.Run(VECT, "P"))
            {
                INFO =  - 1;
            }
            else
            ...
            if (WANTQ)
            {
                ...
                if (M >= K)
                {
                    ...
                    // *           If m >= k, assume m >= n >= k
                    // *
                    _dorgqr.Run(M, N, K, ref A, offset_a, LDA, TAU, offset_tau
                ...
                else
                {
                    ...
                    if (M > 1)
                    {
                        ...
                        // *              Form Q(2:m,2:m)
                        // *
                        _dorgqr.Run(M - 1, M - 1, M - 1, ref A, 2+2 * LDA + o_a, LDA, TAU, offset_tau
                                         , ref WORK, offset_work, LWORK, ref IINFO);
                    }
                }
            }
            else
            {
                ...
                if (K < N)
                {
                    ...
                    // *           If k < n, assume k <= m <= n
                    // *
                    _dorglq.Run(M, N, K, ref A, offset_a, LDA, TAU, offset_tau
                                     , ref WORK, offset_work, LWORK, ref IINFO);
                    // *
                }
                else
                {
                    ...
                    if (N > 1)
                    {
                        ...
                        // *              Form P'(2:n,2:n)
                        // *
                        _dorglq.Run(N - 1, N - 1, N - 1, ref A, 2+2 * LDA + o_a, LDA, TAU, offset_tau
                                         , ref WORK, offset_work, LWORK, ref IINFO);
                    }
                }
            }
