...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLASD2
    {
        ...
        public DLASD2(DLAMCH dlamch, DLAPY2 dlapy2, DCOPY dcopy, DLACPY dlacpy, DLAMRG dlamrg, DLASET dlaset, DROT drot, XERBLA xerbla)
        {
            ...
            _dlamch = dlamch; _dlapy2 = dlapy2; _dcopy = dcopy; _dlacpy = dlacpy; _dlamrg = dlamrg; 
            _dlaset = dlaset;_drot = drot; _xerbla = xerbla; 
                         ...
                         , ref int[] IDX, int offset_idx, ref int[] IDXC, int offset_idxc, ref int[] IDXQ, int offset_idxq, ref int[] COLTYP, int offset_coltyp, ref int INFO)
        {
            ...
            var o_ctot = -1; var o_psm = -1; var CT = 0; var I = 0; var IDXI = 0; 
            var IDXJ = 0;var IDXJP = 0; var J = 0; var JP = 0; var JPREV = 0; var K2 = 0; var M = 0; var N = 0; var NLP1 = 0; 
            var NLP2 = 0;double C = 0; double EPS = 0; double HLFTOL = 0; double S = 0; double TAU = 0; double TOL = 0; 
            double Z1 = 0;
            ...
            var VT_NLP1 = 0; var VT_NLP2 = 0; var U2_1 = 0; 
            ...
            #region Array Index Correction
             ...
             var o_d = -1 + offset_d;  var o_z = -1 + offset_z;  var o_u = -1 - LDU + offset_u;  var o_vt = -1 - LDVT + offset_vt; 
             var o_dsigma = -1 + offset_dsigma; var o_u2 = -1 - LDU2 + offset_u2;  var o_vt2 = -1 - LDVT2 + offset_vt2; 
             var o_idxp = -1 + offset_idxp; var o_idx = -1 + offset_idx;  var o_idxc = -1 + offset_idxc; 
             var o_idxq = -1 + offset_idxq; var o_coltyp = -1 + offset_coltyp; 
            ...
            // *
            _dlamrg.Run(NL, NR, DSIGMA, 2 + o_dsigma, 1, 1, ref IDX, 2 + o_idx);
            // *
            for (I = 2; I <= N; I++)
            {
                IDXI = 1 + IDX[I + o_idx];
        ...
        LABEL100:;
            ...
            else
            {
                ...
                if (Math.Abs(D[J + o_d] - D[JPREV + o_d]) <= TOL)
                {
                    ...
                    _drot.Run(N, ref U, 1+IDXJP * LDU + o_u, 1, ref U, 1+IDXJ * LDU + o_u, 1, C
                                   , S);
                    _drot.Run(M, ref VT, IDXJP+1 * LDVT + o_vt, LDVT, ref VT, IDXJ+1 * LDVT + o_vt, LDVT, C
                                   , S);
        ...
        LABEL120:;
            ...
            for (J = 2; J <= N; J++)
            {
                ...
                _dcopy.Run(N, U, 1+IDXJ * LDU + o_u, 1, ref U2, 1+J * LDU2 + o_u2, 1);
                _dcopy.Run(M, VT, IDXJ+1 * LDVT + o_vt, LDVT, ref VT2, J+1 * LDVT2 + o_vt2, LDVT2);
            }
            // *
            ...
            if (M > N)
            {
                Z[1 + o_z] = _dlapy2.Run(Z1, Z[M + o_z]);
                if (Z[1 + o_z] <= TOL)
                {
                    C = ONE;
                    S = ZERO;
            ...
            // *
            _dcopy.Run(K - 1, U2, 2+1 * LDU2 + o_u2, 1, ref Z, 2 + o_z, 1);
            // *
            // *     Determine the first column of U2, the first row of VT2 and the
            // *     last row of VT.
            // *
            _dlaset.Run("A", N, 1, ZERO, ZERO, ref U2, offset_u2
                             , LDU2);
            ...
            {
                _dcopy.Run(M, VT, NLP1+1 * LDVT + o_vt, LDVT, ref VT2, 1+1 * LDVT2 + o_vt2, LDVT2);
            }
            if (M > N)
            {
                _dcopy.Run(M, VT, M+1 * LDVT + o_vt, LDVT, ref VT2, M+1 * LDVT2 + o_vt2, LDVT2);
            }
            // *
            ...
            if (N > K)
            {
                _dcopy.Run(N - K, DSIGMA, K + 1 + o_dsigma, 1, ref D, K + 1 + o_d, 1);
                _dlacpy.Run("A", N, N - K, U2, 1+(K + 1) * LDU2 + o_u2, LDU2, ref U, 1+(K + 1) * LDU + o_u
                                 , LDU);
                _dlacpy.Run("A", N - K, M, VT2, K + 1+1 * LDVT2 + o_vt2, LDVT2, ref VT, K + 1+1 * LDVT + o_vt
                                 , LDVT);
            }
            // *
