...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGEQP3
    {
        ...
        public DGEQP3(DGEQRF dgeqrf, DLAQP2 dlaqp2, DLAQPS dlaqps, DORMQR dormqr, DSWAP dswap, XERBLA xerbla, ILAENV ilaenv, DNRM2 dnrm2)
        {
            ...
            _dgeqrf = dgeqrf; _dlaqp2 = dlaqp2; _dlaqps = dlaqps; _dormqr = dormqr; _dswap = dswap; 
            _xerbla = xerbla;_ilaenv = ilaenv; _dnrm2 = dnrm2; 
                         ...
                         , ref double[] WORK, int offset_work, int LWORK, ref int INFO)
        {
            ...
            var LQUERY = false; var FJB = 0; var IWS = 0; var J = 0; var JB = 0; var LWKOPT = 0; var MINMN = 0; var MINWS = 0; 
            var NA = 0;var NB = 0; var NBMIN = 0; var NFXD = 0; var NX = 0; var SM = 0; var SMINMN = 0; var SN = 0; 
            var TOPBMN = 0;
            ...
            if (INFO == 0)
            {
                ...
                {
                    IWS = 3 * N + 1;
                    NB = _ilaenv.Run(INB, "DGEQRF", " ", M, N,  - 1,  - 1);
                    LWKOPT = 2 * N + (N + 1) * NB;
                }
            ...
            for (J = 1; J <= N; J++)
            {
                if (JPVT[J + o_jpvt] != 0)
                {
                    if (J != NFXD)
                    {
                        _dswap.Run(M, ref A, 1+J * LDA + o_a, 1, ref A, 1+NFXD * LDA + o_a, 1);
            ...
            if (NFXD > 0)
            {
                ...
                _dgeqrf.Run(M, NA, ref A, offset_a, LDA, ref TAU, offset_tau, ref WORK, offset_work
                                 , LWORK, ref INFO);
                ...
                if (NA < N)
                {
                    ...
                    _dormqr.Run("Left", "Transpose", M, N - NA, NA, ref A, offset_a
            ...
            if (NFXD < MINMN)
            {
                ...
                // *
                NB = _ilaenv.Run(INB, "DGEQRF", " ", SM, SN,  - 1,  - 1);
                NBMIN = 2;
                NX = 0;
                // *
                if (NB > 1 && NB < SMINMN)
                {
                    ...
                    // *
                    NX = Math.Max(0, _ilaenv.Run(IXOVER, "DGEQRF", " ", SM, SN,  - 1,  - 1));
                    // *
                    // *
                    if (NX < SMINMN)
                    {
                        ...
                        if (LWORK < MINWS)
                        {
                            ...
                            // *
                            NB = (LWORK - 2 * SN) / (SN + 1);
                            NBMIN = Math.Max(2, _ilaenv.Run(INBMIN, "DGEQRF", " ", SM, SN,  - 1,  - 1));
                            // *
                            // *
                        }
                    }
                ...
                for (J = NFXD + 1; J <= N; J++)
                {
                    WORK[J + o_work] = _dnrm2.Run(SM, A, NFXD + 1+J * LDA + o_a, 1);
                ...
                LABEL30:;
                    if (J <= TOPBMN)
                    {
                        ...
                        // *
                        _dlaqps.Run(M, N - J + 1, J - 1, JB, ref FJB, ref A, 1+J * LDA + o_a
                ...
                if (J <= MINMN)
                {
                    _dlaqp2.Run(M, N - J + 1, J - 1, ref A, 1+J * LDA + o_a, LDA, ref JPVT, J + o_jpvt
