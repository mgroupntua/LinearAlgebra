...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DTRCON
    {
        ...
        public DTRCON(LSAME lsame, IDAMAX idamax, DLAMCH dlamch, DLANTR dlantr, DLACON dlacon, DLATRS dlatrs, DRSCL drscl, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _idamax = idamax; _dlamch = dlamch; _dlantr = dlantr; _dlacon = dlacon; 
            _dlatrs = dlatrs;_drscl = drscl; _xerbla = xerbla; 
        ...
        public DTRCON()
        {
            ...
            var dlantr = new DLANTR(dlassq, lsame);
            var dlacon = new DLACON(idamax, dasum, dcopy);
            var dtrsv = new DTRSV(lsame, xerbla);
            var dlatrs = new DLATRS(lsame, idamax, dasum, ddot, dlamch, daxpy, dscal, dtrsv, xerbla);
            var drscl = new DRSCL(dlamch, dscal, dlabad);
                         ...
                         , ref double RCOND, ref double[] WORK, int offset_work, ref int[] IWORK, int offset_iwork, ref int INFO)
        {
            ...
            var NOUNIT = false; var ONENRM = false; var UPPER = false; var NORMIN = new string(' ', 1); var IX = 0; 
            var KASE = 0;var KASE1 = 0; double AINVNM = 0; double ANORM = 0; double SCALE = 0; double SMLNUM = 0; 
            double XNORM = 0;
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_work = -1 + offset_work;  var o_iwork = -1 + offset_iwork; 
            ...
            ONENRM = NORM == "1" || _lsame.Run(NORM, "O");
            NOUNIT = _lsame.Run(DIAG, "N");
            // *
            if (!ONENRM && !_lsame.Run(NORM, "I"))
            {
                INFO =  - 1;
            }
            else
            ...
            // *
            RCOND = ZERO;
            SMLNUM = _dlamch.Run("Safe minimum") * Convert.ToDouble(Math.Max(1, N));
            // *
            // *     Compute the norm of the triangular matrix A.
            // *
            ANORM = _dlantr.Run(NORM, UPLO, DIAG, N, N, A, offset_a, LDA, ref WORK, offset_work);
            // *
            ...
            if (ANORM > ZERO)
            {
                ...
                KASE = 0;
            LABEL10:;
                _dlacon.Run(N, ref WORK, N + 1 + o_work, ref WORK, offset_work, ref IWORK, offset_iwork, ref AINVNM, ref KASE);
                if (KASE != 0)
                {
                    if (KASE == KASE1)
                    {
                        ...
                        // *              Multiply by inv(A).
                        // *
                        _dlatrs.Run(UPLO, "No transpose", DIAG, NORMIN, N, A, offset_a
                    ...
                    else
                    {
                        ...
                        // *              Multiply by inv(A').
                        // *
                        _dlatrs.Run(UPLO, "Transpose", DIAG, NORMIN, N, A, offset_a
                                         , LDA, ref WORK, offset_work, ref SCALE, ref WORK, 2 * N + 1 + o_work, ref INFO);
                    }
                    ...
                    if (SCALE != ONE)
                    {
                        IX = _idamax.Run(N, WORK, offset_work, 1);
                        XNORM = Math.Abs(WORK[IX + o_work]);
                        if (SCALE < XNORM * SMLNUM || SCALE == ZERO) goto LABEL20;
                        _drscl.Run(N, SCALE, ref WORK, offset_work, 1);
                    }
                    goto LABEL10;
                }
                // *
