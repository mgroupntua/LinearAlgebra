...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGEQRF
    {
        ...
        public DGEQRF(DGEQR2 dgeqr2, DLARFB dlarfb, DLARFT dlarft, XERBLA xerbla, ILAENV ilaenv)
        {
            ...
            _dgeqr2 = dgeqr2; _dlarfb = dlarfb; _dlarft = dlarft; _xerbla = xerbla; _ilaenv = ilaenv; 
                         ...
                         , int LWORK, ref int INFO)
        {
            ...
            if (NB > 1 && NB < K)
            {
                ...
                // *
                NX = Math.Max(0, _ilaenv.Run(3, "DGEQRF", " ", M, N,  - 1,  - 1));
                if (NX < K)
                {
                    // *
                    ...
                    if (LWORK < IWS)
                    {
                        ...
                        // *
                        NB = LWORK / LDWORK;
                        NBMIN = Math.Max(2, _ilaenv.Run(2, "DGEQRF", " ", M, N,  - 1,  - 1));
                    }
                }
            }
            // *
            if (NB >= NBMIN && NB < K && NX < K)
            {
                ...
                for (I = 1; NB >= 0 ? I <= K - NX : I >= K - NX; I += NB)
                {
                    ...
                    // *           A(i:m,i:i+ib-1)
                    // *
                    _dgeqr2.Run(M - I + 1, IB, ref A, I+I * LDA + o_a, LDA, ref TAU, I + o_tau, ref WORK, offset_work
                                     , ref IINFO);
                    if (I + IB <= N)
                    {
                        // *
                        ...
                        // *
                        _dlarft.Run("Forward", "Columnwise", M - I + 1, IB, ref A, I+I * LDA + o_a, LDA
                                         , TAU, I + o_tau, ref WORK, offset_work, LDWORK);
                        // *
                        // *              Apply H' to A(i:m,i+ib:n) from the left
                        // *
                        _dlarfb.Run("Left", "Transpose", "Forward", "Columnwise", M - I + 1, N - I - IB + 1
            ...
            if (I <= K)
            {
                _dgeqr2.Run(M - I + 1, N - I + 1, ref A, I+I * LDA + o_a, LDA, ref TAU, I + o_tau, ref WORK, offset_work
                                 , ref IINFO);
            }
            // *
            WORK[1 + o_work] = IWS;
