...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGESV
    {
        ...
        public DGESV(DGETRF dgetrf, DGETRS dgetrs, XERBLA xerbla)
        {
            ...
            _dgetrf = dgetrf; _dgetrs = dgetrs; _xerbla = xerbla; 
        ...
        public DGESV()
        {
            ...
            var dgetrf = new DGETRF(dgemm, dgetf2, dlaswp, dtrsm, xerbla, ilaenv);
            var dgetrs = new DGETRS(lsame, dlaswp, dtrsm, xerbla);
                         ...
                         , int LDB, ref int INFO)
        {
            ...
            if (INFO != 0)
            {
                _xerbla.Run("DGESV ",  - INFO);
                return;
            }
            // *
            // *     Compute the LU factorization of A.
            // *
            _dgetrf.Run(N, N, ref A, offset_a, LDA, ref IPIV, offset_ipiv, ref INFO);
            if (INFO == 0)
            {
                // *
                // *        Solve the system A*X = B, overwriting B with X.
                // *
                _dgetrs.Run("No transpose", N, NRHS, A, offset_a, LDA, IPIV, offset_ipiv
