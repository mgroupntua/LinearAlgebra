...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLARFB
    {
        ...
        public DLARFB(LSAME lsame, DCOPY dcopy, DGEMM dgemm, DTRMM dtrmm)
        {
            ...
            _lsame = lsame; _dcopy = dcopy; _dgemm = dgemm; _dtrmm = dtrmm; 
                         ...
                         , int LDC, ref double[] WORK, int offset_work, int LDWORK)
        {
            ...
            var TRANST = new string(' ', 1); var I = 0; var J = 0; 
            ...
            var WORK_J = 0; var C_J = 0; var C_0 = 0; var C_1 = 0; 
            ...
            #region Array Index Correction
             ...
             var o_v = -1 - LDV + offset_v;  var o_t = -1 - LDT + offset_t;  var o_c = -1 - LDC + offset_c; 
             var o_work = -1 - LDWORK + offset_work;
            ...
            // *
            if (_lsame.Run(STOREV, "C"))
            {
                // *
                if (_lsame.Run(DIRECT, "F"))
                {
                    // *
                    ...
                    if (_lsame.Run(SIDE, "L"))
                    {
                        ...
                        for (J = 1; J <= K; J++)
                        {
                            _dcopy.Run(N, C, J+1 * LDC + o_c, LDC, ref WORK, 1+J * LDWORK + o_work, 1);
                        }
                        // *
                        // *              W := W * V1
                        // *
                        _dtrmm.Run("Right", "Lower", "No transpose", "Unit", N, K
                        ...
                        if (M > K)
                        {
                            ...
                            // *                 W := W + C2'*V2
                            // *
                            _dgemm.Run("Transpose", "No transpose", N, K, M - K, ONE
                        ...
                        // *              W := W * T'  or  W * T
                        // *
                        _dtrmm.Run("Right", "Upper", TRANST, "Non-unit", N, K
                        ...
                        if (M > K)
                        {
                            ...
                            // *                 C2 := C2 - V2 * W'
                            // *
                            _dgemm.Run("No transpose", "Transpose", M - K, N, K,  - ONE
                        ...
                        // *              W := W * V1'
                        // *
                        _dtrmm.Run("Right", "Lower", "Transpose", "Unit", N, K
                                        , ONE, V, offset_v, LDV, ref WORK, offset_work, LDWORK);
                        // *
                        // *              C1 := C1 - W'
                        // *
                    ...
                    else
                    {
                        if (_lsame.Run(SIDE, "R"))
                        {
                            ...
                            for (J = 1; J <= K; J++)
                            {
                                _dcopy.Run(M, C, 1+J * LDC + o_c, 1, ref WORK, 1+J * LDWORK + o_work, 1);
                            }
                            // *
                            // *              W := W * V1
                            // *
                            _dtrmm.Run("Right", "Lower", "No transpose", "Unit", M, K
                                            , ONE, V, offset_v, LDV, ref WORK, offset_work, LDWORK);
                            if (N > K)
                            {
                                // *
                                // *                 W := W + C2 * V2
                                // *
                                _dgemm.Run("No transpose", "No transpose", M, K, N - K, ONE
                            ...
                            // *              W := W * T  or  W * T'
                            // *
                            _dtrmm.Run("Right", "Upper", TRANS, "Non-unit", M, K
                                            , ONE, T, offset_t, LDT, ref WORK, offset_work, LDWORK);
                            // *
                            // *              C := C - W * V'
                            // *
                            if (N > K)
                            {
                                ...
                                // *                 C2 := C2 - W * V2'
                                // *
                                _dgemm.Run("No transpose", "Transpose", M, N - K, K,  - ONE
                            ...
                            // *              W := W * V1'
                            // *
                            _dtrmm.Run("Right", "Lower", "Transpose", "Unit", M, K
                                            , ONE, V, offset_v, LDV, ref WORK, offset_work, LDWORK);
                            // *
                            // *              C1 := C1 - W
                            // *
                ...
                else
                {
                    ...
                    if (_lsame.Run(SIDE, "L"))
                    {
                        ...
                        for (J = 1; J <= K; J++)
                        {
                            _dcopy.Run(N, C, M - K + J+1 * LDC + o_c, LDC, ref WORK, 1+J * LDWORK + o_work, 1);
                        }
                        // *
                        // *              W := W * V2
                        // *
                        _dtrmm.Run("Right", "Upper", "No transpose", "Unit", N, K
                        ...
                        // *              W := W * T'  or  W * T
                        // *
                        _dtrmm.Run("Right", "Lower", TRANST, "Non-unit", N, K
                                        , ONE, T, offset_t, LDT, ref WORK, offset_work, LDWORK);
                        // *
                        // *              C := C - V * W'
                        // *
                        ...
                        // *              W := W * V2'
                        // *
                        _dtrmm.Run("Right", "Upper", "Transpose", "Unit", N, K
                                        , ONE, V, M - K + 1+1 * LDV + o_v, LDV, ref WORK, offset_work, LDWORK);
                        // *
                        // *              C2 := C2 - W'
                        // *
                    ...
                    else
                    {
                        if (_lsame.Run(SIDE, "R"))
                        {
                            ...
                            for (J = 1; J <= K; J++)
                            {
                                _dcopy.Run(M, C, 1+(N - K + J) * LDC + o_c, 1, ref WORK, 1+J * LDWORK + o_work, 1);
                            }
                            // *
                            // *              W := W * V2
                            // *
                            _dtrmm.Run("Right", "Upper", "No transpose", "Unit", M, K
                            ...
                            // *              W := W * T  or  W * T'
                            // *
                            _dtrmm.Run("Right", "Lower", TRANS, "Non-unit", M, K
                                            , ONE, T, offset_t, LDT, ref WORK, offset_work, LDWORK);
                            // *
                            // *              C := C - W * V'
                            // *
                            ...
                            // *              W := W * V2'
                            // *
                            _dtrmm.Run("Right", "Upper", "Transpose", "Unit", M, K
                                            , ONE, V, N - K + 1+1 * LDV + o_v, LDV, ref WORK, offset_work, LDWORK);
                            // *
                            // *              C2 := C2 - W
                            // *
                    ...
                    if (_lsame.Run(DIRECT, "F"))
                    {
                        ...
                        if (_lsame.Run(SIDE, "L"))
                        {
                            ...
                            if (M > K)
                            {
                                ...
                                // *                 W := W + C2'*V2'
                                // *
                                _dgemm.Run("Transpose", "Transpose", N, K, M - K, ONE
                            ...
                            if (M > K)
                            {
                                ...
                                // *                 C2 := C2 - V2' * W'
                                // *
                                _dgemm.Run("Transpose", "Transpose", M - K, N, K,  - ONE
                        ...
                        else
                        {
                            if (_lsame.Run(SIDE, "R"))
                            {
                                ...
                                if (N > K)
                                {
                                    ...
                                    // *                 C2 := C2 - W * V2
                                    // *
                                    _dgemm.Run("No transpose", "No transpose", M, N - K, K,  - ONE
