...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DTRTI2
    {
        ...
        public DTRTI2(LSAME lsame, DSCAL dscal, DTRMV dtrmv, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _dscal = dscal; _dtrmv = dtrmv; _xerbla = xerbla; 
        ...
        public void Run(string UPLO, string DIAG, int N, ref double[] A, int offset_a, int LDA, ref int INFO)
        {
            ...
            var NOUNIT = false; var UPPER = false; var J = 0; double AJJ = 0; 
            ...
            if (UPPER)
            {
                ...
                for (J = 1; J <= N; J++)
                {
                    ...
                    // *
                    _dtrmv.Run("Upper", "No transpose", DIAG, J - 1, A, offset_a, LDA
                                    , ref A, 1+J * LDA + o_a, 1);
                    _dscal.Run(J - 1, AJJ, ref A, 1+J * LDA + o_a, 1);
                }
            }
            else
            {
                ...
                for (J = N; J >= 1; J +=  - 1)
                {
                    ...
                    if (J < N)
                    {
                        ...
                        // *
                        _dtrmv.Run("Lower", "No transpose", DIAG, N - J, A, J + 1+(J + 1) * LDA + o_a, LDA
                                        , ref A, J + 1+J * LDA + o_a, 1);
                        _dscal.Run(N - J, AJJ, ref A, J + 1+J * LDA + o_a, 1);
                    }
                }
            }
            // *
