...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAQR5
    {
        ...
        public DLAQR5(DLAMCH dlamch, DGEMM dgemm, DLABAD dlabad, DLACPY dlacpy, DLAQR1 dlaqr1, DLARFG dlarfg, DLASET dlaset, DTRMM dtrmm)
        {
            ...
            _dlamch = dlamch; _dgemm = dgemm; _dlabad = dlabad; _dlacpy = dlacpy; _dlaqr1 = dlaqr1; 
            _dlarfg = dlarfg;_dlaset = dlaset; _dtrmm = dtrmm; 
                         ...
                         , int LDWH)
        {
            ...
            double TST2 = 0;double ULP = 0; var I = 0; var I2 = 0; var I4 = 0; var INCOL = 0; var J = 0; var J2 = 0; var J4 = 0; 
            var JBOT = 0;var JCOL = 0; var JLEN = 0; var JROW = 0; var JTOP = 0; var K = 0; var K1 = 0; var KDU = 0; var KMS = 0; 
            var KNZ = 0;var KRCOL = 0; var KZS = 0; var M = 0; var M22 = 0; var MBOT = 0; var MEND = 0; var MSTART = 0; 
            var MTOP = 0;var NBMPS = 0; var NDCOL = 0; var NS = 0; var NU = 0; var ACCUM = false; var BLK22 = false; 
            var BMP22 = false;var offset_vt = 0; var o_vt = -1; 
            ...
            var H_0 = 0; var H_1 = 0; var H_2 = 0; var H_3 = 0; var H_4 = 0; var H_5 = 0; var U_0 = 0; var U_1 = 0; var U_2 = 0; 
            var U_3 = 0;var U_4 = 0; var U_5 = 0; var Z_0 = 0; var Z_1 = 0; var Z_2 = 0; var Z_3 = 0; var Z_4 = 0; var Z_5 = 0; 
            var H_6 = 0;var H_7 = 0; var H_8 = 0; var H_9 = 0; var U_6 = 0; var U_7 = 0; var U_8 = 0; var U_9 = 0; var Z_6 = 0; 
            var Z_7 = 0;var Z_8 = 0; var Z_9 = 0; 
            ...
            #region Array Index Correction
             ...
             var o_sr = -1 + offset_sr;  var o_si = -1 + offset_si;  var o_h = -1 - LDH + offset_h; 
             var o_z = -1 - LDZ + offset_z; var o_v = -1 - LDV + offset_v;  var o_u = -1 - LDU + offset_u; 
             var o_wv = -1 - LDWV + offset_wv; var o_wh = -1 - LDWH + offset_wh; 
            ...
            for (INCOL = 3 * (1 - NBMPS) + KTOP - 1; 3 * NBMPS - 2 >= 0 ? INCOL <= KBOT - 2 : INCOL >= KBOT - 2; INCOL += 3 * NBMPS - 2)
            {
                ...
                if (ACCUM)
                {
                    _dlaset.Run("ALL", KDU, KDU, ZERO, ONE, ref U, offset_u
                                     , LDU);
                }
                // *
                ...
                for (KRCOL = INCOL; KRCOL <= Math.Min(INCOL + 3 * NBMPS - 3, KBOT - 2); KRCOL++)
                {
                    ...
                    for (M = MTOP; M <= MBOT; M++)
                    {
                        ...
                        if (K == KTOP - 1)
                        {
                            _dlaqr1.Run(3, H, KTOP+KTOP * LDH + o_h, LDH, SR[2 * M - 1 + o_sr], SI[2 * M - 1 + o_si], SR[2 * M + o_sr]
                                             , SI[2 * M + o_si], ref V, 1+M * LDV + o_v);
                            ALPHA = V[1+M * LDV + o_v];
                            _dlarfg.Run(3, ref ALPHA, ref V, 2+M * LDV + o_v, 1, ref V[1+M * LDV + o_v]);
                        }
                        else
                        {
                            ...
                            _dlarfg.Run(3, ref BETA, ref V, 2+M * LDV + o_v, 1, ref V[1+M * LDV + o_v]);
                            // *
                            ...
                            else
                            {
                                ...
                                // *
                                _dlaqr1.Run(3, H, K + 1+(K + 1) * LDH + o_h, LDH, SR[2 * M - 1 + o_sr], SI[2 * M - 1 + o_si], SR[2 * M + o_sr]
                                ...
                                else
                                {
                                    ...
                                    // *
                                    ALPHA = VT[1 + o_vt];
                                    _dlarfg.Run(3, ref ALPHA, ref VT, 2 + o_vt, 1, ref VT[1 + o_vt]);
                    ...
                    if (BMP22)
                    {
                        if (K == KTOP - 1)
                        {
                            _dlaqr1.Run(2, H, K + 1+(K + 1) * LDH + o_h, LDH, SR[2 * M22 - 1 + o_sr], SI[2 * M22 - 1 + o_si], SR[2 * M22 + o_sr]
                                             , SI[2 * M22 + o_si], ref V, 1+M22 * LDV + o_v);
                            BETA = V[1+M22 * LDV + o_v];
                            _dlarfg.Run(2, ref BETA, ref V, 2+M22 * LDV + o_v, 1, ref V[1+M22 * LDV + o_v]);
                        }
                        else
                        {
                            BETA = H[K + 1+K * LDH + o_h];
                ...
                if (ACCUM)
                {
                    ...
                    if (!BLK22 || INCOL < KTOP || NDCOL > KBOT || NS <= 2)
                    {
                        ...
                        for (JCOL = Math.Min(NDCOL, KBOT) + 1; NH >= 0 ? JCOL <= JBOT : JCOL >= JBOT; JCOL += NH)
                        {
                            ...
                            _dgemm.Run("C", "N", NU, JLEN, NU, ONE
                                            , U, K1+K1 * LDU + o_u, LDU, H, INCOL + K1+JCOL * LDH + o_h, LDH, ZERO, ref WH, offset_wh
                                            , LDWH);
                            _dlacpy.Run("ALL", NU, JLEN, WH, offset_wh, LDWH, ref H, INCOL + K1+JCOL * LDH + o_h
                                             , LDH);
                        }
                        // *
                        // *              ==== Vertical multiply ====
                        ...
                        for (JROW = JTOP; NV >= 0 ? JROW <= Math.Max(KTOP, INCOL) - 1 : JROW >= Math.Max(KTOP, INCOL) - 1; JROW += NV)
                        {
                            ...
                            _dgemm.Run("N", "N", JLEN, NU, NU, ONE
                                            , H, JROW+(INCOL + K1) * LDH + o_h, LDH, U, K1+K1 * LDU + o_u, LDU, ZERO, ref WV, offset_wv
                                            , LDWV);
                            _dlacpy.Run("ALL", JLEN, NU, WV, offset_wv, LDWV, ref H, JROW+(INCOL + K1) * LDH + o_h
                                             , LDH);
                        }
                        // *
                        ...
                        if (WANTZ)
                        {
                            for (JROW = ILOZ; NV >= 0 ? JROW <= IHIZ : JROW >= IHIZ; JROW += NV)
                            {
                                ...
                                _dgemm.Run("N", "N", JLEN, NU, NU, ONE
                                                ...
                                                , LDWV);
                                _dlacpy.Run("ALL", JLEN, NU, WV, offset_wv, LDWV, ref Z, JROW+(INCOL + K1) * LDZ + o_z
                                                 , LDZ);
                            }
                        }
                    }
                    else
                    {
                        ...
                        for (JCOL = Math.Min(NDCOL, KBOT) + 1; NH >= 0 ? JCOL <= JBOT : JCOL >= JBOT; JCOL += NH)
                        {
                            ...
                            // *
                            _dlacpy.Run("ALL", KNZ, JLEN, H, INCOL + 1 + J2+JCOL * LDH + o_h, LDH, ref WH, KZS + 1+1 * LDWH + o_wh
                                             , LDWH);
                            // *
                            // *                 ==== Multiply by U21' ====
                            // *
                            _dlaset.Run("ALL", KZS, JLEN, ZERO, ZERO, ref WH, offset_wh
                                             , LDWH);
                            _dtrmm.Run("L", "U", "C", "N", KNZ, JLEN
                                            , ONE, U, J2 + 1+(1 + KZS) * LDU + o_u, LDU, ref WH, KZS + 1+1 * LDWH + o_wh, LDWH);
                            // *
                            // *                 ==== Multiply top of H by U11' ====
                            // *
                            _dgemm.Run("C", "N", I2, JLEN, J2, ONE
                                            , U, offset_u, LDU, H, INCOL + 1+JCOL * LDH + o_h, LDH, ONE, ref WH, offset_wh
                                            , LDWH);
                            // *
                            // *                 ==== Copy top of H bottom of WH ====
                            // *
                            _dlacpy.Run("ALL", J2, JLEN, H, INCOL + 1+JCOL * LDH + o_h, LDH, ref WH, I2 + 1+1 * LDWH + o_wh
                                             , LDWH);
                            // *
                            // *                 ==== Multiply by U21' ====
                            // *
                            _dtrmm.Run("L", "L", "C", "N", J2, JLEN
                                            , ONE, U, 1+(I2 + 1) * LDU + o_u, LDU, ref WH, I2 + 1+1 * LDWH + o_wh, LDWH);
                            // *
                            // *                 ==== Multiply by U22 ====
                            // *
                            _dgemm.Run("C", "N", I4 - I2, JLEN, J4 - J2, ONE
                                            , U, J2 + 1+(I2 + 1) * LDU + o_u, LDU, H, INCOL + 1 + J2+JCOL * LDH + o_h, LDH, ONE, ref WH, I2 + 1+1 * LDWH + o_wh
                                            , LDWH);
                            // *
                            // *                 ==== Copy it back ====
                            // *
                            _dlacpy.Run("ALL", KDU, JLEN, WH, offset_wh, LDWH, ref H, INCOL + 1+JCOL * LDH + o_h
                                             , LDH);
                        }
                        // *
                        // *              ==== Vertical multiply ====
                        ...
                        for (JROW = JTOP; NV >= 0 ? JROW <= Math.Max(INCOL, KTOP) - 1 : JROW >= Math.Max(INCOL, KTOP) - 1; JROW += NV)
                        {
                            ...
                            // *
                            _dlacpy.Run("ALL", JLEN, KNZ, H, JROW+(INCOL + 1 + J2) * LDH + o_h, LDH, ref WV, 1+(1 + KZS) * LDWV + o_wv
                                             , LDWV);
                            // *
                            // *                 ==== Multiply by U21 ====
                            // *
                            _dlaset.Run("ALL", JLEN, KZS, ZERO, ZERO, ref WV, offset_wv
                                             , LDWV);
                            _dtrmm.Run("R", "U", "N", "N", JLEN, KNZ
                                            , ONE, U, J2 + 1+(1 + KZS) * LDU + o_u, LDU, ref WV, 1+(1 + KZS) * LDWV + o_wv, LDWV);
                            // *
                            // *                 ==== Multiply by U11 ====
                            // *
                            _dgemm.Run("N", "N", JLEN, I2, J2, ONE
                                            , H, JROW+(INCOL + 1) * LDH + o_h, LDH, U, offset_u, LDU, ONE, ref WV, offset_wv
                                            , LDWV);
                            // *
                            // *                 ==== Copy left of H to right of scratch ====
                            // *
                            _dlacpy.Run("ALL", JLEN, J2, H, JROW+(INCOL + 1) * LDH + o_h, LDH, ref WV, 1+(1 + I2) * LDWV + o_wv
                                             , LDWV);
                            // *
                            // *                 ==== Multiply by U21 ====
                            // *
                            _dtrmm.Run("R", "L", "N", "N", JLEN, I4 - I2
                                            , ONE, U, 1+(I2 + 1) * LDU + o_u, LDU, ref WV, 1+(1 + I2) * LDWV + o_wv, LDWV);
                            // *
                            // *                 ==== Multiply by U22 ====
                            // *
                            _dgemm.Run("N", "N", JLEN, I4 - I2, J4 - J2, ONE
                                            , H, JROW+(INCOL + 1 + J2) * LDH + o_h, LDH, U, J2 + 1+(I2 + 1) * LDU + o_u, LDU, ONE, ref WV, 1+(1 + I2) * LDWV + o_wv
                                            , LDWV);
                            // *
                            // *                 ==== Copy it back ====
                            // *
                            _dlacpy.Run("ALL", JLEN, KDU, WV, offset_wv, LDWV, ref H, JROW+(INCOL + 1) * LDH + o_h
                                             , LDH);
                        }
                        // *
                        ...
                        if (WANTZ)
                        {
                            for (JROW = ILOZ; NV >= 0 ? JROW <= IHIZ : JROW >= IHIZ; JROW += NV)
                            {
                                ...
                                // *
                                _dlacpy.Run("ALL", JLEN, KNZ, Z, JROW+(INCOL + 1 + J2) * LDZ + o_z, LDZ, ref WV, 1+(1 + KZS) * LDWV + o_wv
                                                 , LDWV);
                                // *
                                // *                    ==== Multiply by U12 ====
                                // *
                                ...
                                // *
                                _dlacpy.Run("ALL", JLEN, J2, Z, JROW+(INCOL + 1) * LDZ + o_z, LDZ, ref WV, 1+(1 + I2) * LDWV + o_wv
                                                 , LDWV);
                                // *
                                // *                    ==== Multiply by U21 ====
                                // *
                                ...
                                // *
                                _dlacpy.Run("ALL", JLEN, KDU, WV, offset_wv, LDWV, ref Z, JROW+(INCOL + 1) * LDZ + o_z
                                                 , LDZ);
                            }
                        }
                    }
