...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLATRS
    {
        ...
        public DLATRS(LSAME lsame, IDAMAX idamax, DASUM dasum, DDOT ddot, DLAMCH dlamch, DAXPY daxpy, DSCAL dscal, DTRSV dtrsv, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _idamax = idamax; _dasum = dasum; _ddot = ddot; _dlamch = dlamch; 
            _daxpy = daxpy;_dscal = dscal; _dtrsv = dtrsv; _xerbla = xerbla; 
        ...
        public DLATRS()
        {
            ...
            var dtrsv = new DTRSV(lsame, xerbla);
                         ...
                         , int LDA, ref double[] X, int offset_x, ref double SCALE, ref double[] CNORM, int offset_cnorm, ref int INFO)
        {
            ...
            var NOTRAN = false; var NOUNIT = false; var UPPER = false; var I = 0; var IMAX = 0; var J = 0; var JFIRST = 0; 
            var JINC = 0;var JLAST = 0; double BIGNUM = 0; double GROW = 0; double REC = 0; double SMLNUM = 0; double SUMJ = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_x = -1 + offset_x;  var o_cnorm = -1 + offset_cnorm; 
            ...
            {
                if (!NOTRAN && !_lsame.Run(TRANS, "T") && !_lsame.Run(TRANS, "C"))
                {
                    INFO =  - 2;
                }
                else
                {
                    if (!NOUNIT && !_lsame.Run(DIAG, "U"))
                    {
                        INFO =  - 3;
                    }
                    else
                    {
                        if (!_lsame.Run(NORMIN, "Y") && !_lsame.Run(NORMIN, "N"))
                        {
                            INFO =  - 4;
                        }
                        else
            ...
            // *
            SMLNUM = _dlamch.Run("Safe minimum") / _dlamch.Run("Precision");
            BIGNUM = ONE / SMLNUM;
            SCALE = ONE;
            // *
            if (_lsame.Run(NORMIN, "N"))
            {
                // *
                ...
                if (UPPER)
                {
                    ...
                    for (J = 1; J <= N; J++)
                    {
                        CNORM[J + o_cnorm] = _dasum.Run(J - 1, A, 1+J * LDA + o_a, 1);
                    }
                }
                else
                {
                    ...
                    for (J = 1; J <= N - 1; J++)
                    {
                        CNORM[J + o_cnorm] = _dasum.Run(N - J, A, J + 1+J * LDA + o_a, 1);
                    }
            ...
            // *     greater than BIGNUM.
            // *
            IMAX = _idamax.Run(N, CNORM, offset_cnorm, 1);
            ...
            else
            {
                ...
                _dscal.Run(N, TSCAL, ref CNORM, offset_cnorm, 1);
            }
            // *
            // *     Compute a bound on the computed solution vector to see if the
            // *     Level 2 BLAS routine DTRSV can be used.
            // *
            J = _idamax.Run(N, X, offset_x, 1);
            ...
            if (GROW * TSCAL > SMLNUM)
            {
                ...
                // *
                _dtrsv.Run(UPLO, TRANS, DIAG, N, A, offset_a, LDA
                                , ref X, offset_x, 1);
            }
            else
            {
                ...
                if (XMAX > BIGNUM)
                {
                    ...
                    // *
                    SCALE = BIGNUM / XMAX;
                    _dscal.Run(N, SCALE, ref X, offset_x, 1);
                    XMAX = BIGNUM;
                }
                // *
                if (NOTRAN)
                        ...
                        if (TJJ > SMLNUM)
                        {
                            ...
                            if (TJJ < ONE)
                            {
                                if (XJ > TJJ * BIGNUM)
                                {
                                    ...
                                    // *
                                    REC = ONE / XJ;
                                    _dscal.Run(N, REC, ref X, offset_x, 1);
                                    SCALE *= REC;
                                    XMAX *= REC;
                                }
                            }
                    ...
                    LABEL100:;
                        ...
                        else
                        {
                            if (XJ * CNORM[J + o_cnorm] > BIGNUM - XMAX)
                            {
                                ...
                                // *                 Scale x by 1/2.
                                // *
                                _dscal.Run(N, HALF, ref X, offset_x, 1);
                                SCALE *= HALF;
                            }
                        }
                        // *
                        if (UPPER)
                        {
                            if (J > 1)
                            {
                                ...
                                // *
                                _daxpy.Run(J - 1,  - X[J + o_x] * TSCAL, A, 1+J * LDA + o_a, 1, ref X, offset_x, 1);
                                I = _idamax.Run(J - 1, X, offset_x, 1);
                        ...
                        else
                        {
                            if (J < N)
                            {
                                ...
                                // *                       x(j+1:n) := x(j+1:n) - x(j) * A(j+1:n,j)
                                // *
                                _daxpy.Run(N - J,  - X[J + o_x] * TSCAL, A, J + 1+J * LDA + o_a, 1, ref X, J + 1 + o_x, 1);
                                I = J + _idamax.Run(N - J, X, J + 1 + o_x, 1);
                                XMAX = Math.Abs(X[I + o_x]);
                            }
                        }
                    }
                ...
                else
                {
                    ...
                    for (J = JFIRST; JINC >= 0 ? J <= JLAST : J >= JLAST; J += JINC)
                    {
                        ...
                        if (USCAL == ONE)
                        {
                            ...
                            if (UPPER)
                            {
                                SUMJ = _ddot.Run(J - 1, A, 1+J * LDA + o_a, 1, X, offset_x, 1);
                            }
                            else
                            {
                                if (J < N)
                                {
                                    SUMJ = _ddot.Run(N - J, A, J + 1+J * LDA + o_a, 1, X, J + 1 + o_x, 1);
                                }
                            }
                        }
                        else
            ...
            if (TSCAL != ONE)
            {
                _dscal.Run(N, ONE / TSCAL, ref CNORM, offset_cnorm, 1);
            }
            // *
            return;
            // *
