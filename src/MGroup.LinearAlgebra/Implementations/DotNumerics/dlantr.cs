...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLANTR
    {
        ...
        double dlantr = 0;
            ...
            var UDIAG = false; var I = 0; var J = 0; double SCALE = 0; double SUM = 0; double VALUE = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_work = -1 + offset_work; 
            ...
            {
                if (_lsame.Run(NORM, "M"))
                {
                    // *
                    // *        Find max(abs(A(i,j))).
                    // *
                    if (_lsame.Run(DIAG, "U"))
                    {
                        VALUE = ONE;
                ...
                {
                    if (_lsame.Run(NORM, "O") || NORM == "1")
                    {
                        // *
                        // *        Find norm1(A).
                        // *
                        VALUE = ZERO;
                        UDIAG = _lsame.Run(DIAG, "U");
                        if (_lsame.Run(UPLO, "U"))
                        {
                            for (J = 1; J <= N; J++)
                            {
                    ...
                    else
                    {
                        ...
                        {
                            if (_lsame.Run(NORM, "F") || _lsame.Run(NORM, "E"))
                            {
                                // *
                                // *        Find normF(A).
                                // *
                                if (_lsame.Run(UPLO, "U"))
                                {
                                    if (_lsame.Run(DIAG, "U"))
                                    {
                                        ...
                                        for (J = 2; J <= N; J++)
                                        {
                                            _dlassq.Run(Math.Min(M, J - 1), A, 1+J * LDA + o_a, 1, ref SCALE, ref SUM);
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        for (J = 1; J <= N; J++)
                                        {
                                            _dlassq.Run(Math.Min(M, J), A, 1+J * LDA + o_a, 1, ref SCALE, ref SUM);
                                        }
                                    }
                                }
                                else
                                {
                                    if (_lsame.Run(DIAG, "U"))
                                    {
                                        ...
                                        for (J = 1; J <= N; J++)
                                        {
                                            _dlassq.Run(M - J, A, Math.Min(M, J + 1)+J * LDA + o_a, 1, ref SCALE, ref SUM);
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        for (J = 1; J <= N; J++)
                                        {
                                            _dlassq.Run(M - J + 1, A, J+J * LDA + o_a, 1, ref SCALE, ref SUM);
                                        }
                                    }
                                }
