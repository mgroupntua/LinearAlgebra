...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAEDA
    {
        ...
        public DLAEDA(DCOPY dcopy, DGEMV dgemv, DROT drot, XERBLA xerbla)
        {
            ...
            _dcopy = dcopy; _dgemv = dgemv; _drot = drot; _xerbla = xerbla; 
                         ...
                         , ref double[] ZTEMP, int offset_ztemp, ref int INFO)
        {
            ...
            var BSIZ1 = 0; var BSIZ2 = 0; var CURR = 0; var I = 0; var K = 0; var MID = 0; var PSIZ1 = 0; var PSIZ2 = 0; 
            var PTR = 0;var ZPTR1 = 0; 
            ...
            #region Array Index Correction
             ...
             var o_prmptr = -1 + offset_prmptr;  var o_perm = -1 + offset_perm;  var o_givptr = -1 + offset_givptr; 
             var o_givcol = -3 + offset_givcol; var o_givnum = -3 + offset_givnum;  var o_q = -1 + offset_q; 
             var o_qptr = -1 + offset_qptr; var o_z = -1 + offset_z;  var o_ztemp = -1 + offset_ztemp; 
            ...
            _dcopy.Run(BSIZ1, Q, QPTR[CURR + o_qptr] + BSIZ1 - 1 + o_q, BSIZ1, ref Z, MID - BSIZ1 + o_z, 1);
            _dcopy.Run(BSIZ2, Q, QPTR[CURR + 1 + o_qptr] + o_q, BSIZ2, ref Z, MID + o_z, 1);
            ...
            for (K = 1; K <= CURLVL - 1; K++)
            {
                ...
                {
                    _drot.Run(1, ref Z, ZPTR1 + GIVCOL[1+I * 2 + o_givcol] - 1 + o_z, 1, ref Z, ZPTR1 + GIVCOL[2+I * 2 + o_givcol] - 1 + o_z, 1, GIVNUM[1+I * 2 + o_givnum]
                                   , GIVNUM[2+I * 2 + o_givnum]);
                }
                for (I = GIVPTR[CURR + 1 + o_givptr]; I <= GIVPTR[CURR + 2 + o_givptr] - 1; I++)
                {
                    _drot.Run(1, ref Z, MID - 1 + GIVCOL[1+I * 2 + o_givcol] + o_z, 1, ref Z, MID - 1 + GIVCOL[2+I * 2 + o_givcol] + o_z, 1, GIVNUM[1+I * 2 + o_givnum]
                                   , GIVNUM[2+I * 2 + o_givnum]);
                }
                PSIZ1 = PRMPTR[CURR + 1 + o_prmptr] - PRMPTR[CURR + o_prmptr];
                PSIZ2 = PRMPTR[CURR + 2 + o_prmptr] - PRMPTR[CURR + 1 + o_prmptr];
                ...
                if (BSIZ1 > 0)
                {
                    _dgemv.Run("T", BSIZ1, BSIZ1, ONE, Q, QPTR[CURR + o_qptr] + o_q, BSIZ1
                                    , ZTEMP, 1 + o_ztemp, 1, ZERO, ref Z, ZPTR1 + o_z, 1);
                }
                _dcopy.Run(PSIZ1 - BSIZ1, ZTEMP, BSIZ1 + 1 + o_ztemp, 1, ref Z, ZPTR1 + BSIZ1 + o_z, 1);
                if (BSIZ2 > 0)
                {
                    _dgemv.Run("T", BSIZ2, BSIZ2, ONE, Q, QPTR[CURR + 1 + o_qptr] + o_q, BSIZ2
                                    , ZTEMP, PSIZ1 + 1 + o_ztemp, 1, ZERO, ref Z, MID + o_z, 1);
                }
                _dcopy.Run(PSIZ2 - BSIZ2, ZTEMP, PSIZ1 + BSIZ2 + 1 + o_ztemp, 1, ref Z, MID + BSIZ2 + o_z, 1);
                // *
