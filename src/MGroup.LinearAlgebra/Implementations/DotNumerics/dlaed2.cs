...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAED2
    {
        ...
        public DLAED2(IDAMAX idamax, DLAMCH dlamch, DLAPY2 dlapy2, DCOPY dcopy, DLACPY dlacpy, DLAMRG dlamrg, DROT drot, DSCAL dscal, XERBLA xerbla)
        {
            ...
            _idamax = idamax; _dlamch = dlamch; _dlapy2 = dlapy2; _dcopy = dcopy; _dlacpy = dlacpy; 
            _dlamrg = dlamrg;_drot = drot; _dscal = dscal; _xerbla = xerbla; 
                         ...
                         , ref int[] INDX, int offset_indx, ref int[] INDXC, int offset_indxc, ref int[] INDXP, int offset_indxp, ref int[] COLTYP, int offset_coltyp, ref int INFO)
        {
            ...
            var o_ctot = -1;var o_psm = -1; var CT = 0; var I = 0; var IMAX = 0; 
            var IQ1 = 0;var IQ2 = 0; var J = 0; var JMAX = 0; var JS = 0; var K2 = 0; var N1P1 = 0; var N2 = 0; var NJ = 0; 
            var PJ = 0;double C = 0; double EPS = 0; double S = 0; double T = 0; double TAU = 0; double TOL = 0; 
            ...
            #region Array Index Correction
             ...
             var o_d = -1 + offset_d;  var o_q = -1 - LDQ + offset_q;  var o_indxq = -1 + offset_indxq;  var o_z = -1 + offset_z; 
             var o_dlamda = -1 + offset_dlamda; var o_w = -1 + offset_w;  var o_q2 = -1 + offset_q2; 
             var o_indx = -1 + offset_indx; var o_indxc = -1 + offset_indxc;  var o_indxp = -1 + offset_indxp; 
             var o_coltyp = -1 + offset_coltyp;
            ...
            if (RHO < ZERO)
            {
                _dscal.Run(N2, MONE, ref Z, N1P1 + o_z, 1);
            }
            // *
            ...
            // *
            T = ONE / Math.Sqrt(TWO);
            _dscal.Run(N, T, ref Z, offset_z, 1);
            // *
            ...
            _dlamrg.Run(N1, N2, DLAMDA, offset_dlamda, 1, 1, ref INDXC, offset_indxc);
            for (I = 1; I <= N; I++)
            {
            ...
            // *
            IMAX = _idamax.Run(N, Z, offset_z, 1);
            JMAX = _idamax.Run(N, D, offset_d, 1);
            ...
            if (RHO * Math.Abs(Z[IMAX + o_z]) <= TOL)
            {
                ...
                {
                    I = INDX[J + o_indx];
                    _dcopy.Run(N, Q, 1+I * LDQ + o_q, 1, ref Q2, IQ2 + o_q2, 1);
                    DLAMDA[J + o_dlamda] = D[I + o_d];
                    IQ2 += N;
                }
                _dlacpy.Run("A", N, N, Q2, offset_q2, N, ref Q, offset_q
                                 , LDQ);
                _dcopy.Run(N, DLAMDA, offset_dlamda, 1, ref D, offset_d, 1);
                goto LABEL190;
            }
            // *
        ...
        LABEL80:;
            ...
            else
            {
                ...
                if (Math.Abs(T * C * S) <= TOL)
                {
                    ...
                    _drot.Run(N, ref Q, 1+PJ * LDQ + o_q, 1, ref Q, 1+NJ * LDQ + o_q, 1, C
                                   , S);
        ...
        LABEL100:;
            ...
            {
                JS = INDX[I + o_indx];
                _dcopy.Run(N1, Q, 1+JS * LDQ + o_q, 1, ref Q2, IQ1 + o_q2, 1);
                Z[I + o_z] = D[JS + o_d];
                I += 1;
                IQ1 += N1;
            }
            ...
            for (J = 1; J <= CTOT[2 + o_ctot]; J++)
            {
                JS = INDX[I + o_indx];
                _dcopy.Run(N1, Q, 1+JS * LDQ + o_q, 1, ref Q2, IQ1 + o_q2, 1);
                _dcopy.Run(N2, Q, N1 + 1+JS * LDQ + o_q, 1, ref Q2, IQ2 + o_q2, 1);
                Z[I + o_z] = D[JS + o_d];
                I += 1;
                IQ1 += N1;
                IQ2 += N2;
            ...
            {
                JS = INDX[I + o_indx];
                _dcopy.Run(N, Q, 1+JS * LDQ + o_q, 1, ref Q2, IQ2 + o_q2, 1);
                IQ2 += N;
                Z[I + o_z] = D[JS + o_d];
                I += 1;
            }
            ...
            // *
            _dlacpy.Run("A", N, CTOT[4 + o_ctot], Q2, IQ1 + o_q2, N, ref Q, 1+(K + 1) * LDQ + o_q
                             , LDQ);
            _dcopy.Run(N - K, Z, K + 1 + o_z, 1, ref D, K + 1 + o_d, 1);
            // *
