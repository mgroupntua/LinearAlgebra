...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGETF2
    {
        ...
        public DGETF2(DLAMCH dlamch, IDAMAX idamax, DGER dger, DSCAL dscal, DSWAP dswap, XERBLA xerbla)
        {
            ...
            _dlamch = dlamch; _idamax = idamax; _dger = dger; _dscal = dscal; _dswap = dswap; 
            _xerbla = xerbla;
        ...
        public void Run(int M, int N, ref double[] A, int offset_a, int LDA, ref int[] IPIV, int offset_ipiv, ref int INFO)
        {
            ...
            double SFMIN = 0; var I = 0; var J = 0; var JP = 0; 
            ...
            for (J = 1; J <= Math.Min(M, N); J++)
            {
                ...
                // *
                JP = J - 1 + _idamax.Run(M - J + 1, A, J+J * LDA + o_a, 1);
                IPIV[J + o_ipiv] = JP;
                if (A[JP+J * LDA + o_a] != ZERO)
                {
                    ...
                    // *
                    if (JP != J) _dswap.Run(N, ref A, J+1 * LDA + o_a, LDA, ref A, JP+1 * LDA + o_a, LDA);
                    // *
                    ...
                    if (J < M)
                    {
                        ...
                        {
                            _dscal.Run(M - J, ONE / A[J+J * LDA + o_a], ref A, J + 1+J * LDA + o_a, 1);
                        }
                        else
                        {
                            A_J = J * LDA + o_a;
                ...
                if (J < Math.Min(M, N))
                {
                    ...
                    // *
                    _dger.Run(M - J, N - J,  - ONE, A, J + 1+J * LDA + o_a, 1, A, J+(J + 1) * LDA + o_a
