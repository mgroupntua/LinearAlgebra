...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGEQPF
    {
        ...
        public DGEQPF(DGEQR2 dgeqr2, DLARF dlarf, DLARFG dlarfg, DORM2R dorm2r, DSWAP dswap, XERBLA xerbla, IDAMAX idamax, DLAMCH dlamch, DNRM2 dnrm2)
        {
			...
			_dgeqr2 = dgeqr2; _dlarf = dlarf; _dlarfg = dlarfg; _dorm2r = dorm2r; _dswap = dswap;
			_xerbla = xerbla; _idamax = idamax; _dlamch = dlamch; _dnrm2 = dnrm2; 
                         ...
                         , ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var I = 0; var ITEMP = 0; var J = 0; var MA = 0; var MN = 0; var PVT = 0; double AII = 0; double TEMP = 0; 
            ...
            for (I = 1; I <= N; I++)
            {
                if (JPVT[I + o_jpvt] != 0)
                {
                    if (I != ITEMP)
                    {
						_dswap.Run(M, ref A, 1+I * LDA + o_a, 1, ref A, 1+ITEMP * LDA + o_a, 1);
            ...
            {
                MA = Math.Min(ITEMP, M);
				_dgeqr2.Run(M, MA, ref A, offset_a, LDA, ref TAU, offset_tau, ref WORK, offset_work
                                 , ref INFO);
                if (MA < N)
                {
					_dorm2r.Run("Left", "Transpose", M, N - MA, MA, ref A, offset_a
            ...
            if (ITEMP < MN)
            {
                ...
                for (I = ITEMP + 1; I <= N; I++)
                {
                    WORK[I + o_work] = _dnrm2.Run(M - ITEMP, A, ITEMP + 1+I * LDA + o_a, 1);
                ...
                for (I = ITEMP + 1; I <= MN; I++)
                {
                    ...
                    // *
                    PVT = I - 1 + _idamax.Run(N - I + 1, WORK, I + o_work, 1);
                    // *
                    if (PVT != I)
                    {
						_dswap.Run(M, ref A, 1+PVT * LDA + o_a, 1, ref A, 1+I * LDA + o_a, 1);
                    ...
                    if (I < M)
                    {
						_dlarfg.Run(M - I + 1, ref A[I+I * LDA + o_a], ref A, I + 1+I * LDA + o_a, 1, ref TAU[I + o_tau]);
                    }
                    else
                    {
						_dlarfg.Run(1, ref A[M+M * LDA + o_a], ref A, M+M * LDA + o_a, 1, ref TAU[M + o_tau]);
                    }
                    // *
                    if (I < N)
                    {
                        ...
                        AII = A[I+I * LDA + o_a];
                        A[I+I * LDA + o_a] = ONE;
						_dlarf.Run("LEFT", M - I + 1, N - I, A, I+I * LDA + o_a, 1, TAU[I + o_tau]
                    ...
                    for (J = I + 1; J <= N; J++)
                    {
                        if (WORK[J + o_work] != ZERO)
                        {
                            ...
                            if (TEMP2 <= TOL3Z)
                            {
                                if (M - I > 0)
                                {
                                    WORK[J + o_work] = _dnrm2.Run(M - I, A, I + 1+J * LDA + o_a, 1);
