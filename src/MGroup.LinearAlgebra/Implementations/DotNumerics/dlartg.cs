...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLARTG
    {
        ...
        public void Run(double F, double G, ref double CS, ref double SN, ref double R)
        {
            ...
            var COUNT = 0; var I = 0; double EPS = 0; double F1 = 0; double G1 = 0; double SAFMIN = 0; double SAFMN2 = 0; 
            ...
            EPS = _dlamch.Run("E");
            SAFMN2 = Math.Pow(_dlamch.Run("B"),Convert.ToInt32(Math.Truncate(Math.Log(SAFMIN / EPS) / Math.Log(_dlamch.Run("B")) / TWO)));
            SAFMX2 = ONE / SAFMN2;
            // *        FIRST = .FALSE.
            // *     END IF
            if (G == ZERO)
