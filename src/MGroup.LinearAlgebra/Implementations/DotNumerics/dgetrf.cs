...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGETRF
    {
        ...
        public DGETRF(DGEMM dgemm, DGETF2 dgetf2, DLASWP dlaswp, DTRSM dtrsm, XERBLA xerbla, ILAENV ilaenv)
        {
            ...
            _dgemm = dgemm; _dgetf2 = dgetf2; _dlaswp = dlaswp; _dtrsm = dtrsm; _xerbla = xerbla; 
            _ilaenv = ilaenv;
        ...
        public DGETRF()
        {
            ...
            var dgetf2 = new DGETF2(dlamch, idamax, dger, dscal, dswap, xerbla);
        ...
        public void Run(int M, int N, ref double[] A, int offset_a, int LDA, ref int[] IPIV, int offset_ipiv, ref int INFO)
        {
            ...
            var I = 0; var IINFO = 0; var J = 0; var JB = 0; var NB = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_ipiv = -1 + offset_ipiv; 
            ...
            // *
            NB = _ilaenv.Run(1, "DGETRF", " ", M, N,  - 1,  - 1);
            if (NB <= 1 || NB >= Math.Min(M, N))
            {
                // *
                // *        Use unblocked code.
                // *
                _dgetf2.Run(M, N, ref A, offset_a, LDA, ref IPIV, offset_ipiv, ref INFO);
            }
            else
            {
                // *
                ...
                for (J = 1; NB >= 0 ? J <= Math.Min(M, N) : J >= Math.Min(M, N); J += NB)
                {
                    ...
                    // *           singularity.
                    // *
                    _dgetf2.Run(M - J + 1, JB, ref A, J+J * LDA + o_a, LDA, ref IPIV, J + o_ipiv, ref IINFO);
                    // *
                    ...
                    // *
                    _dlaswp.Run(J - 1, ref A, offset_a, LDA, J, J + JB - 1, IPIV, offset_ipiv
                                     , 1);
                    // *
                    if (J + JB <= N)
                    {
                        ...
                        // *
                        _dlaswp.Run(N - J - JB + 1, ref A, 1+(J + JB) * LDA + o_a, LDA, J, J + JB - 1, IPIV, offset_ipiv
                                         , 1);
                        // *
                        // *              Compute block row of U.
                        // *
                        _dtrsm.Run("Left", "Lower", "No transpose", "Unit", JB, N - J - JB + 1
                        ...
                        if (J + JB <= M)
                        {
                            ...
                            // *
                            _dgemm.Run("No transpose", "No transpose", M - J - JB + 1, N - J - JB + 1, JB,  - ONE
