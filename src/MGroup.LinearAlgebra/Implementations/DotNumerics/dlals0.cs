...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLALS0
    {
        ...
        public DLALS0(DCOPY dcopy, DGEMV dgemv, DLACPY dlacpy, DLASCL dlascl, DROT drot, DSCAL dscal, XERBLA xerbla, DLAMC3 dlamc3, DNRM2 dnrm2)
        {
            ...
            _dcopy = dcopy; _dgemv = dgemv; _dlacpy = dlacpy; _dlascl = dlascl; _drot = drot; 
            _dscal = dscal;_xerbla = xerbla; _dlamc3 = dlamc3; _dnrm2 = dnrm2; 
                         ...
                         , double[] Z, int offset_z, int K, double C, double S, ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var I = 0; var J = 0; var M = 0; var N = 0; var NLP1 = 0; double DIFLJ = 0; double DIFRJ = 0; double DJ = 0; 
            ...
            #region Array Index Correction
             ...
             var o_b = -1 - LDB + offset_b;  var o_bx = -1 - LDBX + offset_bx;  var o_perm = -1 + offset_perm; 
             var o_givcol = -1 - LDGCOL + offset_givcol; var o_givnum = -1 - LDGNUM + offset_givnum; 
             var o_poles = -1 - LDGNUM + offset_poles; var o_difl = -1 + offset_difl;  var o_difr = -1 - LDGNUM + offset_difr; 
             var o_z = -1 + offset_z; var o_work = -1 + offset_work; 
            ...
            if (ICOMPQ == 0)
            {
                ...
                for (I = 1; I <= GIVPTR; I++)
                {
                    _drot.Run(NRHS, ref B, GIVCOL[I+2 * LDGCOL + o_givcol]+1 * LDB + o_b, LDB, ref B, GIVCOL[I+1 * LDGCOL + o_givcol]+1 * LDB + o_b, LDB, GIVNUM[I+2 * LDGNUM + o_givnum]
                                   , GIVNUM[I+1 * LDGNUM + o_givnum]);
                }
                // *
                // *        Step (2L): permute rows of B.
                // *
                _dcopy.Run(NRHS, B, NLP1+1 * LDB + o_b, LDB, ref BX, 1+1 * LDBX + o_bx, LDBX);
                for (I = 2; I <= N; I++)
                {
                    _dcopy.Run(NRHS, B, PERM[I + o_perm]+1 * LDB + o_b, LDB, ref BX, I+1 * LDBX + o_bx, LDBX);
                }
                // *
                ...
                if (K == 1)
                {
                    _dcopy.Run(NRHS, BX, offset_bx, LDBX, ref B, offset_b, LDB);
                    if (Z[1 + o_z] < ZERO)
                    {
                        _dscal.Run(NRHS, NEGONE, ref B, offset_b, LDB);
                    }
                }
                else
                {
                    ...
                    for (J = 1; J <= K; J++)
                    {
                        ...
                        for (I = 1; I <= J - 1; I++)
                        {
                            ...
                            {
                                WORK[I + o_work] = POLES[I+2 * LDGNUM + o_poles] * Z[I + o_z] / (_dlamc3.Run(POLES[I+2 * LDGNUM + o_poles], DSIGJ) - DIFLJ) / (POLES[I+2 * LDGNUM + o_poles] + DJ);
                            }
                        }
                        for (I = J + 1; I <= K; I++)
                        {
                            ...
                            {
                                WORK[I + o_work] = POLES[I+2 * LDGNUM + o_poles] * Z[I + o_z] / (_dlamc3.Run(POLES[I+2 * LDGNUM + o_poles], DSIGJP) + DIFRJ) / (POLES[I+2 * LDGNUM + o_poles] + DJ);
                            }
                        }
                        WORK[1 + o_work] = NEGONE;
                        TEMP = _dnrm2.Run(K, WORK, offset_work, 1);
                        _dgemv.Run("T", K, NRHS, ONE, BX, offset_bx, LDBX
                                        , WORK, offset_work, 1, ZERO, ref B, J+1 * LDB + o_b, LDB);
                        _dlascl.Run("G", 0, 0, TEMP, ONE, 1
                ...
                if (K < Math.Max(M, N))
                {
                    _dlacpy.Run("A", N - K, NRHS, BX, K + 1+1 * LDBX + o_bx, LDBX, ref B, K + 1+1 * LDB + o_b
                                     , LDB);
                }
            }
            else
            {
                ...
                if (K == 1)
                {
                    _dcopy.Run(NRHS, B, offset_b, LDB, ref BX, offset_bx, LDBX);
                }
                else
                {
                    POLES_2 = 2 * LDGNUM + o_poles;
                    for (J = 1; J <= K; J++)
                    {
                        ...
                        for (I = 1; I <= J - 1; I++)
                        {
                            ...
                            {
                                WORK[I + o_work] = Z[J + o_z] / (_dlamc3.Run(DSIGJ,  - POLES[I + 1+2 * LDGNUM + o_poles]) - DIFR[I+1 * LDGNUM + o_difr]) / (DSIGJ + POLES[I+1 * LDGNUM + o_poles]) / DIFR[I+2 * LDGNUM + o_difr];
                            }
                        }
                        for (I = J + 1; I <= K; I++)
                        {
                            ...
                            {
                                WORK[I + o_work] = Z[J + o_z] / (_dlamc3.Run(DSIGJ,  - POLES[I+2 * LDGNUM + o_poles]) - DIFL[I + o_difl]) / (DSIGJ + POLES[I+1 * LDGNUM + o_poles]) / DIFR[I+2 * LDGNUM + o_difr];
                            }
                        }
                        _dgemv.Run("T", K, NRHS, ONE, B, offset_b, LDB
                ...
                if (SQRE == 1)
                {
                    _dcopy.Run(NRHS, B, M+1 * LDB + o_b, LDB, ref BX, M+1 * LDBX + o_bx, LDBX);
                    _drot.Run(NRHS, ref BX, 1+1 * LDBX + o_bx, LDBX, ref BX, M+1 * LDBX + o_bx, LDBX, C
                                   , S);
                }
                if (K < Math.Max(M, N))
                {
                    _dlacpy.Run("A", N - K, NRHS, B, K + 1+1 * LDB + o_b, LDB, ref BX, K + 1+1 * LDBX + o_bx
                                     , LDBX);
                }
                // *
                // *        Step (3R): permute rows of B.
                // *
                _dcopy.Run(NRHS, BX, 1+1 * LDBX + o_bx, LDBX, ref B, NLP1+1 * LDB + o_b, LDB);
                if (SQRE == 1)
                {
                    _dcopy.Run(NRHS, BX, M+1 * LDBX + o_bx, LDBX, ref B, M+1 * LDB + o_b, LDB);
                }
                for (I = 2; I <= N; I++)
                {
                    _dcopy.Run(NRHS, BX, I+1 * LDBX + o_bx, LDBX, ref B, PERM[I + o_perm]+1 * LDB + o_b, LDB);
                }
                // *
