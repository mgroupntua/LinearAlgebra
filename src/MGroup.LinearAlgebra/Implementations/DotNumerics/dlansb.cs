...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLANSB
    {
        ...
        double dlansb = 0;
            ...
            var I = 0; var J = 0; var L = 0; double ABSA = 0; double SCALE = 0; double SUM = 0; double VALUE = 0; 
            ...
            #region Array Index Correction
             ...
             var o_ab = -1 - LDAB + offset_ab;  var o_work = -1 + offset_work; 
            ...
            else
            {
                ...
                {
                    if (_lsame.Run(NORM, "I") || _lsame.Run(NORM, "O") || NORM == "1")
                    {
                        // *
                        ...
                        if (_lsame.Run(NORM, "F") || _lsame.Run(NORM, "E"))
                        {
                            ...
                            if (K > 0)
                            {
                                if (_lsame.Run(UPLO, "U"))
                                {
                                    for (J = 2; J <= N; J++)
                                    {
                                        _dlassq.Run(Math.Min(J - 1, K), AB, Math.Max(K + 2 - J, 1)+J * LDAB + o_ab, 1, ref SCALE, ref SUM);
                                    }
                                    L = K + 1;
                                }
                                else
                                {
                                    for (J = 1; J <= N - 1; J++)
                                    {
                                        _dlassq.Run(Math.Min(N - J, K), AB, 2+J * LDAB + o_ab, 1, ref SCALE, ref SUM);
                                    }
                                    L = 1;
                                }
                                SUM *= 2;
                            ...
                            _dlassq.Run(N, AB, L+1 * LDAB + o_ab, LDAB, ref SCALE, ref SUM);
