...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAQP2
    {
        ...
        public DLAQP2(DLARF dlarf, DLARFG dlarfg, DSWAP dswap, IDAMAX idamax, DLAMCH dlamch, DNRM2 dnrm2)
        {
            ...
            _dlarf = dlarf; _dlarfg = dlarfg; _dswap = dswap; _idamax = idamax; _dlamch = dlamch; 
            _dnrm2 = dnrm2;
                         ...
                         , ref double[] TAU, int offset_tau, ref double[] VN1, int offset_vn1, ref double[] VN2, int offset_vn2, ref double[] WORK, int offset_work)
        {
            ...
            var I = 0; var ITEMP = 0; var J = 0; var MN = 0; var OFFPI = 0; var PVT = 0; double AII = 0; double TEMP = 0; 
            ...
            #region Array Index Correction
             ...
             var o_vn1 = -1 + offset_vn1; var o_vn2 = -1 + offset_vn2;  var o_work = -1 + offset_work; 
            ...
            for (I = 1; I <= MN; I++)
            {
                ...
                // *
                PVT = I - 1 + _idamax.Run(N - I + 1, VN1, I + o_vn1, 1);
                // *
                if (PVT != I)
                {
                ...
                if (OFFPI < M)
                {
                    _dlarfg.Run(M - OFFPI + 1, ref A[OFFPI+I * LDA + o_a], ref A, OFFPI + 1+I * LDA + o_a, 1, ref TAU[I + o_tau]);
                }
                else
                {
                    _dlarfg.Run(1, ref A[M+I * LDA + o_a], ref A, M+I * LDA + o_a, 1, ref TAU[I + o_tau]);
                }
                // *
                if (I < N)
                {
                    ...
                    _dlarf.Run("Left", M - OFFPI + 1, N - I, A, OFFPI+I * LDA + o_a, 1, TAU[I + o_tau]
                ...
                for (J = I + 1; J <= N; J++)
                {
                    if (VN1[J + o_vn1] != ZERO)
                    {
                        ...
                        if (TEMP2 <= TOL3Z)
                        {
                            if (OFFPI < M)
                            {
                                VN1[J + o_vn1] = _dnrm2.Run(M - OFFPI, A, OFFPI + 1+J * LDA + o_a, 1);
