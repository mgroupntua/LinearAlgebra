...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAED3
    {
        ...
        public DLAED3(DLAMC3 dlamc3, DNRM2 dnrm2, DCOPY dcopy, DGEMM dgemm, DLACPY dlacpy, DLAED4 dlaed4, DLASET dlaset, XERBLA xerbla)
        {
            ...
            _dlaed4 = dlaed4;_dlaset = dlaset; _xerbla = xerbla; 
                         ...
                         , ref double[] S, int offset_s, ref int INFO)
        {
            ...
            var I = 0; var II = 0; var IQ2 = 0; var J = 0; var N12 = 0; var N2 = 0; var N23 = 0; double TEMP = 0; 
            ...
            #region Array Index Correction
             ...
             var o_d = -1 + offset_d;  var o_q = -1 - LDQ + offset_q;  var o_dlamda = -1 + offset_dlamda; 
             var o_q2 = -1 + offset_q2; var o_indx = -1 + offset_indx;  var o_ctot = -1 + offset_ctot;  var o_w = -1 + offset_w; 
             var o_s = -1 + offset_s;
            ...
            for (I = 1; I <= K; I++)
            {
                DLAMDA[I + o_dlamda] = _dlamc3.Run(DLAMDA[I + o_dlamda], DLAMDA[I + o_dlamda]) - DLAMDA[I + o_dlamda];
            }
            // *
            for (J = 1; J <= K; J++)
            {
                _dlaed4.Run(K, J, DLAMDA, offset_dlamda, W, offset_w, ref Q, 1+J * LDQ + o_q, RHO
            ...
            // *     Compute updated W.
            // *
            _dcopy.Run(K, W, offset_w, 1, ref S, offset_s, 1);
            // *
            // *     Initialize W(I) = Q(I,I)
            // *
            _dcopy.Run(K, Q, offset_q, LDQ + 1, ref W, offset_w, 1);
            for (J = 1; J <= K; J++)
            {
                Q_J = J * LDQ + o_q;
                for (I = 1; I <= J - 1; I++)
            ...
            for (J = 1; J <= K; J++)
            {
                ...
                TEMP = _dnrm2.Run(K, S, offset_s, 1);
                Q_J = J * LDQ + o_q;
                for (I = 1; I <= K; I++)
                {
                    II = INDX[I + o_indx];
        ...
        LABEL110:;
            ...
            // *
            _dlacpy.Run("A", N23, K, Q, CTOT[1 + o_ctot] + 1+1 * LDQ + o_q, LDQ, ref S, offset_s
                             , N23);
            IQ2 = N1 * N12 + 1;
            if (N23 != 0)
            {
                _dgemm.Run("N", "N", N2, K, N23, ONE
                                , Q2, IQ2 + o_q2, N2, S, offset_s, N23, ZERO, ref Q, N1 + 1+1 * LDQ + o_q
                                , LDQ);
            }
            else
            {
                _dlaset.Run("A", N2, K, ZERO, ZERO, ref Q, N1 + 1+1 * LDQ + o_q
                                 , LDQ);
            }
            // *
            _dlacpy.Run("A", N12, K, Q, offset_q, LDQ, ref S, offset_s
                             , N12);
            if (N12 != 0)
            {
                _dgemm.Run("N", "N", N1, K, N12, ONE
                                , Q2, offset_q2, N1, S, offset_s, N12, ZERO, ref Q, offset_q
                                , LDQ);
            }
            else
            {
                _dlaset.Run("A", N1, K, ZERO, ZERO, ref Q, 1+1 * LDQ + o_q
                                 , LDQ);
            }
            // *
            // *
