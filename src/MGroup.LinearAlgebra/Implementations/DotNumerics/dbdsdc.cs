...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DBDSDC
    {
                      ...
                      , DLASET dlaset, DLASR dlasr, DSWAP dswap, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _ilaenv = ilaenv; _dlamch = dlamch; _dlanst = dlanst; _dcopy = dcopy; 
            _dlartg = dlartg;_dlascl = dlascl; _dlasd0 = dlasd0; _dlasda = dlasda; _dlasdq = dlasdq; 
            _dlaset = dlaset;_dlasr = dlasr; _dswap = dswap; _xerbla = xerbla; 
                         ...
                         , ref int[] IWORK, int offset_iwork, ref int INFO)
        {
            ...
            var DIFL = 0; var DIFR = 0; var GIVCOL = 0; var GIVNUM = 0; var GIVPTR = 0; var I = 0; var IC = 0; var ICOMPQ = 0; 
            var IERR = 0;var II = 0; var IS = 0; var IU = 0; var IUPLO = 0; var IVT = 0; var J = 0; var K = 0; var KK = 0; 
            var MLVL = 0;var NM1 = 0; var NSIZE = 0; var PERM = 0; var POLES = 0; var QSTART = 0; var SMLSIZ = 0; var SMLSZP = 0; 
            var SQRE = 0;var START = 0; var WSTART = 0; var Z = 0; double CS = 0; double EPS = 0; double ORGNRM = 0; double P = 0; 
            ...
            #region Array Index Correction
             ...
             var o_q = -1 + offset_q; var o_iq = -1 + offset_iq;  var o_work = -1 + offset_work;  var o_iwork = -1 + offset_iwork; 
            ...
            // *
            IUPLO = 0;
            if (_lsame.Run(UPLO, "U")) IUPLO = 1;
            if (_lsame.Run(UPLO, "L")) IUPLO = 2;
            if (_lsame.Run(COMPQ, "N"))
            {
                ICOMPQ = 0;
            }
            else
            {
                if (_lsame.Run(COMPQ, "P"))
                {
                    ICOMPQ = 1;
                }
                else
                {
                    if (_lsame.Run(COMPQ, "I"))
                    {
                        ICOMPQ = 2;
                    }
                    else
            ...
            // *
            if (N == 0) return;
            SMLSIZ = _ilaenv.Run(9, "DBDSDC", " ", 0, 0, 0, 0);
            if (N == 1)
            {
                if (ICOMPQ == 1)
                {
            ...
            if (ICOMPQ == 1)
            {
                _dcopy.Run(N, D, offset_d, 1, ref Q, 1 + o_q, 1);
                _dcopy.Run(N - 1, E, offset_e, 1, ref Q, N + 1 + o_q, 1);
            }
            if (IUPLO == 2)
            {
                QSTART = 5;
            ...
            if (N <= SMLSIZ)
            {
                if (ICOMPQ == 2)
                {
                    _dlaset.Run("A", N, N, ZERO, ONE, ref U, offset_u
                                     , LDU);
                    _dlaset.Run("A", N, N, ZERO, ONE, ref VT, offset_vt
                                     , LDVT);
                    _dlasdq.Run("U", 0, N, N, N, 0
                                     , ref D, offset_d, ref E, offset_e, ref VT, offset_vt, LDVT, ref U, offset_u, LDU
                                     , ref U, offset_u, LDU, ref WORK, WSTART + o_work, ref INFO);
                }
                else
                {
                    if (ICOMPQ == 1)
                    {
                        IU = 1;
                        IVT = IU + N;
                        _dlaset.Run("A", N, N, ZERO, ONE, ref Q, IU + (QSTART - 1) * N + o_q
                                         , N);
                        _dlaset.Run("A", N, N, ZERO, ONE, ref Q, IVT + (QSTART - 1) * N + o_q
                                         , N);
                        _dlasdq.Run("U", 0, N, N, N, 0
            ...
            for (I = 1; I <= NM1; I++)
            {
                if (Math.Abs(E[I + o_e]) < EPS || I == NM1)
                {
                    ...
                    if (ICOMPQ == 2)
                    {
                        _dlasd0.Run(NSIZE, SQRE, ref D, START + o_d, ref E, START + o_e, ref U, START+START * LDU + o_u, LDU
                                         , ref VT, START+START * LDVT + o_vt, LDVT, SMLSIZ, ref IWORK, offset_iwork, ref WORK, WSTART + o_work, ref INFO);
                    }
                    else
                    {
                        _dlasda.Run(ICOMPQ, SMLSIZ, NSIZE, SQRE, ref D, START + o_d, ref E, START + o_e
        ...
        LABEL40:;
            ...
            for (II = 2; II <= N; II++)
            {
                ...
                if (KK != I)
                {
                    ...
                    else
                    {
                        if (ICOMPQ == 2)
                        {
                            _dswap.Run(N, ref U, 1+I * LDU + o_u, 1, ref U, 1+KK * LDU + o_u, 1);
                            _dswap.Run(N, ref VT, I+1 * LDVT + o_vt, LDVT, ref VT, KK+1 * LDVT + o_vt, LDVT);
                        }
                    }
                }
                else
            ...
            if (IUPLO == 2 && ICOMPQ == 2)
            {
                _dlasr.Run("L", "V", "B", N, N, WORK, 1 + o_work
