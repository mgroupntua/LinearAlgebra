...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DBDSQR
    {
                      ...
                      , XERBLA xerbla)
        {
            ...
            _lsame = lsame; _dlamch = dlamch; _dlartg = dlartg; _dlas2 = dlas2; _dlasq1 = dlasq1; 
            _dlasr = dlasr;_dlasv2 = dlasv2; _drot = drot; _dscal = dscal; _dswap = dswap; 
            _xerbla = xerbla;
                         ...
                         , int LDC, ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var LOWER = false; var ROTATE = false; var I = 0; var IDIR = 0; var ISUB = 0; var ITER = 0; var J = 0; var LL = 0; 
            var LLL = 0;var M = 0; var MAXIT = 0; var NM1 = 0; var NM12 = 0; var NM13 = 0; var OLDLL = 0; var OLDM = 0; 
            ...
            #region Array Index Correction
             ...
             var o_d = -1 + offset_d;  var o_e = -1 + offset_e;  var o_vt = -1 - LDVT + offset_vt;  var o_u = -1 - LDU + offset_u; 
             var o_c = -1 - LDC + offset_c; var o_work = -1 + offset_work; 
            ...
            if (!ROTATE)
            {
                _dlasq1.Run(N, ref D, offset_d, E, offset_e, ref WORK, offset_work, ref INFO);
                return;
            }
            // *
            NM1 = N - 1;
            ...
            if (LOWER)
            {
                ...
                if (NRU > 0)
                {
                    _dlasr.Run("R", "V", "F", NRU, N, WORK, 1 + o_work
                                    , WORK, N + o_work, ref U, offset_u, LDU);
                }
                if (NCC > 0)
                {
                    _dlasr.Run("L", "V", "F", N, NCC, WORK, 1 + o_work
        ...
        LABEL90:;
            ...
            if (LL == M - 1)
            {
                ...
                // *
                _dlasv2.Run(D[M - 1 + o_d], E[M - 1 + o_e], D[M + o_d], ref SIGMN, ref SIGMX, ref SINR
                ...
                if (NCVT > 0)
                {
                    _drot.Run(NCVT, ref VT, M - 1+1 * LDVT + o_vt, LDVT, ref VT, M+1 * LDVT + o_vt, LDVT, COSR
                                   , SINR);
                }
                if (NRU > 0)
                {
                    _drot.Run(NRU, ref U, 1+(M - 1) * LDU + o_u, 1, ref U, 1+M * LDU + o_u, 1, COSL
                                   , SINL);
                }
                if (NCC > 0)
                {
                    _drot.Run(NCC, ref C, M - 1+1 * LDC + o_c, LDC, ref C, M+1 * LDC + o_c, LDC, COSL
                                   , SINL);
                }
                M -= 2;
                goto LABEL60;
            ...
            else
            {
                ...
                if (IDIR == 1)
                {
                    ...
                    _dlas2.Run(D[M - 1 + o_d], E[M - 1 + o_e], D[M + o_d], ref SHIFT, ref R);
                }
                else
                {
                    SLL = Math.Abs(D[M + o_d]);
                    _dlas2.Run(D[LL + o_d], E[LL + o_e], D[LL + 1 + o_d], ref SHIFT, ref R);
                }
                // *
            ...
            if (SHIFT == ZERO)
            {
                if (IDIR == 1)
                {
                    ...
                    for (I = LL; I <= M - 1; I++)
                    {
                        _dlartg.Run(D[I + o_d] * CS, E[I + o_e], ref CS, ref SN, ref R);
                        if (I > LL) E[I - 1 + o_e] = OLDSN * R;
                        _dlartg.Run(OLDCS * R, D[I + 1 + o_d] * SN, ref OLDCS, ref OLDSN, ref D[I + o_d]);
                    ...
                    if (NCVT > 0)
                    {
                        _dlasr.Run("L", "V", "F", M - LL + 1, NCVT, WORK, 1 + o_work
                                        , WORK, N + o_work, ref VT, LL+1 * LDVT + o_vt, LDVT);
                    }
                    if (NRU > 0)
                    {
                        _dlasr.Run("R", "V", "F", NRU, M - LL + 1, WORK, NM12 + 1 + o_work
                                        , WORK, NM13 + 1 + o_work, ref U, 1+LL * LDU + o_u, LDU);
                    }
                    if (NCC > 0)
                    {
                        _dlasr.Run("L", "V", "F", M - LL + 1, NCC, WORK, NM12 + 1 + o_work
                ...
                else
                {
                    ...
                    for (I = M; I >= LL + 1; I +=  - 1)
                    {
                        _dlartg.Run(D[I + o_d] * CS, E[I - 1 + o_e], ref CS, ref SN, ref R);
                    ...
                    if (NCVT > 0)
                    {
                        _dlasr.Run("L", "V", "B", M - LL + 1, NCVT, WORK, NM12 + 1 + o_work
                                        , WORK, NM13 + 1 + o_work, ref VT, LL+1 * LDVT + o_vt, LDVT);
                    }
                    if (NRU > 0)
                    {
                        _dlasr.Run("R", "V", "B", NRU, M - LL + 1, WORK, 1 + o_work
                                        , WORK, N + o_work, ref U, 1+LL * LDU + o_u, LDU);
                    }
                    if (NCC > 0)
                    {
                        _dlasr.Run("L", "V", "B", M - LL + 1, NCC, WORK, 1 + o_work
            ...
            else
            {
                ...
                if (IDIR == 1)
                {
                    ...
                    for (I = LL; I <= M - 1; I++)
                    {
                        _dlartg.Run(F, G, ref COSR, ref SINR, ref R);
                        if (I > LL) E[I - 1 + o_e] = R;
                        F = COSR * D[I + o_d] + SINR * E[I + o_e];
                        E[I + o_e] = COSR * E[I + o_e] - SINR * D[I + o_d];
                        G = SINR * D[I + 1 + o_d];
                        D[I + 1 + o_d] *= COSR;
                        _dlartg.Run(F, G, ref COSL, ref SINL, ref R);
                        D[I + o_d] = R;
        ...
        LABEL160:;
            for (I = 1; I <= N; I++)
            {
                if (D[I + o_d] < ZERO)
                {
                    ...
                    // *
                    if (NCVT > 0) _dscal.Run(NCVT, NEGONE, ref VT, I+1 * LDVT + o_vt, LDVT);
                }
            }
            // *
            ...
            for (I = 1; I <= N - 1; I++)
            {
                ...
                if (ISUB != N + 1 - I)
                {
                    ...
                    D[N + 1 - I + o_d] = SMIN;
                    if (NCVT > 0) _dswap.Run(NCVT, ref VT, ISUB+1 * LDVT + o_vt, LDVT, ref VT, N + 1 - I+1 * LDVT + o_vt, LDVT);
                    if (NRU > 0) _dswap.Run(NRU, ref U, 1+ISUB * LDU + o_u, 1, ref U, 1+(N + 1 - I) * LDU + o_u, 1);
                    if (NCC > 0) _dswap.Run(NCC, ref C, ISUB+1 * LDC + o_c, LDC, ref C, N + 1 - I+1 * LDC + o_c, LDC);
                }
            }
            goto LABEL220;
            // *
