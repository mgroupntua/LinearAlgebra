...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLASCL
    {
                         ...
                         , int N, ref double[] A, int offset_a, int LDA, ref int INFO)
        {
            ...
            var DONE = false; var I = 0; var ITYPE = 0; var J = 0; var K1 = 0; var K2 = 0; var K3 = 0; var K4 = 0; 
            ...
            INFO = 0;
            // *
            if (_lsame.Run(TYPE, "G"))
            {
                ITYPE = 0;
            }
            else
            {
                if (_lsame.Run(TYPE, "L"))
                {
                    ITYPE = 1;
                }
                else
                {
                    if (_lsame.Run(TYPE, "U"))
                    {
                        ITYPE = 2;
                    }
                    else
                    {
                        if (_lsame.Run(TYPE, "H"))
                        {
                            ITYPE = 3;
                        }
                        else
                        {
                            if (_lsame.Run(TYPE, "B"))
                            {
                                ITYPE = 4;
                            }
                            else
                            {
                                if (_lsame.Run(TYPE, "Q"))
                                {
                                    ITYPE = 5;
                                }
                                else
                                {
                                    if (_lsame.Run(TYPE, "Z"))
                                    {
                                        ITYPE = 6;
                                    }
                                    else
