...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DSYTRD
    {
        ...
        public DSYTRD(DLATRD dlatrd, DSYR2K dsyr2k, DSYTD2 dsytd2, XERBLA xerbla, LSAME lsame, ILAENV ilaenv)
        {
            ...
            _dlatrd = dlatrd; _dsyr2k = dsyr2k; _dsytd2 = dsytd2; _xerbla = xerbla; _lsame = lsame; 
            _ilaenv = ilaenv;
                         ...
                         , ref double[] TAU, int offset_tau, ref double[] WORK, int offset_work, int LWORK, ref int INFO)
        {
            ...
            var LQUERY = false; var UPPER = false; var I = 0; var IINFO = 0; var IWS = 0; var J = 0; var KK = 0; var LDWORK = 0; 
            var LWKOPT = 0;var NB = 0; var NBMIN = 0; var NX = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_d = -1 + offset_d;  var o_e = -1 + offset_e;  var o_tau = -1 + offset_tau; 
            ...
            if (NB > 1 && NB < N)
            {
                ...
                // *
                NX = Math.Max(NB, _ilaenv.Run(3, "DSYTRD", UPLO, N,  - 1,  - 1,  - 1));
                if (NX < N)
                {
                    // *
                    ...
                    if (LWORK < IWS)
                    {
                        ...
                        NBMIN = _ilaenv.Run(2, "DSYTRD", UPLO, N,  - 1,  - 1,  - 1);
                        if (NB < NBMIN) NX = N;
                    }
                }
                else
            ...
            if (UPPER)
            {
                ...
                for (I = N - NB + 1;  - NB >= 0 ? I <= KK + 1 : I >= KK + 1; I +=  - NB)
                {
                    ...
                    // *           the matrix
                    // *
                    _dlatrd.Run(UPLO, I + NB - 1, NB, ref A, offset_a, LDA, ref E, offset_e
                                     , ref TAU, offset_tau, ref WORK, offset_work, LDWORK);
                    // *
                    // *           Update the unreduced submatrix A(1:i-1,1:i-1), using an
                    // *           update of the form:  A := A - V*W' - W*V'
                    // *
                    _dsyr2k.Run(UPLO, "No transpose", I - 1, NB,  - ONE, A, 1+I * LDA + o_a
                ...
                // *
                _dsytd2.Run(UPLO, KK, ref A, offset_a, LDA, ref D, offset_d, ref E, offset_e
            ...
            else
            {
                ...
                for (I = 1; NB >= 0 ? I <= N - NX : I >= N - NX; I += NB)
                {
                    ...
                    // *           the matrix
                    // *
                    _dlatrd.Run(UPLO, N - I + 1, NB, ref A, I+I * LDA + o_a, LDA, ref E, I + o_e
                                     , ref TAU, I + o_tau, ref WORK, offset_work, LDWORK);
                    // *
                    // *           Update the unreduced submatrix A(i+ib:n,i+ib:n), using
                    // *           an update of the form:  A := A - V*W' - W*V'
                    // *
                    _dsyr2k.Run(UPLO, "No transpose", N - I - NB + 1, NB,  - ONE, A, I + NB+I * LDA + o_a
                ...
                // *        Use unblocked code to reduce the last or only block
                // *
                _dsytd2.Run(UPLO, N - I + 1, ref A, I+I * LDA + o_a, LDA, ref D, I + o_d, ref E, I + o_e
