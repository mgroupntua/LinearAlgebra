...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DTRTRI
    {
        ...
        public DTRTRI(LSAME lsame, ILAENV ilaenv, DTRMM dtrmm, DTRSM dtrsm, DTRTI2 dtrti2, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _ilaenv = ilaenv; _dtrmm = dtrmm; _dtrsm = dtrsm; _dtrti2 = dtrti2; 
            _xerbla = xerbla;
        ...
        public void Run(string UPLO, string DIAG, int N, ref double[] A, int offset_a, int LDA, ref int INFO)
        {
            ...
            var NOUNIT = false; var UPPER = false; var J = 0; var JB = 0; var NB = 0; var NN = 0; 
            ...
            // *
            NB = _ilaenv.Run(1, "DTRTRI", UPLO + DIAG, N,  - 1,  - 1,  - 1);
            if (NB <= 1 || NB >= N)
            {
                // *
                // *        Use unblocked code
                // *
                _dtrti2.Run(UPLO, DIAG, N, ref A, offset_a, LDA, ref INFO);
            }
            else
            {
                // *
                ...
                if (UPPER)
                {
                    ...
                    for (J = 1; NB >= 0 ? J <= N : J >= N; J += NB)
                    {
                        ...
                        // *
                        _dtrmm.Run("Left", "Upper", "No transpose", DIAG, J - 1, JB
                                        , ONE, A, offset_a, LDA, ref A, 1+J * LDA + o_a, LDA);
                        _dtrsm.Run("Right", "Upper", "No transpose", DIAG, J - 1, JB
                                        ,  - ONE, A, J+J * LDA + o_a, LDA, ref A, 1+J * LDA + o_a, LDA);
                        // *
                        // *              Compute inverse of current diagonal block
                        // *
                        _dtrti2.Run("Upper", DIAG, JB, ref A, J+J * LDA + o_a, LDA, ref INFO);
                    }
                }
                else
                {
                    ...
                    for (J = NN;  - NB >= 0 ? J <= 1 : J >= 1; J +=  - NB)
                    {
                        ...
                        if (J + JB <= N)
                        {
                            ...
                            // *
                            _dtrmm.Run("Left", "Lower", "No transpose", DIAG, N - J - JB + 1, JB
                                            , ONE, A, J + JB+(J + JB) * LDA + o_a, LDA, ref A, J + JB+J * LDA + o_a, LDA);
                            _dtrsm.Run("Right", "Lower", "No transpose", DIAG, N - J - JB + 1, JB
                                            ,  - ONE, A, J+J * LDA + o_a, LDA, ref A, J + JB+J * LDA + o_a, LDA);
                        }
                        // *
                        // *              Compute inverse of current diagonal block
                        // *
                        _dtrti2.Run("Lower", DIAG, JB, ref A, J+J * LDA + o_a, LDA, ref INFO);
                    }
                }
            }
            // *
