...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAPLL
    {
        ...
        public DLAPLL(DDOT ddot, DAXPY daxpy, DLARFG dlarfg, DLAS2 dlas2)
        {
            ...
            _ddot = ddot; _daxpy = daxpy; _dlarfg = dlarfg; _dlas2 = dlas2; 
        ...
        public void Run(int N, ref double[] X, int offset_x, int INCX, ref double[] Y, int offset_y, int INCY, ref double SSMIN)
        {
            ...
            #region Array Index Correction
             ...
             var o_x = -1 + offset_x;  var o_y = -1 + offset_y; 
            ...
            // *
            _dlarfg.Run(N, ref X[1 + o_x], ref X, 1 + INCX + o_x, INCX, ref TAU);
            A11 = X[1 + o_x];
            X[1 + o_x] = ONE;
            // *
            C =  - TAU * _ddot.Run(N, X, offset_x, INCX, Y, offset_y, INCY);
            _daxpy.Run(N, C, X, offset_x, INCX, ref Y, offset_y, INCY);
            // *
            _dlarfg.Run(N - 1, ref Y[1 + INCY + o_y], ref Y, 1 + 2 * INCY + o_y, INCY, ref TAU);
            // *
            A12 = Y[1 + o_y];
            A22 = Y[1 + INCY + o_y];
            // *
            ...
            // *
            _dlas2.Run(A11, A12, A22, ref SSMIN, ref SSMAX);
            // *
            return;
            // *
            // *     End of DLAPLL
