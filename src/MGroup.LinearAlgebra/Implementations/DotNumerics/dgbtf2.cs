...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGBTF2
    {
        ...
        public DGBTF2(IDAMAX idamax, DGER dger, DSCAL dscal, DSWAP dswap, XERBLA xerbla)
        {
            ...
            _idamax = idamax; _dger = dger; _dscal = dscal; _dswap = dswap; _xerbla = xerbla; 
                         ...
                         , ref int[] IPIV, int offset_ipiv, ref int INFO)
        {
            ...
            var I = 0; var J = 0; var JP = 0; var JU = 0; var KM = 0; var KV = 0; 
            ...
            for (J = 1; J <= Math.Min(M, N); J++)
            {
                ...
                // *
                KM = Math.Min(KL, M - J);
                JP = _idamax.Run(KM + 1, AB, KV + 1+J * LDAB + o_ab, 1);
                ...
                if (AB[KV + JP+J * LDAB + o_ab] != ZERO)
                {
                    ...
                    // *
                    if (JP != 1) _dswap.Run(JU - J + 1, ref AB, KV + JP+J * LDAB + o_ab, LDAB - 1, ref AB, KV + 1+J * LDAB + o_ab, LDAB - 1);
                    // *
                    if (KM > 0)
                    {
                        // *
                        // *              Compute multipliers.
                        // *
                        _dscal.Run(KM, ONE / AB[KV + 1+J * LDAB + o_ab], ref AB, KV + 2+J * LDAB + o_ab, 1);
                        // *
                        // *              Update trailing submatrix within the band.
                        // *
                        if (JU > J)
                        {
                            _dger.Run(KM, JU - J,  - ONE, AB, KV + 2+J * LDAB + o_ab, 1, AB, KV+(J + 1) * LDAB + o_ab
