...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGEBAL
    {
        ...
        public DGEBAL(LSAME lsame, IDAMAX idamax, DLAMCH dlamch, DSCAL dscal, DSWAP dswap, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _idamax = idamax; _dlamch = dlamch; _dscal = dscal; _dswap = dswap; 
            _xerbla = xerbla;
                         ...
                         , ref double[] SCALE, int offset_scale, ref int INFO)
        {
            ...
            var NOCONV = false; var I = 0; var ICA = 0; var IEXC = 0; var IRA = 0; var J = 0; var K = 0; var L = 0; var M = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_scale = -1 + offset_scale; 
            ...
            INFO = 0;
            if (!_lsame.Run(JOB, "N") && !_lsame.Run(JOB, "P") && !_lsame.Run(JOB, "S") && !_lsame.Run(JOB, "B"))
            {
                INFO =  - 1;
            }
            else
            ...
            // *
            if (_lsame.Run(JOB, "S")) goto LABEL120;
            // *
        ...
        LABEL20:;
            ...
            if (J == M) goto LABEL30;
            // *
            _dswap.Run(L, ref A, 1+J * LDA + o_a, 1, ref A, 1+M * LDA + o_a, 1);
            _dswap.Run(N - K + 1, ref A, J+K * LDA + o_a, LDA, ref A, M+K * LDA + o_a, LDA);
            // *
        LABEL30:;
            switch (IEXC)
            {
        ...
        LABEL120:;
            ...
            // *
            if (_lsame.Run(JOB, "P")) goto LABEL210;
            // *
            // *     Balance the submatrix in rows K to L.
            // *
            // *     Iterative loop for norm reduction
            // *
            SFMIN1 = _dlamch.Run("S") / _dlamch.Run("P");
            SFMAX1 = ONE / SFMIN1;
        ...
        LABEL140:;
            ...
            for (I = K; I <= L; I++)
            {
                ...
                ICA = _idamax.Run(L, A, 1+I * LDA + o_a, 1);
                CA = Math.Abs(A[ICA+I * LDA + o_a]);
                IRA = _idamax.Run(N - K + 1, A, I+K * LDA + o_a, LDA);
            ...
            LABEL190:;
                ...
                NOCONV = true;
                // *
                _dscal.Run(N - K + 1, G, ref A, I+K * LDA + o_a, LDA);
                _dscal.Run(L, F, ref A, 1+I * LDA + o_a, 1);
                // *
            LABEL200:;
            }
            // *
