...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGGGLM
    {
        ...
        public DGGGLM(DCOPY dcopy, DGEMV dgemv, DGGQRF dggqrf, DORMQR dormqr, DORMRQ dormrq, DTRTRS dtrtrs, XERBLA xerbla, ILAENV ilaenv)
        {
            ...
            _dcopy = dcopy; _dgemv = dgemv; _dggqrf = dggqrf; _dormqr = dormqr; _dormrq = dormrq; 
            _dtrtrs = dtrtrs;_xerbla = xerbla; _ilaenv = ilaenv; 
        ...
        public DGGGLM()
        {
            ...
            var dggqrf = new DGGQRF(dgeqrf, dgerqf, dormqr, xerbla, ilaenv);
                         ...
                         , ref int INFO)
        {
            ...
            var LQUERY = false; var I = 0; var LOPT = 0; var LWKMIN = 0; var LWKOPT = 0; var NB = 0; var NB1 = 0; var NB2 = 0; 
            var NB3 = 0;var NB4 = 0; var NP = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_b = -1 - LDB + offset_b;  var o_d = -1 + offset_d;  var o_x = -1 + offset_x; 
             var o_y = -1 + offset_y; var o_work = -1 + offset_work; 
            ...
            if (INFO == 0)
            {
                ...
                {
                    NB1 = _ilaenv.Run(1, "DGEQRF", " ", N, M,  - 1,  - 1);
                    NB2 = _ilaenv.Run(1, "DGERQF", " ", N, M,  - 1,  - 1);
                    NB3 = _ilaenv.Run(1, "DORMQR", " ", N, M, P,  - 1);
                    NB4 = _ilaenv.Run(1, "DORMRQ", " ", N, M, P,  - 1);
            ...
            // *     orthogonal.
            // *
            _dggqrf.Run(N, M, P, ref A, offset_a, LDA, ref WORK, offset_work
            ...
            // *                                             ( d2 ) N-M
            // *
            _dormqr.Run("Left", "Transpose", N, 1, M, ref A, offset_a
            ...
            if (N > M)
            {
                _dtrtrs.Run("Upper", "No transpose", "Non unit", N - M, 1, B, M + 1+(M + P - N + 1) * LDB + o_b
                ...
                // *
                _dcopy.Run(N - M, D, M + 1 + o_d, 1, ref Y, M + P - N + 1 + o_y, 1);
            }
            // *
            // *     Set y1 = 0
            // *
            ...
            // *     Update d1 = d1 - T12*y2
            // *
            _dgemv.Run("No transpose", M, N - M,  - ONE, B, 1+(M + P - N + 1) * LDB + o_b, LDB
            ...
            if (M > 0)
            {
                _dtrtrs.Run("Upper", "No Transpose", "Non unit", M, 1, A, offset_a
                ...
                // *        Copy D to X
                // *
                _dcopy.Run(M, D, offset_d, 1, ref X, offset_x, 1);
            }
            // *
            // *     Backward transformation y = Z'*y
            // *
            _dormrq.Run("Left", "Transpose", P, 1, NP, ref B, Math.Max(1, N - P + 1)+1 * LDB + o_b
