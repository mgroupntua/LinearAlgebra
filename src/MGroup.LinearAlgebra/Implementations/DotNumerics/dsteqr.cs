...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DSTEQR
    {
                      ...
                      , DLASRT dlasrt, DSWAP dswap, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _dlamch = dlamch; _dlanst = dlanst; _dlapy2 = dlapy2; _dlae2 = dlae2; 
            _dlaev2 = dlaev2;_dlartg = dlartg; _dlascl = dlascl; _dlaset = dlaset; _dlasr = dlasr; 
            _dlasrt = dlasrt;_dswap = dswap; _xerbla = xerbla; 
                         ...
                         , ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var I = 0; var ICOMPZ = 0; var II = 0; var ISCALE = 0; var J = 0; var JTOT = 0; var K = 0; var L = 0; var L1 = 0; 
            var LEND = 0;var LENDM1 = 0; var LENDP1 = 0; var LENDSV = 0; var LM1 = 0; var LSV = 0; var M = 0; var MM = 0; 
            var MM1 = 0;var NM1 = 0; var NMAXIT = 0; double ANORM = 0; double B = 0; double C = 0; double EPS = 0; 
        ...
        LABEL30:;
            ...
            LABEL60:;
                ...
                if (M == L + 1)
                {
                    if (ICOMPZ > 0)
                    {
                        _dlaev2.Run(D[L + o_d], E[L + o_e], D[L + 1 + o_d], ref RT1, ref RT2, ref C
                                         , ref S);
                        WORK[L + o_work] = C;
                        WORK[N - 1 + L + o_work] = S;
                        _dlasr.Run("R", "V", "B", N, 2, WORK, L + o_work
                                        , WORK, N - 1 + L + o_work, ref Z, 1+L * LDZ + o_z, LDZ);
                    }
                    else
                    {
                        _dlae2.Run(D[L + o_d], E[L + o_e], D[L + 1 + o_d], ref RT1, ref RT2);
                    }
                    D[L + o_d] = RT1;
                    D[L + 1 + o_d] = RT2;
                    E[L + o_e] = ZERO;
                ...
                for (I = MM1; I >= L; I +=  - 1)
                {
                    F = S * E[I + o_e];
                    B = C * E[I + o_e];
                    _dlartg.Run(G, F, ref C, ref S, ref R);
                ...
                {
                    MM = M - L + 1;
                    _dlasr.Run("R", "V", "B", N, MM, WORK, L + o_work
                                    , WORK, N - 1 + L + o_work, ref Z, 1+L * LDZ + o_z, LDZ);
                }
                // *
                D[L + o_d] -= P;
            ...
            LABEL110:;
                ...
                if (M == L - 1)
                {
                    if (ICOMPZ > 0)
                    {
                        _dlaev2.Run(D[L - 1 + o_d], E[L - 1 + o_e], D[L + o_d], ref RT1, ref RT2, ref C
                                         , ref S);
                        WORK[M + o_work] = C;
                        WORK[N - 1 + M + o_work] = S;
                        _dlasr.Run("R", "V", "F", N, 2, WORK, M + o_work
                                        , WORK, N - 1 + M + o_work, ref Z, 1+(L - 1) * LDZ + o_z, LDZ);
                    }
                    else
                    {
                        _dlae2.Run(D[L - 1 + o_d], E[L - 1 + o_e], D[L + o_d], ref RT1, ref RT2);
                    }
                    D[L - 1 + o_d] = RT1;
                    D[L + o_d] = RT2;
                    E[L - 1 + o_e] = ZERO;
                ...
                {
                    MM = L - M + 1;
                    _dlasr.Run("R", "V", "F", N, MM, WORK, M + o_work
        ...
        LABEL140:;
            if (ISCALE == 1)
            {
                ...
                _dlascl.Run("G", 0, 0, SSFMAX, ANORM, LENDSV - LSV
            ...
            else
            {
                if (ISCALE == 2)
                {
                    _dlascl.Run("G", 0, 0, SSFMIN, ANORM, LENDSV - LSV + 1
                                     , 1, ref D, LSV + o_d, N, ref INFO);
                    _dlascl.Run("G", 0, 0, SSFMIN, ANORM, LENDSV - LSV
                                     , 1, ref E, LSV + o_e, N, ref INFO);
                }
            }
            // *
