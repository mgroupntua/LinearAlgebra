...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DSYEVD
    {
                      ...
                      , DSYTRD dsytrd, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _dlamch = dlamch; _dlansy = dlansy; _ilaenv = ilaenv; _dlacpy = dlacpy; 
            _dlascl = dlascl;_dormtr = dormtr; _dscal = dscal; _dstedc = dstedc; _dsterf = dsterf; 
            _dsytrd = dsytrd;_xerbla = xerbla; 
        ...
        public DSYEVD()
        {
            ...
            var dlansy = new DLANSY(dlassq, lsame);
            ...
            var dorm2l = new DORM2L(lsame, dlarf, xerbla);
            var dormql = new DORMQL(lsame, ilaenv, dlarfb, dlarft, dorm2l, xerbla);
            var dorm2r = new DORM2R(lsame, dlarf, xerbla);
            var dormqr = new DORMQR(lsame, ilaenv, dlarfb, dlarft, dorm2r, xerbla);
            var dormtr = new DORMTR(lsame, ilaenv, dormql, dormqr, xerbla);
            ...
            var dsteqr = new DSTEQR(lsame, dlamch, dlanst, dlapy2, dlae2, dlaev2, dlartg, dlascl, dlaset, dlasr
                                       , dlasrt, dswap, xerbla);
            var dlaed0 = new DLAED0(dcopy, dgemm, dlacpy, dlaed1, dlaed7, dsteqr, xerbla, ilaenv);
            var dsterf = new DSTERF(dlamch, dlanst, dlapy2, dlae2, dlascl, dlasrt, xerbla);
            var dstedc = new DSTEDC(lsame, ilaenv, dlamch, dlanst, dgemm, dlacpy, dlaed0, dlascl, dlaset, dlasrt
                                       , dsteqr, dsterf, dswap, xerbla);
            var dlarfg = new DLARFG(dlamch, dlapy2, dnrm2, dscal);
            var dsymv = new DSYMV(lsame, xerbla);
            var dlatrd = new DLATRD(daxpy, dgemv, dlarfg, dscal, dsymv, lsame, ddot);
            var dsyr2k = new DSYR2K(lsame, xerbla);
            var dsyr2 = new DSYR2(lsame, xerbla);
            var dsytd2 = new DSYTD2(daxpy, dlarfg, dsymv, dsyr2, xerbla, lsame, ddot);
            var dsytrd = new DSYTRD(dlatrd, dsyr2k, dsytd2, xerbla, lsame, ilaenv);
                         ...
                         , ref double[] WORK, int offset_work, int LWORK, ref int[] IWORK, int offset_iwork, int LIWORK, ref int INFO)
        {
            ...
            var LOWER = false; var LQUERY = false; var WANTZ = false; var IINFO = 0; var INDE = 0; var INDTAU = 0; 
            var INDWK2 = 0;var INDWRK = 0; var ISCALE = 0; var LIOPT = 0; var LIWMIN = 0; var LLWORK = 0; var LLWRK2 = 0; 
            var LOPT = 0;var LWMIN = 0; double ANRM = 0; double BIGNUM = 0; double EPS = 0; double RMAX = 0; double RMIN = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_w = -1 + offset_w;  var o_work = -1 + offset_work; 
            ...
            if (INFO == 0)
            {
                ...
                else
                {
                    ...
                    LOPT = Math.Max(LWMIN, 2 * N + _ilaenv.Run(1, "DSYTRD", UPLO, N,  - 1,  - 1,  - 1));
                    LIOPT = LIWMIN;
                }
                WORK[1 + o_work] = LOPT;
            ...
            // *
            ANRM = _dlansy.Run("M", UPLO, N, A, offset_a, LDA, ref WORK, offset_work);
            ISCALE = 0;
            ...
            if (ISCALE == 1)
            {
                _dlascl.Run(UPLO, 0, 0, ONE, SIGMA, N
            ...
            // *
            _dsytrd.Run(UPLO, N, ref A, offset_a, LDA, ref W, offset_w, ref WORK, INDE + o_work
            ...
            {
                _dstedc.Run("I", N, ref W, offset_w, ref WORK, INDE + o_work, ref WORK, INDWRK + o_work, N
                                 , ref WORK, INDWK2 + o_work, LLWRK2, ref IWORK, offset_iwork, LIWORK, ref INFO);
                _dormtr.Run("L", UPLO, "N", N, N, ref A, offset_a
                                 , LDA, WORK, INDTAU + o_work, ref WORK, INDWRK + o_work, N, ref WORK, INDWK2 + o_work, LLWRK2
                                 , ref IINFO);
                _dlacpy.Run("A", N, N, WORK, INDWRK + o_work, N, ref A, offset_a
                                 , LDA);
            ...
            // *
            if (ISCALE == 1) _dscal.Run(N, ONE / SIGMA, ref W, offset_w, 1);
            // *
            WORK[1 + o_work] = LOPT;
            IWORK[1 + o_iwork] = LIOPT;
            // *
