...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DSYTD2
    {
        ...
        public DSYTD2(DAXPY daxpy, DLARFG dlarfg, DSYMV dsymv, DSYR2 dsyr2, XERBLA xerbla, LSAME lsame, DDOT ddot)
        {
            ...
            _daxpy = daxpy; _dlarfg = dlarfg; _dsymv = dsymv; _dsyr2 = dsyr2; _xerbla = xerbla; 
                         ...
                         , ref double[] TAU, int offset_tau, ref int INFO)
        {
            ...
            var UPPER = false; var I = 0; double ALPHA = 0; double TAUI = 0; 
            ...
            if (UPPER)
            {
                ...
                for (I = N - 1; I >= 1; I +=  - 1)
                {
                    ...
                    // *
                    _dlarfg.Run(I, ref A[I+(I + 1) * LDA + o_a], ref A, 1+(I + 1) * LDA + o_a, 1, ref TAUI);
                    ...
                    if (TAUI != ZERO)
                    {
                        ...
                        // *
                        _dsymv.Run(UPLO, I, TAUI, A, offset_a, LDA, A, 1+(I + 1) * LDA + o_a
                                        , 1, ZERO, ref TAU, offset_tau, 1);
                        // *
                        // *              Compute  w := x - 1/2 * tau * (x'*v) * v
                        // *
                        ALPHA =  - HALF * TAUI * _ddot.Run(I, TAU, offset_tau, 1, A, 1+(I + 1) * LDA + o_a, 1);
                        _daxpy.Run(I, ALPHA, A, 1+(I + 1) * LDA + o_a, 1, ref TAU, offset_tau, 1);
                        // *
                        // *              Apply the transformation as a rank-2 update:
                        // *                 A := A - v * w' - w * v'
                        // *
                        _dsyr2.Run(UPLO, I,  - ONE, A, 1+(I + 1) * LDA + o_a, 1, TAU, offset_tau
            ...
            else
            {
                ...
                for (I = 1; I <= N - 1; I++)
                {
                    ...
                    // *
                    _dlarfg.Run(N - I, ref A[I + 1+I * LDA + o_a], ref A, Math.Min(I + 2, N)+I * LDA + o_a, 1, ref TAUI);
                    ...
                    if (TAUI != ZERO)
                    {
                        ...
                        // *
                        _dsymv.Run(UPLO, N - I, TAUI, A, I + 1+(I + 1) * LDA + o_a, LDA, A, I + 1+I * LDA + o_a
                                        , 1, ZERO, ref TAU, I + o_tau, 1);
                        // *
                        // *              Compute  w := x - 1/2 * tau * (x'*v) * v
                        // *
                        ALPHA =  - HALF * TAUI * _ddot.Run(N - I, TAU, I + o_tau, 1, A, I + 1+I * LDA + o_a, 1);
                        _daxpy.Run(N - I, ALPHA, A, I + 1+I * LDA + o_a, 1, ref TAU, I + o_tau, 1);
                        // *
                        // *              Apply the transformation as a rank-2 update:
                        // *                 A := A - v * w' - w * v'
                        // *
                        _dsyr2.Run(UPLO, N - I,  - ONE, A, I + 1+I * LDA + o_a, 1, TAU, I + o_tau
