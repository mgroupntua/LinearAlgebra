...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DGGSVP
    {
                      ...
                      , XERBLA xerbla)
        {
            ...
            _lsame = lsame; _dgeqpf = dgeqpf; _dgeqr2 = dgeqr2; _dgerq2 = dgerq2; _dlacpy = dlacpy; 
            _dlapmt = dlapmt;_dlaset = dlaset; _dorg2r = dorg2r; _dorm2r = dorm2r; _dormr2 = dormr2; 
            _xerbla = xerbla;
        ...
        public DGGSVP()
        {
            ...
            var dgeqpf = new DGEQPF(dgeqr2, dlarf, dlarfg, dorm2r, dswap, xerbla, idamax, dlamch, dnrm2);
            ...
            var dormr2 = new DORMR2(lsame, dlarf, xerbla);
                         ...
                         , ref double[] Q, int offset_q, int LDQ, ref int[] IWORK, int offset_iwork, ref double[] TAU, int offset_tau, ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var FORWRD = false; var WANTQ = false; var WANTU = false; var WANTV = false; var I = 0; var J = 0; 
            ...
            #region Array Index Correction
             ...
             var o_a = -1 - LDA + offset_a;  var o_b = -1 - LDB + offset_b;  var o_u = -1 - LDU + offset_u; 
             var o_v = -1 - LDV + offset_v; var o_q = -1 - LDQ + offset_q;  var o_iwork = -1 + offset_iwork; 
             var o_tau = -1 + offset_tau; var o_work = -1 + offset_work; 
            ...
            WANTU = _lsame.Run(JOBU, "U");
            WANTV = _lsame.Run(JOBV, "V");
            WANTQ = _lsame.Run(JOBQ, "Q");
            FORWRD = true;
            // *
            INFO = 0;
            if (!(WANTU || _lsame.Run(JOBU, "N")))
            {
                INFO =  - 1;
            }
            else
            {
                if (!(WANTV || _lsame.Run(JOBV, "N")))
                {
                    INFO =  - 2;
                }
                else
                {
                    if (!(WANTQ || _lsame.Run(JOBQ, "N")))
                    {
                        INFO =  - 3;
                    }
                    else
            ...
            _dgeqpf.Run(P, N, ref B, offset_b, LDB, ref IWORK, offset_iwork, ref TAU, offset_tau
                             , ref WORK, offset_work, ref INFO);
            // *
            // *     Update A := A*P
            // *
            _dlapmt.Run(FORWRD, M, N, ref A, offset_a, LDA, ref IWORK, offset_iwork);
            // *
            ...
            if (WANTV)
            {
                ...
                // *
                _dlaset.Run("Full", P, P, ZERO, ZERO, ref V, offset_v
                                 , LDV);
                if (P > 1)
                {
                    _dlacpy.Run("Lower", P - 1, N, B, 2+1 * LDB + o_b, LDB, ref V, 2+1 * LDV + o_v
                                     , LDV);
                }
                _dorg2r.Run(P, P, Math.Min(P, N), ref V, offset_v, LDV, TAU, offset_tau
                                 , ref WORK, offset_work, ref INFO);
            }
            // *
            // *     Clean up B
            ...
            if (P > L)
            {
                _dlaset.Run("Full", P - L, N, ZERO, ZERO, ref B, L + 1+1 * LDB + o_b
                                 , LDB);
            }
            // *
            if (WANTQ)
            {
                ...
                _dlaset.Run("Full", N, N, ZERO, ONE, ref Q, offset_q
                                 , LDQ);
                _dlapmt.Run(FORWRD, N, N, ref Q, offset_q, LDQ, ref IWORK, offset_iwork);
            }
            // *
            if (P >= L && N != L)
            {
                ...
                // *
                _dgerq2.Run(L, N, ref B, offset_b, LDB, ref TAU, offset_tau, ref WORK, offset_work
                                 , ref INFO);
                // *
                // *        Update A := A*Z'
                // *
                _dormr2.Run("Right", "Transpose", M, N, L, ref B, offset_b
                ...
                if (WANTQ)
                {
                    ...
                    // *           Update Q := Q*Z'
                    // *
                    _dormr2.Run("Right", "Transpose", N, N, L, ref B, offset_b
                                     , LDB, TAU, offset_tau, ref Q, offset_q, LDQ, ref WORK, offset_work, ref INFO);
                }
                // *
                // *        Clean up B
                // *
                _dlaset.Run("Full", L, N - L, ZERO, ZERO, ref B, offset_b
                                 , LDB);
                for (J = N - L + 1; J <= N; J++)
                {
                    B_J = J * LDB + o_b;
            ...
            _dgeqpf.Run(M, N - L, ref A, offset_a, LDA, ref IWORK, offset_iwork, ref TAU, offset_tau
                             , ref WORK, offset_work, ref INFO);
            // *
            ...
            // *
            _dorm2r.Run("Left", "Transpose", M, L, Math.Min(M, N - L), ref A, offset_a
            ...
            if (WANTU)
            {
                ...
                // *
                _dlaset.Run("Full", M, M, ZERO, ZERO, ref U, offset_u
                                 , LDU);
                if (M > 1)
                {
                    _dlacpy.Run("Lower", M - 1, N - L, A, 2+1 * LDA + o_a, LDA, ref U, 2+1 * LDU + o_u
                                     , LDU);
                }
                _dorg2r.Run(M, M, Math.Min(M, N - L), ref U, offset_u, LDU, TAU, offset_tau
                                 , ref WORK, offset_work, ref INFO);
            }
            // *
            if (WANTQ)
            {
                ...
                // *
                _dlapmt.Run(FORWRD, N, N - L, ref Q, offset_q, LDQ, ref IWORK, offset_iwork);
            }
            // *
            ...
            if (M > K)
            {
                _dlaset.Run("Full", M - K, N - L, ZERO, ZERO, ref A, K + 1+1 * LDA + o_a
                                 , LDA);
            }
            // *
            if (N - L > K)
            {
                ...
                // *
                _dgerq2.Run(K, N - L, ref A, offset_a, LDA, ref TAU, offset_tau, ref WORK, offset_work
                                 , ref INFO);
                // *
                if (WANTQ)
                {
                    ...
                    // *
                    _dormr2.Run("Right", "Transpose", N, N - L, K, ref A, offset_a
                                     , LDA, TAU, offset_tau, ref Q, offset_q, LDQ, ref WORK, offset_work, ref INFO);
                }
                // *
                // *        Clean up A
                // *
                _dlaset.Run("Full", K, N - L - K, ZERO, ZERO, ref A, offset_a
                                 , LDA);
            ...
            if (M > K)
            {
                ...
                // *
                _dgeqr2.Run(M - K, L, ref A, K + 1+(N - L + 1) * LDA + o_a, LDA, ref TAU, offset_tau, ref WORK, offset_work
                                 , ref INFO);
                // *
                if (WANTU)
                {
                    ...
                    // *
                    _dorm2r.Run("Right", "No transpose", M, M - K, Math.Min(M - K, L), ref A, K + 1+(N - L + 1) * LDA + o_a
