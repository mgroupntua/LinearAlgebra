...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLAMCH
    {
        ...
        double dlamch = 0;
            ...
            var LRND = false; var BETA = 0; var IMAX = 0; var IMIN = 0; var IT = 0; double RMACH = 0; double SMALL = 0; 
            #endregion
            #region Strings
            ...
            if (FIRST)
            {
                _dlamc2.Run(ref BETA, ref IT, ref LRND, ref EPS, ref IMIN, ref RMIN
                                 , ref IMAX, ref RMAX);
                BASE = BETA;
                T = IT;
                if (LRND)
            ...
            // *
            if (_lsame.Run(CMACH, "E"))
            {
                RMACH = EPS;
            }
            else
            {
                if (_lsame.Run(CMACH, "S"))
                {
                    RMACH = SFMIN;
                }
                else
                {
                    if (_lsame.Run(CMACH, "B"))
                    {
                        RMACH = BASE;
                    }
                    else
                    {
                        if (_lsame.Run(CMACH, "P"))
                        {
                            RMACH = PREC;
                        }
                        else
                        {
                            if (_lsame.Run(CMACH, "N"))
                            {
                                RMACH = T;
                            }
                            else
                            {
                                if (_lsame.Run(CMACH, "R"))
                                {
                                    RMACH = RND;
                                }
                                else
                                {
                                    if (_lsame.Run(CMACH, "M"))
                                    {
                                        RMACH = EMIN;
                                    }
                                    else
                                    {
                                        if (_lsame.Run(CMACH, "U"))
                                        {
                                            RMACH = RMIN;
                                        }
                                        else
                                        {
                                            if (_lsame.Run(CMACH, "L"))
                                            {
                                                RMACH = EMAX;
                                            }
                                            else
                                            {
                                                if (_lsame.Run(CMACH, "O"))
                                                {
                                                    RMACH = RMAX;
                                                }
                                            }
    ...
    public class DLAMC1
    {
        ...
        public void Run(ref int BETA, ref int T, ref bool RND, ref bool IEEE1)
        {
            ...
            LABEL10:;
                ...
                {
                    A *= 2;
                    C = (double)_dlamc3.Run(A, ONE);
                    C = (double)_dlamc3.Run(C, -A);
                    goto LABEL10;
                }
                // *+       END WHILE
                // *
            ...
            LABEL20:;
                ...
                // *
                B = LBETA;
                F = (double)_dlamc3.Run(B / 2, -B / 100);
                C = (double)_dlamc3.Run(F, A);
                if (C == A)
                {
                    LRND = true;
                }
                ...
                // *
                T1 = (double)_dlamc3.Run(B / 2, A);
                T2 = (double)_dlamc3.Run(B / 2, SAVEC);
    ...
    public class DLAMC2
    {
        ...
        public DLAMC2(DLAMC3 dlamc3, DLAMC1 dlamc1, DLAMC4 dlamc4, DLAMC5 dlamc5)
        {
            ...
            _dlamc3 = dlamc3; _dlamc1 = dlamc1; _dlamc4 = dlamc4; _dlamc5 = dlamc5; 
            #endregion
            #region Data Initialization
                         ...
                         , ref int EMAX, ref double RMAX)
        {
            ...
            var IEEE = false; var LIEEE1 = false; var LRND = false; var GNMIN = 0; var GPMIN = 0; var I = 0; var NGNMIN = 0; 
            var NGPMIN = 0;double A = 0; double B = 0; double C = 0; double HALF = 0; double ONE = 0; double RBASE = 0; 
            ...
            if (FIRST)
            {
                ...
                // *
                _dlamc1.Run(ref LBETA, ref LT, ref LRND, ref LIEEE1);
                // *
                // *        Start to find EPS.
                // *
                B = LBETA;
                ...
                B = TWO / 3;
                HALF = ONE / 2;
                SIXTH = _dlamc3.Run(B,  - HALF);
                THIRD = _dlamc3.Run(SIXTH, SIXTH);
                B = (double)_dlamc3.Run(THIRD,  - HALF);
                B = (double)_dlamc3.Run(B, SIXTH);
                B = Math.Abs(B);
                if (B < LEPS) B = LEPS;
                // *
                LEPS = 1;
            ...
            LABEL10:;
                ...
                {
                    LEPS = B;
                    C = (double)_dlamc3.Run(HALF * LEPS, Math.Pow(TWO,5) * Math.Pow(LEPS,2));
                    C = (double)_dlamc3.Run(HALF, -C);
                    B = (double)_dlamc3.Run(HALF, C);
                    C = (double)_dlamc3.Run(HALF, -B);
                    B = (double)_dlamc3.Run(HALF, C);
                    goto LABEL10;
                }
                // *+       END WHILE
                ...
                for (I = 1; I <= 3; I++)
                {
                    SMALL = (double)_dlamc3.Run(SMALL * RBASE, ZERO);
                }
                A = (double)_dlamc3.Run(ONE, SMALL);
                _dlamc4.Run(ref NGPMIN, ONE, LBETA);
                _dlamc4.Run(ref NGNMIN,  - ONE, LBETA);
                _dlamc4.Run(ref GPMIN, A, LBETA);
                _dlamc4.Run(ref GNMIN,  - A, LBETA);
                IEEE = false;
                // *
                ...
                for (I = 1; I <= 1 - LEMIN; I++)
                {
                    LRMIN = _dlamc3.Run(LRMIN * RBASE, ZERO);
                }
                // *
                // *        Finally, call DLAMC5 to compute EMAX and RMAX.
                // *
                _dlamc5.Run(LBETA, LT, LEMIN, IEEE, ref LEMAX, ref LRMAX);
            }
            // *
            BETA = LBETA;
            T = LT;
    ...
    public class DLAMC4
    {
        ...
        public void Run(ref int EMIN, double START, int BASE)
        {
            ...
            var I = 0; double A = 0; double B1 = 0; double B2 = 0; double C1 = 0; double C2 = 0; double D1 = 0; double D2 = 0; 
            ...
            ZERO = 0;
            EMIN = 1;
            B1 = (double)_dlamc3.Run(A * RBASE, ZERO);
            C1 = A;
            C2 = A;
            D1 = A;
            D2 = A;
        ...
        LABEL10:;
            if (C1 == A && C2 == A && D1 == A && D2 == A)
            {
                EMIN -= 1;
                A = B1;
                B1 = (double)_dlamc3.Run(A / BASE, ZERO);
                C1 = (double)_dlamc3.Run(B1 * BASE, ZERO);
                D1 = ZERO;
                for (I = 1; I <= BASE; I++)
                {
                    D1 += B1;
                }
                B2 = (double)_dlamc3.Run(A * RBASE, ZERO);
                C2 = (double)_dlamc3.Run(B2 / RBASE, ZERO);
                D2 = ZERO;
                for (I = 1; I <= BASE; I++)
                {
                    D2 += B2;
    ...
    public class DLAMC5
    {
        ...
        public void Run(int BETA, int P, int EMIN, bool IEEE, ref int EMAX, ref double RMAX)
        {
            ...
            var EXBITS = 0; var EXPSUM = 0; var I = 0; var LEXP = 0; var NBITS = 0; var TRY = 0; var UEXP = 0; double OLDY = 0; 
