...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DSTEDC
    {
                      ...
                      , DSTEQR dsteqr, DSTERF dsterf, DSWAP dswap, XERBLA xerbla)
        {
            ...
            _lsame = lsame; _ilaenv = ilaenv; _dlamch = dlamch; _dlanst = dlanst; _dgemm = dgemm; 
            _dlacpy = dlacpy;_dlaed0 = dlaed0; _dlascl = dlascl; _dlaset = dlaset; _dlasrt = dlasrt; 
            _dsteqr = dsteqr;_dsterf = dsterf; _dswap = dswap; _xerbla = xerbla; 
                         ...
                         , ref double[] WORK, int offset_work, int LWORK, ref int[] IWORK, int offset_iwork, int LIWORK, ref int INFO)
        {
            ...
            var LQUERY = false; var FINISH = 0; var I = 0; var ICOMPZ = 0; var II = 0; var J = 0; var K = 0; var LGN = 0; 
            var LIWMIN = 0;var LWMIN = 0; var M = 0; var SMLSIZ = 0; var START = 0; var STOREZ = 0; var STRTRW = 0; 
            ...
            #region Array Index Correction
             ...
             var o_d = -1 + offset_d;  var o_e = -1 + offset_e;  var o_z = -1 - LDZ + offset_z;  var o_work = -1 + offset_work; 
            ...
            // *
            if (_lsame.Run(COMPZ, "N"))
            {
                ICOMPZ = 0;
            }
            else
            {
                if (_lsame.Run(COMPZ, "V"))
                {
                    ICOMPZ = 1;
                }
                else
                {
                    if (_lsame.Run(COMPZ, "I"))
                    {
                        ICOMPZ = 2;
                    }
                    else
            ...
            if (INFO == 0)
            {
                ...
                // *
                SMLSIZ = _ilaenv.Run(9, "DSTEDC", " ", 0, 0, 0, 0);
                if (N <= 1 || ICOMPZ == 0)
                {
                    LIWMIN = 1;
                    LWMIN = 1;
            ...
            if (ICOMPZ == 0)
            {
                _dsterf.Run(N, ref D, offset_d, ref E, offset_e, ref INFO);
                goto LABEL50;
            }
            // *
            ...
            {
                // *
                _dsteqr.Run(COMPZ, N, ref D, offset_d, ref E, offset_e, ref Z, offset_z, LDZ
            ...
            else
            {
                ...
                if (ICOMPZ == 2)
                {
                    _dlaset.Run("Full", N, N, ZERO, ONE, ref Z, offset_z
                                     , LDZ);
                }
                // *
                // *        Scale.
            ...
            LABEL10:;
                ...
                LABEL20:;
                    ...
                    if (M > SMLSIZ)
                    {
                        ...
                        // *              Scale.
                        // *
                        ORGNRM = _dlanst.Run("M", M, D, START + o_d, E, START + o_e);
                        _dlascl.Run("G", 0, 0, ORGNRM, ONE, M
                                         , 1, ref D, START + o_d, M, ref INFO);
                        _dlascl.Run("G", 0, 0, ORGNRM, ONE, M - 1
                        ...
                        _dlaed0.Run(ICOMPZ, N, M, ref D, START + o_d, ref E, START + o_e, ref Z, STRTRW+START * LDZ + o_z
                        ...
                        // *              Scale back.
                        // *
                        _dlascl.Run("G", 0, 0, ONE, ORGNRM, M
                                         , 1, ref D, START + o_d, M, ref INFO);
                        // *
                    }
                    else
                    {
                        if (ICOMPZ == 1)
                        {
                            ...
                            // *
                            _dsteqr.Run("I", M, ref D, START + o_d, ref E, START + o_e, ref WORK, offset_work, M
                                             , ref WORK, M * M + 1 + o_work, ref INFO);
                            _dlacpy.Run("A", N, M, Z, 1+START * LDZ + o_z, LDZ, ref WORK, STOREZ + o_work
                                             , N);
                            _dgemm.Run("N", "N", N, M, M, ONE
                        ...
                        else
                        {
                            if (ICOMPZ == 2)
                            {
                                _dsteqr.Run("I", M, ref D, START + o_d, ref E, START + o_e, ref Z, START+START * LDZ + o_z, LDZ
                                                 , ref WORK, offset_work, ref INFO);
                            }
                            else
                            {
                                _dsterf.Run(M, ref D, START + o_d, ref E, START + o_e, ref INFO);
                            }
                        }
                        if (INFO != 0)
                        {
                    ...
                    else
                    {
                        ...
                        for (II = 2; II <= N; II++)
                        {
                            ...
                            if (K != I)
                            {
                                D[K + o_d] = D[I + o_d];
                                D[I + o_d] = P;
                                _dswap.Run(N, ref Z, 1+I * LDZ + o_z, 1, ref Z, 1+K * LDZ + o_z, 1);
                            }
                        }
                    }
                }
