...
namespace MGroup.LinearAlgebra.Implementations.DotNumerics
{
    ...
    public class DLASDQ
    {
        ...
        public DLASDQ(DBDSQR dbdsqr, DLARTG dlartg, DLASR dlasr, DSWAP dswap, XERBLA xerbla, LSAME lsame)
        {
            ...
            _dbdsqr = dbdsqr; _dlartg = dlartg; _dlasr = dlasr; _dswap = dswap; _xerbla = xerbla; 
            _lsame = lsame;
                         ...
                         , ref double[] C, int offset_c, int LDC, ref double[] WORK, int offset_work, ref int INFO)
        {
            ...
            var ROTATE = false; var I = 0; var ISUB = 0; var IUPLO = 0; var J = 0; var NP1 = 0; var SQRE1 = 0; double CS = 0; 
            ...
            if (IUPLO == 1 && SQRE1 == 1)
            {
                ...
                _dlartg.Run(D[N + o_d], E[N + o_e], ref CS, ref SN, ref R);
                D[N + o_d] = R;
                E[N + o_e] = ZERO;
                if (ROTATE)
                {
                ...
                if (NCVT > 0)
                {
                    _dlasr.Run("L", "V", "F", NP1, NCVT, WORK, 1 + o_work
            ...
            if (IUPLO == 2)
            {
                ...
                if (NRU > 0)
                {
                    ...
                    {
                        _dlasr.Run("R", "V", "F", NRU, NP1, WORK, 1 + o_work
                                        , WORK, NP1 + o_work, ref U, offset_u, LDU);
                    }
                }
                if (NCC > 0)
                {
                    ...
                    {
                        _dlasr.Run("L", "V", "F", NP1, NCC, WORK, 1 + o_work
                                        , WORK, NP1 + o_work, ref C, offset_c, LDC);
                    }
                }
            }
            ...
            // *
            _dbdsqr.Run("U", N, NCVT, NRU, NCC, ref D, offset_d
            ...
            for (I = 1; I <= N; I++)
            {
                ...
                if (ISUB != I)
                {
                    ...
                    D[ISUB + o_d] = D[I + o_d];
                    D[I + o_d] = SMIN;
                    if (NCVT > 0) _dswap.Run(NCVT, ref VT, ISUB+1 * LDVT + o_vt, LDVT, ref VT, I+1 * LDVT + o_vt, LDVT);
                    if (NRU > 0) _dswap.Run(NRU, ref U, 1+ISUB * LDU + o_u, 1, ref U, 1+I * LDU + o_u, 1);
                    if (NCC > 0) _dswap.Run(NCC, ref C, ISUB+1 * LDC + o_c, LDC, ref C, I+1 * LDC + o_c, LDC);
                }
            }
            // *
            return;
