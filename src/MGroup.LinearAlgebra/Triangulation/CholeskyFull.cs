...
namespace MGroup.LinearAlgebra.Triangulation
{
	...
	public class CholeskyFull : ITriangulation
	{
		...
		{
			// Call LAPACK
			int indefiniteMinorIdx = GlobalProvider.LapackLinearEquations.Dpotrf(StoredTriangle.Upper, order, matrix, 0, order);
		...
		public Matrix Invert(bool inPlace)
		{
			...
			int indefiniteMinorIdx = GlobalProvider.LapackLinearEquations.Dpotri(StoredTriangle.Upper, Order, inverse, 0, Order);
		...
		public void SolveLinearSystem(Vector rhs, Vector solution)
		{
			...
			GlobalProvider.LapackLinearEquations.Dpotrs(
				StoredTriangle.Upper, Order, numRhs, data, 0, Order, solution.RawData, 0, leadingDimB);
		}
