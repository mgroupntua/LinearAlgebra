...
namespace MGroup.LinearAlgebra.Triangulation
{
	...
	public class CholeskyPacked : ITriangulation
	{
		...
		{
			// Call LAPACK
			int indefiniteMinorIdx = GlobalProvider.LapackLinearEquations.Dpptrf(StoredTriangle.Upper, order, matrix, 0);
		...
		public SymmetricMatrix Invert(bool inPlace)
		{
			...
			int indefiniteMinorIdx = GlobalProvider.LapackLinearEquations.Dpptri(StoredTriangle.Upper, Order, inverse, 0);
		...
		public void SolveLinearSystem(Vector rhs, Vector solution)
		{
			...
			GlobalProvider.LapackLinearEquations.Dpptrs(
				StoredTriangle.Upper, Order, numRhs, data, 0, solution.RawData, 0, leadingDimB);
		}
